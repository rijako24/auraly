...
BEGIN
    ...
    SELECT NEWID(),@OrderId,@BusinessId,j.ProductId,j.Code,j.Code,j.Name,j.Name,j.UnitCode,j.Quantity,j.UnitPrice,j.DocumentUnitCost,j.DiscountAmount,0,j.LineTotal,j.RawPayloadJson,SYSUTCDATETIME()
    FROM OPENJSON(@LinesJson) WITH(
        ProductId UNIQUEIDENTIFIER '$.productId',Code NVARCHAR(100) '$.code',Name NVARCHAR(250) '$.name',
        UnitCode NVARCHAR(24) '$.unitCode',Quantity DECIMAL(19,6) '$.quantity',UnitPrice DECIMAL(19,4) '$.unitPrice',DocumentUnitCost DECIMAL(19,6) '$.documentUnitCost',DiscountAmount DECIMAL(19,4) '$.discountAmount',
