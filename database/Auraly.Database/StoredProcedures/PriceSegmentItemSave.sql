CREATE PROCEDURE [dbo].[PriceSegmentItemSave] @Id UNIQUEIDENTIFIER,@BusinessId UNIQUEIDENTIFIER,@ProductId UNIQUEIDENTIFIER,@MinimumQuantity DECIMAL(19,6),@Amount DECIMAL(19,4),@ValidFrom DATETIMEOFFSET(7),@ValidUntil DATETIMEOFFSET(7)=NULL,@Excluded BIT AS
BEGIN SET NOCOUNT ON;
 IF NOT EXISTS(SELECT 1 FROM dbo.Products WHERE ProductId=@ProductId AND BusinessId=@BusinessId AND IsActive=1) THROW 51004,'Product not found',1;
  ...
  INSERT dbo.ResolvedPriceChannelItems(ResolvedPriceChannelItemId,PriceChannelId,ProductId,MinimumQuantity,Amount,CurrencyCode,ValidFrom,ValidUntil,IsActive,CreatedAt) VALUES(NEWID(),@Id,@ProductId,@MinimumQuantity,@Amount,N'COP',@ValidFrom,@ValidUntil,1,SYSUTCDATETIME());
