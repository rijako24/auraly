...
BEGIN
    ...
    BEGIN
      DECLARE @RouteSellerId UNIQUEIDENTIFIER=(
        SELECT route.SellerId
        FROM dbo.SalesRoutes route
        WHERE route.RouteId=@RouteId AND route.BusinessId=@BusinessId AND route.IsActive=1);
      IF @RouteSellerId IS NULL
        THROW 51300,'La ruta seleccionada no está activa en este negocio.',1;
      IF @SellerId IS NOT NULL AND @SellerId<>@RouteSellerId
    ...
    SELECT @Subtotal=SUM(j.Quantity*j.UnitPrice),@DiscountTotal=SUM(j.DiscountAmount)
    ...
    SELECT NEWID(),@OrderId,@BusinessId,j.ProductId,j.Code,j.Code,j.Name,j.Name,j.UnitCode,j.Quantity,j.UnitPrice,j.DiscountAmount,j.TaxAmount,j.LineTotal,j.RawPayloadJson,SYSUTCDATETIME()
    FROM OPENJSON(@LinesJson) WITH(
