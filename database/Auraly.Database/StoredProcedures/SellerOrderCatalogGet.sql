...
BEGIN
    ...
    SELECT p.ProductId,
           ...
           CASE WHEN listPrice.Amount IS NOT NULL THEN N'PriceList'
                WHEN channelPrice.Amount IS NOT NULL THEN N'PriceChannel' ELSE N'Public' END,
    ...
    OUTER APPLY(SELECT TOP(1)i.Amount FROM dbo.ResolvedPriceChannelItems i
                JOIN dbo.PriceChannels c ON c.PriceChannelId=i.PriceChannelId
                WHERE i.PriceChannelId=setting.PriceChannelId AND c.BusinessId=@BusinessId AND c.IsActive=1
                  AND i.ProductId=p.ProductId AND i.IsActive=1 AND i.ValidFrom<=SYSDATETIMEOFFSET()
                  AND(i.ValidUntil IS NULL OR i.ValidUntil>SYSDATETIMEOFFSET())
                  AND NOT EXISTS(SELECT 1 FROM dbo.PriceChannelExclusions e
