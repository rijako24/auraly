...
BEGIN
    ...
    SELECT p.ProductId,
           ...
           COALESCE(listPrice.Amount,ROUND(basePrice.Amount*(1+channelRule.NumericValue/100),4),basePrice.Amount),
           CASE WHEN listPrice.Amount IS NOT NULL THEN N'PriceList'
                WHEN channel.PriceChannelId IS NOT NULL THEN N'PriceChannel' ELSE N'Public' END,
           ...
           p.ManageStock
    FROM dbo.Products p
    CROSS APPLY(SELECT TOP(1) pp.Amount FROM dbo.ProductPrices pp
    ...
    OUTER APPLY(SELECT TOP(1) r.NumericValue FROM dbo.PriceChannelRules r WHERE r.PriceChannelId=channel.PriceChannelId AND r.RuleKind=N'PercentageVariation' AND r.AppliesTo=N'AllProducts' AND r.IsActive=1 AND r.ValidFrom<=SYSDATETIMEOFFSET() AND(r.ValidUntil IS NULL OR r.ValidUntil>SYSDATETIMEOFFSET()) ORDER BY r.ValidFrom DESC)channelRule
    OUTER APPLY(SELECT TOP(1)i.Amount FROM dbo.PriceListItems i
                JOIN dbo.PriceLists l ON l.PriceListId=i.PriceListId
                WHERE i.PriceListId=setting.PriceListId AND l.BusinessId=@BusinessId AND l.IsActive=1
                  AND i.ProductId=p.ProductId AND i.IsActive=1 AND i.MinimumQuantity<=1
                  AND i.ValidFrom<=SYSDATETIMEOFFSET() AND(i.ValidUntil IS NULL OR i.ValidUntil>SYSDATETIMEOFFSET())
                ORDER BY i.MinimumQuantity DESC,i.ValidFrom DESC)listPrice
