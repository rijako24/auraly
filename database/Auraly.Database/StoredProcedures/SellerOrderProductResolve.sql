...
BEGIN
    ...
    SELECT COALESCE(NULLIF(p.ProductCode,N''),NULLIF(p.Sku,N''),N''),p.Name,
           COALESCE(NULLIF(p.BaseUnitCode,N''),N'EA'),COALESCE(listPrice.Amount,ROUND(basePrice.Amount*(1+channelRule.NumericValue/100),4),basePrice.Amount),
           CASE WHEN listPrice.Amount IS NOT NULL THEN N'PriceList' WHEN channel.PriceChannelId IS NOT NULL THEN N'PriceChannel' ELSE N'Public' END,
