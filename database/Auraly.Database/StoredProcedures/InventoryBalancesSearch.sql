...
BEGIN
    ...
    LEFT JOIN dbo.InventoryBalances b ON b.BusinessId=p.BusinessId AND b.ProductId=p.ProductId AND b.WarehouseId=w.WarehouseId
    WHERE p.BusinessId=@BusinessId AND p.IsActive=1 AND w.BusinessId=@BusinessId AND w.IsActive=1 AND w.IsInventoryVisible=1
    ...
    CROSS JOIN dbo.Warehouses w
    LEFT JOIN dbo.InventoryBalances b ON b.BusinessId=p.BusinessId AND b.ProductId=p.ProductId AND b.WarehouseId=w.WarehouseId
    LEFT JOIN dbo.ProductPrices price ON price.BusinessId=p.BusinessId AND price.ProductId=p.ProductId AND price.IsActive=1
    WHERE p.BusinessId=@BusinessId AND p.IsActive=1 AND w.BusinessId=@BusinessId AND w.IsActive=1 AND w.IsInventoryVisible=1
      AND (@WarehouseId IS NULL OR w.WarehouseId=@WarehouseId)
      AND (@ProductId IS NULL OR p.ProductId=@ProductId)
      AND (@OnlyWithStock=0 OR COALESCE(b.QuantityOnHand,0)<>0)
