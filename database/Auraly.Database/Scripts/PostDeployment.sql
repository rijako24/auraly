-- =============================================================================
-- PostDeployment.sql
-- Conserva los canales creados por la interfaz anterior: su regla activa pasa a la
-- estrategia nativa que consumen POS, pedidos y los catálogos locales.
UPDATE channelValue
SET channelValue.Strategy=N'PercentageOverBasePrice',
    channelValue.Value=legacy.NumericValue
FROM dbo.PriceChannels channelValue
CROSS APPLY(
    SELECT TOP(1) ruleValue.NumericValue
    FROM dbo.PriceChannelRules ruleValue
    WHERE ruleValue.PriceChannelId=channelValue.PriceChannelId
      AND ruleValue.RuleKind=N'PercentageVariation'
      AND ruleValue.AppliesTo=N'AllProducts'
      AND ruleValue.IsActive=1
    ORDER BY ruleValue.ValidFrom DESC
) legacy
WHERE channelValue.Strategy=N'FixedSpecialPrice'
  AND NOT EXISTS(
      SELECT 1 FROM dbo.ResolvedPriceChannelItems item
      WHERE item.PriceChannelId=channelValue.PriceChannelId AND item.IsActive=1
  );
GO
--
