...
DECLARE @WarehouseId UNIQUEIDENTIFIER = '22220001-0000-7000-8000-000000000002';
DECLARE @FiscalAuthorizationId UNIQUEIDENTIFIER = '22220001-0000-7000-8000-000000000004';
DECLARE @FiscalIssuerConfigurationId UNIQUEIDENTIFIER = '22220001-0000-7000-8000-000000000005';
DECLARE @DocumentSeriesId UNIQUEIDENTIFIER = '22220001-0000-7000-8000-000000000006';
DECLARE @FiscalSeriesId UNIQUEIDENTIFIER = '22220001-0000-7000-8000-000000000007';
DECLARE @TaxProfileId UNIQUEIDENTIFIER = '22220001-0000-7000-8000-000000000008';
DECLARE @ProductId UNIQUEIDENTIFIER = '22220001-0000-7000-8000-000000000009';
DECLARE @ProductBarcodeId UNIQUEIDENTIFIER = '22220001-0000-7000-8000-00000000000A';
DECLARE @ProductPriceId UNIQUEIDENTIFIER = '22220001-0000-7000-8000-00000000000B';
DECLARE @OrderOneId UNIQUEIDENTIFIER = '22220001-0000-7000-8000-00000000000C';
DECLARE @OrderTwoId UNIQUEIDENTIFIER = '22220001-0000-7000-8000-00000000000D';
...
IF NOT EXISTS (SELECT 1 FROM dbo.DocumentSeries WHERE DocumentSeriesId = @DocumentSeriesId)
    ...
    VALUES(
        @DocumentSeriesId,@BusinessId,NULL,N'SalesInvoice',
        N'VTA',N'00',8,1,99999999,0,1,@Now);

IF NOT EXISTS (SELECT 1 FROM dbo.FiscalSeries WHERE SeriesId = @FiscalSeriesId)
    INSERT dbo.FiscalSeries(
        SeriesId,BusinessId,DeviceId,EmitterKind,FiscalAuthorizationId,DocumentType,
        Prefix,RangeStart,RangeEnd,IsActive,CreatedAt)
    VALUES(
        @FiscalSeriesId,@BusinessId,NULL,N'Server',@FiscalAuthorizationId,N'SalesInvoice',
...
BEGIN
    ...
    VALUES(
        '22220001-0000-7000-8000-00000000000E',@OrderOneId,@BusinessId,
...
BEGIN
    ...
    INSERT dbo.OrderItems(
        ...
        Quantity,UnitPrice,DiscountAmount,LineTotal,CreatedAt)
    VALUES(
        '22220001-0000-7000-8000-00000000000F',@OrderTwoId,@BusinessId,
        @ProductId,N'DEMO-001',N'Producto Auraly de demostración',
...
SELECT
    ...
    N'Servidor online' AS EmitterName,
