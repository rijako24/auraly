...
(
    ...
    CONSTRAINT [FK_ServerOutboxMessages_Tenants] FOREIGN KEY ([TenantId]) REFERENCES [dbo].[Tenants] ([TenantId]),
    CONSTRAINT [FK_ServerOutboxMessages_SalesDocuments] FOREIGN KEY ([DocumentId]) REFERENCES [dbo].[SalesDocuments] ([DocumentId]),
    CONSTRAINT [UQ_ServerOutboxMessages_Document_Type] UNIQUE ([TenantId], [DocumentId], [Type])
);

GO

CREATE INDEX [IX_ServerOutboxMessages_Pending]
    ON [dbo].[ServerOutboxMessages] ([TenantId], [ProcessedAt], [OccurredAt]);
