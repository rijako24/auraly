CREATE TABLE [dbo].[OrderClaims] (
    ...
    CONSTRAINT [FK_OrderClaims_Businesses] FOREIGN KEY ([BusinessId])
        REFERENCES [dbo].[Businesses] ([BusinessId]),
    CONSTRAINT [FK_OrderClaims_Orders] FOREIGN KEY ([OrderId])
        REFERENCES [dbo].[Orders] ([OrderId]),
    CONSTRAINT [FK_OrderClaims_Registers] FOREIGN KEY ([RegisterId])
        REFERENCES [dbo].[CashRegisters] ([RegisterId]),
    CONSTRAINT [FK_OrderClaims_Devices] FOREIGN KEY ([DeviceId])
        REFERENCES [dbo].[PosDevices] ([DeviceId]),
    CONSTRAINT [FK_OrderClaims_Users] FOREIGN KEY ([UserId])
...
CREATE TABLE [dbo].[OrderInvoiceBatchReceipts] (
    ...
    CONSTRAINT [FK_OrderInvoiceBatchReceipts_Businesses] FOREIGN KEY ([BusinessId])
        REFERENCES [dbo].[Businesses] ([BusinessId]),
    CONSTRAINT [FK_OrderInvoiceBatchReceipts_Registers] FOREIGN KEY ([RegisterId])
        REFERENCES [dbo].[CashRegisters] ([RegisterId]),
    CONSTRAINT [FK_OrderInvoiceBatchReceipts_Users] FOREIGN KEY ([UserId])
        REFERENCES [dbo].[AppUsers] ([UserId]),
    CONSTRAINT [UQ_OrderInvoiceBatchReceipts_Business_Key]
        UNIQUE ([BusinessId], [IdempotencyKey]),
    CONSTRAINT [CK_OrderInvoiceBatchReceipts_Status]
        CHECK ([Status] IN (N'Processing', N'PartiallyCompleted', N'Completed', N'Failed')),
    CONSTRAINT [CK_OrderInvoiceBatchReceipts_Counts]
        CHECK ([RequestedCount] > 0 AND [CompletedCount] >= 0 AND [FailedCount] >= 0),
...
CREATE TABLE [dbo].[OrderInvoiceLinks] (
    ...
    CONSTRAINT [FK_OrderInvoiceLinks_Businesses] FOREIGN KEY ([BusinessId])
        REFERENCES [dbo].[Businesses] ([BusinessId]),
    CONSTRAINT [FK_OrderInvoiceLinks_Orders] FOREIGN KEY ([OrderId])
        REFERENCES [dbo].[Orders] ([OrderId]),
    CONSTRAINT [FK_OrderInvoiceLinks_Documents] FOREIGN KEY ([DocumentId])
        REFERENCES [dbo].[SalesDocuments] ([DocumentId]),
    CONSTRAINT [FK_OrderInvoiceLinks_Batches] FOREIGN KEY ([OperationId])
        REFERENCES [dbo].[OrderInvoiceBatchReceipts] ([OperationId]),
