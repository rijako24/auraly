-- Script para actualizar BusinessInformation (Key: 0) con TODO el contenido consolidado
-- Este script actualiza todos los registros existentes con el contenido completo que incluía:
-- - Persona (antes Key: 0)
-- - GeneralInformation (antes Key: 2)
-- - PlanRules (antes Key: 10)
-- - ServiceDurationRules (duraciones de servicios)
-- - BusinessRules (antes Key: 3)

-- Actualizar todos los registros de BusinessInformation existentes
UPDATE BusinessConfigurations
SET 
    Value = N'Eres María, una asesora comercial experta y muy humana de Mimos Baby Spa, especializada en servicios de spa para bebés. Hablas de forma natural, cálida y conversacional, como si fueras una amiga que conoce mucho sobre el cuidado de bebés.
...
- Toda la información sobre horarios, servicios, duraciones y reglas está en la información del negocio arriba. Úsala para responder preguntas sin necesidad de llamar tools innecesariamente.',
    Description = N'Información completa del negocio consolidada: Persona (María), información general, planes, precios, horarios, reglas de negocio, duraciones de servicios, comportamiento del asesor, herramientas disponibles, TODO',
...
- Toda la información sobre horarios, servicios, duraciones y reglas está en la información del negocio arriba. Úsala para responder preguntas sin necesidad de llamar tools innecesariamente.',
        N'Información completa del negocio consolidada: Persona (María), información general, planes, precios, horarios, reglas de negocio, duraciones de servicios, comportamiento del asesor, herramientas disponibles, TODO',
        1,
        GETUTCDATE()
    );
END
...
SELECT 
    ...
    LEFT(bc.Value, 100) AS ValuePreview,
    bc.Description,
    bc.IsActive
...
PRINT 'Actualización de BusinessInformation completada.';
PRINT 'El contenido ahora incluye TODO: Persona, GeneralInformation, PlanRules, ServiceDurationRules y BusinessRules consolidados.';
GO
