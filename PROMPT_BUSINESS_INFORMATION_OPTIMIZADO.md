# PROMPT OPTIMIZADO PARA BusinessInformation
...
### ✅ Optimizaciones:
1. **Eliminado redundancias** - Información repetida consolidada
2. **Simplificado disponibilidad** - Eliminadas referencias a herramientas técnicas
3. **Enfoque en comportamiento** - Más claro sobre cómo debe actuar el asistente
4. **Mantenido esencial** - Toda la información del negocio, servicios y estilo

### ❌ Eliminado (no se usa):
- Referencias a herramientas (`update_conversation_state`, `check_availability`, `create_reservation`)
- Instrucciones técnicas sobre formato de datos
- Reglas sobre `ConversationState` (el sistema lo maneja automáticamente)
- Flujos transaccionales explícitos (el orquestador los maneja)

### 📝 Nota:
El prompt `ContextFieldsMapping` ya **NO se usa** en el nuevo sistema IA Vendedor porque:
- El sistema maneja el contexto automáticamente mediante `ConversationSession` y `CustomerProfile`
- Las transiciones de estado son automáticas mediante `SalesStateMachine`
- No necesita instrucciones explícitas sobre herramientas
