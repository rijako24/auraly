...
La configuración local `Periféricos` conserva dos flujos comerciales: `Facturas`, compartido por documentos emitidos desde el punto de venta o al facturar pedidos, y `Pedidos`, usado únicamente para imprimir pedidos. Una futura impresión de facturas de servicios debe reutilizar `Facturas`; no puede crear una tercera configuración paralela. Los formatos canónicos siguen siendo `Receipt` (tirilla), `HalfLetter` (media carta), `HalfLegal` (media oficio) y `Letter` (carta), con tirilla de 58 u 80 mm.
