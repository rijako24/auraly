...
`SalesReturnSettlements` también permanece como tronco económico común. Su vínculo al pago original se hace condicional con FK tipadas: pago de venta para `ProductSale` o pago/aplicación contable de servicio para `ServiceInvoice`, exactamente uno cuando la resolución es `Refund`; `CustomerCredit` no referencia un pago y reutiliza `SalesReturnReceivableApplications`/`CustomerCredits`. No se inventa un `SalesPayment` de producto para devolver un servicio.
