...
La facturación recurrente se agrega al ciclo de pagos existente; no se reutiliza el auto-renovado silencioso de `UsageBillingService`. Las `BusinessSubscriptions` actuales siguen siendo la fuente de capacidad/créditos de agentes por negocio, pero su siguiente periodo solo se abre desde una CxC de tenant pagada o dentro de la gracia explícita. La única `TenantSubscription` consolida las líneas recurrentes del tenant —plan empresarial, usuarios, vendedores, cajas, paquetes DIAN, empleados y planes de agentes activos— y genera el documento fuente que el handler existente proyecta en `Receivables`. Para `Monthly` crea la siguiente CxC cada mes; para `Annual`, una vez pagado el año, no crea doce CxC mensuales y genera la siguiente únicamente al acercarse el aniversario. Cada documento fuente conserva snapshot del catálogo, periodicidad, cantidades, impuestos, descuentos, ahorro, moneda y periodo; cambios posteriores no lo reescriben.
...
Wompi y recaudo manual convergen después de validar el medio en un único comando idempotente `SettleTenantReceivable`. Ese comando registra/reutiliza `CustomerPayments`, aplica el recaudo mediante `CustomerPaymentApplications` a la CxC de Auraly, deja la `Receivable` en `Paid`, actualiza la proyección de Suscripción del tenant, activa o renueva el periodo y sus entitlements, y publica la creación de la única factura electrónica correspondiente. La clave de idempotencia incluye `ReceivableId` y transacción/referencia externa. No existe un botón que cambie directamente el estado de la suscripción o de la CxC; revertir un recaudo exige el flujo contable de anulación/devolución y su impacto fiscal, con motivo y auditoría.

El periodo de gracia es configuración de plataforma, 10 días por defecto. En la ancla, una `Receivable` con saldo permanece `Open` pero se proyecta como vencida por `DueDate`; la suscripción pasa a `PastDue`. El tenant conserva operación durante los días 1 a 9 y la capacidad mensual se abre como crédito de gracia trazable. Los avisos previos son configurables (por defecto 5 días antes). La frecuencia vencida también es configurable (por defecto cada 3 días): con gracia 10 se avisa los días 3, 6 y 9 de mora; el día 9 anuncia la fecha de suspensión. Al inicio local del día 10 la suscripción pasa a `Suspended`, se envía aviso final y se bloquea la operación.

Cada evento genera notificación in-app visible en la campanita únicamente para cuentas activas con el rol administrador del tenant; vendedores, cajeros, empleados y demás roles no son destinatarios. El correo es un canal global opcional: habilitado significa que todos los tenants reciben los recordatorios; deshabilitado significa que ninguno los recibe, mientras la campanita permanece obligatoria. No existen excepciones ni interruptores por tenant. Cada entrega tiene clave única `ReceivableId + RecipientUserId + Channel + NotificationKind + DelinquencyDay`, de modo que múltiples administradores reciban su propio aviso y los reintentos no creen duplicados. El outbox registra programado, entregado y error seguro sin guardar contenido sensible innecesario.
...
La campanita y el botón “Pagar ahora” del correo abren una ruta opaca de Auraly que, después de autenticar, lleva a `/dashboard/subscription?receivable={ReceivableId}` y enfoca el detalle autorizado. No se incrusta en el correo una URL estática de checkout Wompi: podría ser reenviada, expirar, pertenecer a una versión anterior de la cuenta o estar ya pagada. Al abrir Auraly se comprueban tenant, rol, saldo y estado actuales y recién entonces el backend crea un intento de pago de uso único.
...
No se crea `TenantBillingCharge` ni una cartera paralela. El cobro de suscripción entra al pipeline existente como `DocumentProcessingJob` con tipo `TenantSubscriptionCharge`; su handler materializa una sola fila en `Receivables`, cuya `SourceDocumentId + SourceDocumentType` ya es única. Periodo, plan, cantidades, impuestos, descuento y snapshot comercial permanecen en el documento fuente de suscripción; la cartera conserva cliente, número, vencimiento, importe y saldo como hoy. “Vencida” se deriva de `DueDate` y saldo abierto, no agrega otro estado. La vista Suscripción es una proyección autorizada de esa misma CxC y la vista global de Auraly consulta las mismas filas con alcance de plataforma.

Al aprobar Wompi o el recaudo manual, `CustomerPayments` y `CustomerPaymentApplications` existentes aplican el pago a esa CxC. El evento procesado reclama la combinación `ReceivableId + PaymentId` y genera una sola `SalesDocument` de contado, ya cubierta por el recaudo; no vuelve a crear cartera. Contablemente, el cobro previo usa la cuenta puente configurada para suscripciones y la factura reclasifica el valor a ingreso e impuestos mediante el mismo procesador contable. Webhook, verificación inmediata, conciliador y registro manual convergen en la misma aplicación y reciben el mismo `SalesDocumentId`. La interfaz distingue `Cuenta por cobrar`, `Pago aplicado` y `Factura electrónica emitida`; un cobro no pagado no inventa factura y un pago nunca genera dos.
...
La unidad canónica facturable sigue siendo `Products`, entendida técnicamente como catálogo de ítems vendibles. Ya contiene código, unidad UBL, perfil tributario, moneda, precio y `ManageStock`; actualmente un registro con `ManageStock=0` pasa por ventas y no genera movimiento de inventario. Se agrega `ItemKind` con valores `Product` y `Service`, predeterminado `Product` para todas las filas históricas. Los planes, adicionales, prorrateos y ajustes de Auraly son ítems `Service`, no inventariables y versionados, dentro de la empresa emisora de plataforma. La interfaz cambia el rótulo contextual a “Producto o servicio”, pero no se renombra físicamente la tabla ni el identificador en esta migración.

La tabla `Services` existente conserva su responsabilidad de agenda, duración, recursos y fulfillment. No se convierte en catálogo fiscal ni se copian allí impuestos y unidades. Cuando un servicio agendable también se cobra, puede referenciar un `BillingProductId` de tipo `Service`; la reserva y la factura comparten identidad comercial mediante esa referencia sin mezclar sus ciclos de vida. Una suscripción de Auraly no crea una fila de agenda.

`SalesDocumentLines` sigue siendo el único detalle y `ProductId` continúa obligatorio. Se agrega únicamente `ItemKindSnapshot`; código, nombre, unidad, perfil tributario, cantidad, precio, descuentos e impuestos ya se congelan en la línea y el snapshot UBL. Para `Product`, el handler conserva kardex, costo, marca y atribución de proveedor cuando correspondan. Para `Service`, exige `ManageStock=0`, deja costo/proveedor/inventario vacíos y el writer existente no genera kardex. Impuestos, retenciones, totales, notas crédito/débito, CUFE, QR, contabilidad y representación gráfica usan el mismo encabezado y las mismas líneas; no existe una segunda grilla ni un segundo generador.

Los contratos actuales conservan `ProductId` para compatibilidad y agregan `ItemKind` solo donde sea necesario validar o mostrar el snapshot. El servidor siempre vuelve a leer el tipo y el perfil tributario del ítem; no confía en el discriminador del navegador. El POS puede buscar servicios facturables mediante el mismo catálogo cuando estén habilitados, sin tratarlos como inventario. Devoluciones físicas se permiten solo para `Product`; la corrección de un servicio usa nota crédito/débito sobre la línea original y el motor fiscal existente.

El contrato de recepción se versiona/generaliza sin romper `PosSaleUploadRequest`: el adaptador POS sigue exigiendo dispositivo y sesión; `SourceMode = PlatformSubscription` exige `ReceivableId`, `PaymentId`, cliente e ítems de servicio, y no crea una sesión de caja ficticia. Ambos convergen antes de `DocumentProcessingJobs` en el mismo documento aceptado. El handler canónico valida el contexto según origen, inserta las mismas tablas y entrega al proceso fiscal existente. No se crea otra numeración, writer fiscal, cola o worker.
...
La factura no se envía al confirmar el pago, sino cuando el proceso fiscal queda `DianAccepted` y existe la respuesta de validación. `SqlFiscalSubmissionWorkStore` ya publica `FiscalDocument.DianAccepted` en `ServerOutboxMessages` y conserva `SignedXml` y `DianApplicationResponse` en `FiscalArtifacts`; el consumidor de entrega documental extiende ese evento, sin consultar estados por un timer ni volver a enviar a DIAN. Reclama una entrega única por `DocumentId + RecipientEmailSnapshot + DeliveryKind + ArtifactVersion`. El correo usa la dirección capturada en el snapshot fiscal del cliente. Si no existe, no se intenta SMTP: la factura queda descargable en Suscripción y una entrega posterior explícita exige guardar primero un correo válido, sin alterar el XML ya emitido.
...
Pruebas adicionales obligatorias: cada tenant aprovisionado por pago o exención queda vinculado como cliente de Auraly; reintentos concurrentes reutilizan una sola Party; NIT existente compatible se vincula y una coincidencia incompatible queda en revisión; el cliente no aparece como tercero propio del tenant; la suscripción usa una `Receivable` y no otra cartera; pago, webhook y conciliador concurrentes producen una aplicación, una factura y ninguna segunda CxC; ítem `Service` conserva `ProductId`, no genera kardex, costo ni sesión; `Product` conserva su comportamiento; importes de factura coinciden con el quote pagado; aislamiento del cliente por tenant; DIAN rechazada no envía correo; aceptación crea una entrega; dos eventos/reintentos SMTP reutilizan un solo ZIP y no duplican factura; correo ausente no intenta SMTP y conserva descarga; ZIP único menor o igual a 2 MB contiene `AttachedDocument`, Invoice, ApplicationResponse y PDF opcional del mismo CUFE y `DocumentId`; asunto cumple el anexo; nota crédito de servicio usa la misma línea origen.
