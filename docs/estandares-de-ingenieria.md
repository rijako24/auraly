...
- Toda operación que reciba una colección se diseña por lote de extremo a extremo: contrato, resolución, autorización, persistencia y efectos. No se acepta envolver un handler unitario en un `foreach` que produzca una consulta, transacción o llamada remota por elemento. Las validaciones de lote devuelven sus novedades agrupadas y conservan una sola regla propietaria.
...
- Cada recurso de servidor presentado en una pantalla tiene un solo propietario de carga y refresco. Una vista oculta o duplicada no permanece montada haciendo I/O; un contador no vuelve a consultar una lista que ya devolvió su total; una mutación reutiliza el estado autoritativo que devuelve y no fuerza un refetch para reconstruirlo.
- El estado operativo cambia por push o por una acción explícita. No se agrega polling periódico de red. Solo un lease activo puede tener heartbeat: agenda el siguiente intento después de terminar el anterior, corre únicamente mientras conserva el lease, usa un intervalo menor a su expiración y se detiene al fallar o liberar. WebSocket y SSE deben usar la política compartida de reconexión con backoff, exclusión de intentos simultáneos y máximo finito; agotado el presupuesto solo una señal explícita de conectividad, visibilidad o usuario puede abrir un ciclo nuevo.
...
- Los indicadores de carga y progreso se modelan por etapa. El texto visible, la métrica y la bandera activa deben finalizar cuando termina esa operación; no se reutiliza `cargando`, `sincronizando` o `actualizando precios` para ocultar una cadena posterior de comandos distintos.
