...
3. La API autentica dispositivo, empresa, caja, bodega y permiso; persiste la cabecera, el snapshot, el trabajo operativo y el proceso fiscal en SQL Server. Las líneas, pagos y demás efectos económicos se confirman dentro del trabajo operativo.
...
El onboarding presenta los ambientes como una progresión, no como un interruptor reversible. El asistente entrega directamente al motor fiscal un snapshot técnico tipado: factura usa `DianInvoiceUblBuilder` y documento soporte usa `DianSupportDocumentUblBuilder`; ambos reutilizan `FiscalDocuments`, `FiscalDocumentProcesses`, firma, workers, artefactos y transporte canónicos. No crea una venta, recepción o gasto y, por tanto, no genera líneas comerciales, pagos, cartera, movimientos de inventario o sesión, outbox comercial, trabajos contables ni proyecciones de analítica. La factura de habilitación aprovisiona de forma idempotente la numeración pública estándar `SETP`; documento soporte usa la resolución de soporte ya asignada a la sede y conserva su consecutivo en el cursor canónico. El `TestSetId` se resuelve por tenant, negocio y familia al iniciar el comando y se congela en ese proceso para que sus reintentos y `GetStatusZip` consulten el mismo set; los documentos productivos conservan `TestSetId = null`. Al activar producción se desactivan las autorizaciones y series de factura de prueba, de modo que nunca compitan con la numeración productiva. La aceptación se calcula de forma independiente para factura y documento soporte.
