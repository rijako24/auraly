...
| Rentabilidad | `RentabilidadDeVenta` | Expone costo/margen bajo permiso | Snapshot de costo del documento | Lectura con `sales.lines.cost-margin.read`; edición con `sales.change-price` y sólo sin control de inventario | Conservar el propietario documental | Prueba de ocultamiento, permisos y recálculo |
