...
real es la API y la vista administrativa, tanto individual como masiva. Todavía
no se conectó el productor automático del bot a un evento de reconciliación.

La siguiente extensión debe publicar ese evento desde la transacción que actualiza
el registro externo y consumirlo mediante el transporte durable existente, sin
sondear la tabla. No se dejó una interfaz vacía para afirmar que esa automatización
ya funciona.
