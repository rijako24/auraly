...
- La demanda de inventario de una presentacion vinculada se resuelve y agrupa en unidades del producto padre mediante `InventoryDemandResolver`. Los validadores reciben producto y cantidad y hacen esa resolucion internamente; UI, endpoints, SQL Server y SQLite solo suministran hechos y no replican la aritmetica.
- Toda validacion de una familia de inventario carga sus productos y saldos en lote. No se permiten consultas por linea ni recorridos de catalogo no afectados. El writer canonico vuelve a resolver producto padre y factor dentro de la escritura transaccional para preservar integridad ante concurrencia.
- Existe exactamente un `InventoryBalances` por negocio, bodega y producto, incluidas las bodegas del sistema; se aprovisiona con cantidad y valor en cero al crear cualquiera de los dos maestros, y con costo promedio inicial igual al costo de la ficha cuando existe. Toda consulta operativa de existencias lee esa proyeccion y nunca suma ni reconstruye el saldo desde `InventoryMovements`. Cada movimiento confirmado actualiza el balance y agrega el kardex en la misma transaccion; el conteo fisico converge por ese mismo motor.
