    ...
    private static string RenderCopy(OnlineSalesReceipt receipt)
    {
        ...
        return $$"""
          <article class="document"><div class="document-content">
            <header class="top"><div><div class="brand-lockup">{{companyLogo}}<h1>{{companyName}}</h1></div><h2>{{documentName}}</h2></div><div class="number"><strong>{{Encode(receipt.DocumentNumber)}}</strong><br>{{issuedAt}}</div></header>
