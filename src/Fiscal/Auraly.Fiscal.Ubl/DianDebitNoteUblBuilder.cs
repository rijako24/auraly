...
public sealed class DianDebitNoteUblBuilder
{
    private static readonly XNamespace Note = DianUblNamespaces.DebitNote;
    ...
    public DianUblDocument Build(DianDebitNote note)
    {
        ArgumentNullException.ThrowIfNull(note);
        note.Validate();
        var issuedAt = DianFiscalDateTime.InColombia(note.IssuedAt);
        var root = new XElement(Note + "DebitNote",
            ...
            new XAttribute(Xsi + "schemaLocation", $"{Note} UBL-DebitNote-2.1.xsd"),
            Extensions(note),
            E(Cbc, "UBLVersionID", "UBL 2.1"),
            E(Cbc, "CustomizationID", note.OperationCode),
            E(Cbc, "ProfileID", "DIAN 2.1: Nota Débito de Factura Electrónica de Venta"),
            ...
            note.Lines.Select(line => DebitLine(line, note.CurrencyCode)));
    ...
    private static XElement Extensions(DianDebitNote note)
    {
    ...
    private static XElement Address(DianAddress value, string name = "Address") =>
        new(Cac + name,
            ...
            new XElement(Cac + "Country",
                ...
                new XElement(Cbc + "Name", new XAttribute("languageID", "es"), value.CountryName)));

    private static XElement RequestedMonetaryTotal(DianDebitNote note) =>
        new(Cac + "RequestedMonetaryTotal",
            MoneyElement("LineExtensionAmount", note.UntaxedAmount, note.CurrencyCode),
            MoneyElement("TaxExclusiveAmount", note.UntaxedAmount, note.CurrencyCode),
            MoneyElement("TaxInclusiveAmount", note.TotalAmount, note.CurrencyCode),
            MoneyElement("PayableAmount", note.TotalAmount, note.CurrencyCode));

    private static XElement DebitLine(DianDebitNoteLine line, string currency) =>
        new(Cac + "DebitNoteLine",
            E(Cbc, "ID", line.Number),
            new XElement(Cbc + "DebitedQuantity", new XAttribute("unitCode", line.UnitCode), Number(line.Quantity)),
            ...
            new XElement(Cac + "Item",
                ...
                new XElement(Cac + "StandardItemIdentification",
                    new XElement(Cbc + "ID", new XAttribute("schemeID", "999"), $"ND-{line.Number}"))),
