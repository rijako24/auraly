...
public sealed class CartCommandBatchProcessorTests
{
    ...
    public async Task Apply_WithMoreThanOneOrderGroup_RejectsWholeBatchBeforeResolution()
    {
        ...
        result.Success.Should().BeFalse();
        result.Code.Should().Be("cart.multiple_orders");
    ...
    private sealed class StubResolver : ICartProductResolver
    {
        ...
        public Task<IReadOnlyList<ProductReference>> FindAsync(AgentToolContext context, string productText, CancellationToken cancellationToken = default)
        {
            Calls++;
    ...
    private sealed class StubStore : ICartMutationStore
    {
        ...
        public Task<OrderSnapshot> GetCurrentAsync(AgentToolContext context, CancellationToken cancellationToken = default) => Task.FromResult(_current);

        public Task<OrderSnapshot> ApplyAtomicallyAsync(AgentToolContext context, IReadOnlyList<ResolvedCartCommand> commands, CancellationToken cancellationToken = default)
        {
            ApplyCalls++;
            Applied = commands;
