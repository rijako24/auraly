...

{
...

    {
        ...
        services.AddSingleton<IPackContextLoader, BookingPackContextLoader>();

        services.AddSingleton<IIdentityAttributeService, IdentityAttributeService>();

        services.AddSingleton<IFactSourceResolver, ChannelPhoneResolver>();

        services.AddSingleton<IFactSourceResolver, ChannelEmailResolver>();

        services.AddSingleton<IFactSourceResolver, EngagementResolver>();
        ...
        services.AddSingleton<IFlowLlm>(sp =>
            new FlowLlm(
                sp.GetRequiredService<MimosBabySpa.Application.LLM.IChatClient>(),
                sp.GetRequiredService<ILogger<FlowLlm>>()));
...

        {
            ...
            [
                ...
                new SetFactTool(
                    ...
                    sp.GetRequiredService<IFactAccessor>(),

                    sp.GetRequiredService<IAddOnCatalogService>(),

                    sp.GetRequiredService<IConversationVerificationService>(),

                    sp.GetRequiredService<IIdentityAttributeService>(),
