...
public static class ReceivablesApi
{
    public static IEndpointRouteBuilder MapReceivablesApi(this IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapGet("/api/commerce/v1/receivable-payments",async(HttpContext context,int page,int pageSize,string? search,Guid? customerId,ReceivablesService service,CancellationToken token)=>
            await Execute(()=>service.ListPaymentsAsync(context.User.ToReceivablesIdentity(),new(page,pageSize,search,customerId),token),Results.Ok)).RequireAuthorization("receivables.user");
