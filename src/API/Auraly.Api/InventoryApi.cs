...
public static class InventoryApi
{
    public static IEndpointRouteBuilder MapInventoryApi(this IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapPost("/api/commerce/v1/inventory/physical-counts/{countId:guid}/drafts/{draftId:guid}/discard", async (ClaimsPrincipal principal, Guid countId, Guid draftId, DiscardInventoryPhysicalCountDraftRequest request, InventoryPhysicalCountService service, CancellationToken token) =>
            await ExecuteAsync(async () => { await service.DiscardDraftAsync(principal.ToInventoryIdentity(), countId, draftId, request, token); return true; }, _ => Results.NoContent())).RequireAuthorization("inventory.user");
