...
public static class InventoryApi
{
    public static IEndpointRouteBuilder MapInventoryApi(this IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapGet("/api/commerce/v1/inventory/physical-count-drafts", async (ClaimsPrincipal principal, Guid? warehouseId, string? search, InventoryPhysicalCountService service, CancellationToken token) =>
            await ExecuteAsync(() => service.ListDraftsAsync(principal.ToInventoryIdentity(), warehouseId, search, token), Results.Ok)).RequireAuthorization("inventory.user");
