...
public static class SellerOrdersApi
{
    ...
    public sealed record SellerCatalogItem(Guid ProductId, string ProductCode, string? Reference, string Name, string UnitCode,
        decimal UnitPrice, string PriceSource, decimal QuantityOnHand, bool ManageStock, decimal DocumentUnitCost);
    ...
    ILogger<SellerOrderWriter> logger)
{
        ...
        SellerOrdersApi.UpdateSellerOrderRequest request,CancellationToken token)
    {
        ...
        if(orderId==Guid.Empty||request.CustomerId==Guid.Empty||string.IsNullOrWhiteSpace(request.IdempotencyKey)||request.Lines.Count is <1 or >500||request.Lines.Any(line=>line.ProductId==Guid.Empty||line.Quantity<=0||line.UnitPrice is null or <0||line.DiscountAmount<0||line.DocumentUnitCost<0||line.Description?.Length>250)||request.Notes?.Length>1000)
        ...
        if(!canFullyEdit)
        {
            ...
            if(editable.Status!=5)
            {
                var unchanged=requested.Length==editable.Lines.Count&&requested.All(input=>
                    editable.Lines.SingleOrDefault(original=>original.Position==input.Position) is { } original
                    && input.ProductId==original.ProductId && input.Quantity==original.Quantity);
                if(!unchanged)
                    throw new SellerOrderConflictException("El permiso de revisión solo permite resolver pedidos con faltantes de inventario.");
                var replayTotal=editable.Lines.Sum(line=>decimal.Round(line.Quantity*line.UnitPrice-line.DiscountAmount,2,MidpointRounding.AwayFromZero));
                await transaction.CommitAsync(token);
                return new(orderId,editable.Number,"Confirmed",replayTotal,false,[]);
            }
            if(requested.Any(input=>editable.Lines.All(original=>original.Position!=input.Position||original.ProductId!=input.ProductId)))
                throw new SellerOrderConflictException("Durante la revisión no se pueden agregar productos nuevos.");
            foreach(var original in editable.Lines)
            {
                var submitted=requested.SingleOrDefault(input=>input.Position==original.Position);
        ...
        try
        {
            var metadataOnly=editable.Status==2
                ...
                && requested.All(input=>editable.Lines.SingleOrDefault(original=>original.Position==input.Position) is { } original
                    ...
                    && (input.DocumentUnitCost??original.DocumentUnitCost)==original.DocumentUnitCost
                    && (string.IsNullOrWhiteSpace(input.Description)?original.Description:input.Description.Trim())==original.Description);
            if(metadataOnly)
            {
                ...
                logger.LogInformation(
                    "Seller order {OrderId} metadata updated in {ElapsedMs} ms (open {OpenMs}, editable {EditableMs}, update {UpdateMs}, commit {CommitMs}).",
                    orderId,timing.ElapsedMilliseconds,openedAt,editableAt-openedAt,metadataUpdatedAt-editableAt,metadataCommittedAt-metadataUpdatedAt);
            ...
            var factsInput=requested.Concat(editable.Lines
                ...
                .Select(original=>new NormalizedOrderLine(
                    DeterministicGuid($"seller-order-edit-original:{original.Position}:{original.ProductId:N}"),
                    original.Position,original.ProductId,original.Quantity,original.UnitPrice,
                    original.DiscountAmount,original.PriceSource,original.Description,
            ...
            foreach(var input in requested)
            {
                ...
                var original=editable.Lines.SingleOrDefault(value=>value.Position==input.Position);
                ...
                var isGenericProduct=original?.IsGenericProductSnapshot??line.IsGenericProductSnapshot;
                ...
                var documentUnitCost=preserveCommercialValues
                    ? original!.DocumentUnitCost
                    : input.DocumentUnitCost??original?.DocumentUnitCost??line.DocumentUnitCost;
                var description=preserveCommercialValues
                    ? original!.Description
                    : string.IsNullOrWhiteSpace(input.Description)?line.Name:input.Description.Trim();
                if(unitPrice<=0)
                    throw new SellerOrderValidationException(
                        $"{description}: define un precio de venta mayor que cero antes de guardar el pedido.");
                lines.Add(line with{Quantity=input.Quantity,UnitPrice=unitPrice,PriceSource=priceSource,DiscountAmount=discountAmount,Position=++position,DocumentUnitCost=documentUnitCost,Name=description,IsGenericProductSnapshot=isGenericProduct});
            }
            var previousDemand=InventoryDemandResolver.Resolve(editable.Lines
                .Where(line=>line.ReservedQuantity>0)
                .Select(line=>
                {
                    var fact=resolvedLines[line.ProductId];
                    return new InventoryDemandLine(
                        DeterministicGuid($"seller-order-edit-demand:{line.Position}:{line.ProductId:N}"),line.ProductId,fact.InventoryProductId,fact.InventoryFactor,
                        line.ReservedQuantity,fact.ManageStock);
                })).ToDictionary(value=>value.InventoryProductId);
            var availableAfterRelease=resolvedLines.Values
                .GroupBy(line=>line.InventoryProductId)
                .ToDictionary(group=>group.Key,group=>
                    group.First().Available+
                    (previousDemand.GetValueOrDefault(group.Key)?.RequiredInventoryQuantity??0m));
            var allocations=InventoryDemandResolver.AllocateWholeLines(
                lines.Select(line=>new InventoryDemandLine(
                    DeterministicGuid($"seller-order-edit-allocation:{line.Position}:{line.ProductId:N}"),line.ProductId,line.InventoryProductId,line.InventoryFactor,
                    line.Quantity,line.ManageStock)),availableAfterRelease);
            var allocationByPosition=allocations.Select((value,index)=>(value,index)).ToDictionary(value=>value.index+1,value=>value.value);
            ...
            await SellerOrderReviewPersistence.ReplaceAsync(connection,transaction,orderId,actor.BusinessId,customerId,context.PartySiteId,context.Name,context.Identification,context.Email,context.Phone,context.Address,request.Notes,total,DeterministicGuid($"seller-order-edit:{orderId:N}:{key}"),
                ...
                lines.Select(line=>new SellerOrderReplacementLine(line.ProductId,line.Code,line.Name,line.UnitCode,line.Quantity,line.UnitPrice,line.DocumentUnitCost,line.DiscountAmount,line.LineTotal,line.TaxAmount,JsonSerializer.Serialize(new{line.PriceSource,line.TaxCode,line.TaxRate,Available=InventoryDemandResolver.InProductUnits(line.Available,line.InventoryFactor),ReservedQuantity=line.ManageStock&&line.CanReserve?line.Quantity:0m,LinePosition=line.Position}),line.IsGenericProductSnapshot)).ToArray(),
            ...
            logger.LogInformation(
                "Seller order {OrderId} updated in {ElapsedMs} ms (open {OpenMs}, editable {EditableMs}, context {ContextMs}, facts {FactsMs}, replace {ReplaceMs}, commit {CommitMs}).",
                orderId,timing.ElapsedMilliseconds,openedAt,editableAt-openedAt,contextAt-editableAt,factsAt-contextAt,replacedAt-factsAt,committedAt-replacedAt);
        ...
        SellerOrdersApi.SellerCatalogRequest request,CancellationToken token)
    {
        ...
        while(await reader.ReadAsync(token))candidates.Add(new(
            ...
            reader.GetDecimal(5),reader.GetBoolean(6),reader.GetDecimal(7)));
        ...
        var values=candidates.Select(value=>
        {
            ...
            return new SellerOrdersApi.SellerCatalogItem(
                ...
                price.PriceSource,value.QuantityOnHand,value.ManageStock,value.DocumentUnitCost);
        }).ToList();
        ...
        try
        {
            ...
            var allocations=InventoryDemandResolver.AllocateWholeLines(
                requested.Select(input=>
                {
                    ...
                    return new InventoryDemandLine(
                        input.LineId,input.ProductId,fact.InventoryProductId,
            ...
            var allocationByLine=allocations.ToDictionary(value=>value.Line.LineId);
            ...
            var legacyPrices=missingPrices.Length==0
                ...
                : await SqlOnlineSalesDraftStore.ResolveCommercePricesAsync(
                    ...
                    missingPrices.Select(input=>new CommercePriceRequest(
                        input.LineId.ToString("D"),input.ProductId,input.Quantity)).ToArray(),token,
                    independentLines:true);
            foreach(var input in requested)
            {
                var line=resolvedLines[input.ProductId];
            ...
            var persistedLines=lines.Select(line=>{var lineTotal=line.LineTotal;var reservedQuantity=line.ManageStock&&line.CanReserve?line.Quantity:0m;return new{productId=line.ProductId,code=line.Code,name=line.Name,unitCode=line.UnitCode,quantity=line.Quantity,unitPrice=line.UnitPrice,documentUnitCost=line.DocumentUnitCost,isGenericProductSnapshot=line.IsGenericProductSnapshot,discountAmount=line.DiscountAmount,taxAmount=line.TaxAmount,lineTotal,rawPayloadJson=JsonSerializer.Serialize(new{line.PriceSource,line.TaxCode,line.TaxRate,Available=InventoryDemandResolver.InProductUnits(line.Available,line.InventoryFactor),ReservedQuantity=reservedQuantity,LinePosition=line.Position})};});
    ...
    private static async Task<IReadOnlyDictionary<Guid,OrderLine>> ResolveProductFactsAsync(
        ...
        Guid customerId,IReadOnlyCollection<NormalizedOrderLine> inputs,
        CancellationToken token)
    {
        var lines=new Dictionary<Guid,OrderLine>();
        var inputByProductId=inputs.GroupBy(input=>input.ProductId)
        ...
        await using(var command=Procedure("dbo.SellerOrderProductResolve",connection,transaction))
        {
            command.Parameters.AddRange([
                ...
                P("@LinesJson",JsonSerializer.Serialize(inputs.GroupBy(input=>input.ProductId).Select(group=>new{productId=group.Key,quantity=group.Sum(input=>input.Quantity)}))) ]);
            await using var reader=await command.ExecuteReaderAsync(token);
            while(await reader.ReadAsync(token))
            {
                ...
                lines.Add(productId,new(
                    productId,reader.GetString(1),string.IsNullOrWhiteSpace(input.Description)?reader.GetString(2):input.Description.Trim(),reader.GetString(3),input.Quantity,0,
                    "Captured",reader.GetDecimal(4),reader.GetBoolean(5),reader.GetDecimal(6),0,
                    reader.GetDecimal(9),0m,reader.GetGuid(7),reader.GetDecimal(8),reader.GetString(10),false,reader.GetBoolean(11)));
            }
        }
    ...
    private static NormalizedOrderLine[] NormalizeOrderLines(IReadOnlyCollection<SellerOrdersApi.SellerOrderLineInput> lines)
    {
        return lines.Select((line,index)=>new NormalizedOrderLine(
            DeterministicGuid($"seller-order-line:{index+1}:{line.ProductId:N}"),
            index+1,line.ProductId,line.Quantity,line.UnitPrice,line.DiscountAmount,
            NormalizePriceSource(line.PriceSource),line.Description,
    ...
    private sealed record OrderLine(Guid ProductId,string Code,string Name,string UnitCode,decimal Quantity,decimal UnitPrice,string PriceSource,decimal Available,bool ManageStock,decimal TaxRate,int Position,decimal DocumentUnitCost,decimal DiscountAmount=0m,Guid InventoryProductId=default,decimal InventoryFactor=1m,string TaxCode="01",bool CanReserve=false,bool IsGenericProductSnapshot=false)
    {
        public decimal LineTotal=>MonetaryRounding.RoundLineAmount(UnitPrice*Quantity-DiscountAmount);
        public decimal TaxAmount=>TaxRate<=0?0:decimal.Round(LineTotal*TaxRate/(100+TaxRate),2,MidpointRounding.AwayFromZero);
    }
    private sealed record NormalizedOrderLine(
        Guid LineId,int Position,Guid ProductId,decimal Quantity,decimal? UnitPrice,
        decimal DiscountAmount,string PriceSource,string? Description,
    ...
    private sealed record SellerCatalogCandidate(
        ...
        decimal QuantityOnHand,bool ManageStock,decimal DocumentUnitCost);
