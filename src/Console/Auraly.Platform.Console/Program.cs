...
using Auraly.Platform.Console.Services;
...
{
    Environment.ExitCode = await Auraly.Platform.Console.LeadQualificationConsoleScenario.RunAsync();
    return;
}

if (args is ["product-resolution-smoke"])
{
    Environment.ExitCode = await Auraly.Platform.Console.ProductResolutionConsoleScenario.RunAsync();
    return;
}
...

{
    ...
    Environment.ExitCode = await Auraly.Platform.Console.PaymentApprovalConsoleScenario.RunAsync();
...

{
    ...
    builder.AddConsoleFormatter<Auraly.Platform.Console.CleanConsoleFormatter, ConsoleFormatterOptions>();

    builder.AddConsole(options => options.FormatterName = Auraly.Platform.Console.CleanConsoleFormatter.FormatterName);
