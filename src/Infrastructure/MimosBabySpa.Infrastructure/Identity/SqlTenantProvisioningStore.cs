    ...
    IAuralyIdGenerator ids) : ITenantProvisioningStore
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            const string sql = """
                ...
                VALUES(@TenantId,@TenantKey,@TradeName,@CompanyEmail,1,@Now);
            ...
            return new(request.ProvisioningRequestId, tenantId, businessId, tenantKey,
                salesWarehouseId, ordersWarehouseId, customerId, adminUserId, "Completed");
        }
        catch
        {
    ...
    private static async Task<ProvisionTenantResult?> FindReceiptAsync(SqlConnection connection, SqlTransaction transaction, Guid requestId, CancellationToken ct)
    {
        ...
        return await reader.ReadAsync(ct)
            ? new(reader.GetGuid(0),reader.GetGuid(1),reader.GetGuid(3),reader.GetString(2),reader.GetGuid(4),reader.GetGuid(5),reader.GetGuid(6),reader.GetGuid(7),reader.GetString(8))
            : null;
    }
