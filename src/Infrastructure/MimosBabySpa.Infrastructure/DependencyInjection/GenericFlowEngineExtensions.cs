...
namespace MimosBabySpa.Infrastructure.DependencyInjection;

/// <summary>
/// Registers the Generic Flow engine and related services (same surface as the Azure Functions host)
/// so the Web API can run agent playground chat and any flow actions.
/// </summary>
public static class GenericFlowEngineExtensions
{
    public static IServiceCollection AddQuantixGenericFlowEngine(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddSingleton(_ =>
        {
            var connectionString = configuration["AzureWebJobsStorage"]
                ?? throw new InvalidOperationException(
                    "AzureWebJobsStorage debe estar configurado para el motor de flujo (Blob / adjuntos).");
