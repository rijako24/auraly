...
internal static class ReservationChangeSupport
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        OperationJsonHelper.TryGetString(arguments, "service", out var service);
        OperationJsonHelper.TryGetString(arguments, "date", out var dateStr);
        OperationJsonHelper.TryGetString(arguments, "time", out var timeStr);
        OperationJsonHelper.TryGetString(arguments, "add_ons", out var addOns);
        OperationJsonHelper.TryGetString(arguments, "add_ons_mode", out var addOnsMode);

        DateOnly? date = null;
        if (!string.IsNullOrWhiteSpace(dateStr))
        {
            if (!AgentDateRules.TryParseDate(dateStr, out var parsedDate))
                return (null, OperationJsonHelper.ErrorWithNextAction(
                    "invalid_date",
                    $"'{dateStr}' is not a valid date.",
                    "collect_valid_date",
                    new { expected_format = "yyyy-MM-dd" }));
            if (AgentDateRules.IsPastDate(parsedDate, ctx.BusinessToday))
                return (null, OperationJsonHelper.Error("past_date", "New date must be today or in the future."));
            date = parsedDate;
        }
        ...
        {
            return (null, OperationJsonHelper.MissingPrerequisites(["service/date/time/add_ons"]));
        }
    ...
    internal static string BuildErrorResult(UpdateReservationChangeResult result)
    {
        ...
        return OperationJsonHelper.ErrorWithNextAction(
            code,
            message,
            nextAction,
            new
