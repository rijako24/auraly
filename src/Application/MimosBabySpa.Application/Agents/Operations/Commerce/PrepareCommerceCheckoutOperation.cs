...
public sealed class PrepareCommerceCheckoutOperation : IAgentOperation
{
private readonly IUnitOfWork _unitOfWork;
    ...
    public PrepareCommerceCheckoutOperation(
    ...
    public OperationDescriptor Descriptor { get; } = new(
        "commerce.prepare_checkout", InputSchema,
        ["order.checkout_ready", "order.checkout_payment_required", "order.checkout_pending_manual_payment", "missing_prerequisites", "order_draft_missing", "product_inactive", "checkout_mode_missing", "invalid_order_total", "payment_link_failed", "manual_payment_failed"],
        ["commerce.checkout.prepare"], [], ["order_intake"]);
        ...
        CancellationToken cancellationToken = default)
    {
        var ctx = context.Session
            ?? throw new InvalidOperationException("commerce.prepare_checkout requires a conversation session.");
        ...
        var order = await _unitOfWork.OrderDrafts.GetActiveByConversationAsync(
            ...
            cancellationToken);
        if (order is null)
            return OperationOutcome.Fail("order_draft_missing", "No active order draft was found.", true);

        var items = await _unitOfWork.OrderDraftItems.GetByDraftIdAsync(ctx.BusinessId, order.OrderDraftId, cancellationToken);
        if (items.Count == 0)
            return OperationOutcome.Fail("missing_prerequisites", "order_items is required.", true);
        var unavailableItems = await _availability.FindUnavailableDraftItemsAsync(ctx.BusinessId, items, cancellationToken);
        if (unavailableItems.Count > 0)
        {
            return OperationOutcome.Fail("product_inactive", "The order contains an unavailable product and cannot be checked out.", true);
        }
        ...
        {
            return OperationOutcome.Fail("checkout_mode_missing", "Checkout mode 'order' is not configured for this agent.");
        }
        ...
        if (missing.Count > 0)
            return OperationOutcome.Fail("missing_prerequisites", $"Missing: {string.Join(", ", missing)}.", true);
        ...
        if (paymentSelection.MissingPaymentMethod)
            return OperationOutcome.Fail("missing_prerequisites", "payment_method is required.", true);
        ...
        if (quote.RequiresManualConfirmation)
        {
            ...
            if (!manualResult.Success)
                return OperationOutcome.Fail("manual_payment_failed", manualResult.ErrorMessage ?? "Failed to prepare manual payment confirmation.", true);
        ...
        else if (quote.PayableCents > 0)
        {
            ...
            if (!linkResult.Success)
                return OperationOutcome.Fail("payment_link_failed", linkResult.ErrorMessage ?? "Failed to generate payment link.", true);
        ...
        var presentation = new OperationPresentation(
            quote.TemplateId, templateData, FragmentRenderMode.Exclusive, FragmentPriority.Required);
        ...
        {
            new SaveVerificationEffect(VerificationFactTypes.CheckoutPrepared, checkoutDependencies, null)
        };
        if (quote.PayableCents <= 0 && !quote.RequiresManualConfirmation)
            effects.Add(new SaveVerificationEffect(VerificationFactTypes.CheckoutNoPaymentPrepared, checkoutDependencies, null));

        var outcomeCode = quote.RequiresManualConfirmation
            ? "order.checkout_pending_manual_payment"
            : quote.PayableCents > 0 ? "order.checkout_payment_required" : "order.checkout_ready";
        return OperationOutcome.Ok(
            outcomeCode,
            new
            {
                checkout_token = $"checkout:{order.OrderDraftId:N}",
    ...
    private static bool TryGetString(JsonElement args, string property, out string? value) =>
        OperationJsonHelper.TryGetString(args, property, out value);

    private static string? GetFact(AgentConversationContext ctx, string key) =>
        ctx.Facts.TryGetValue(key, out var value) ? value : null;

    private static OperationOutcome ToolError(CheckoutPaymentSelectionError error)
    {
        ...
        return OperationOutcome.Fail(error.Code, error.Message, error.Recoverable, context: llm);
    }
