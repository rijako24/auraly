...
public sealed class GetCustomerHistoryOperation : IAgentOperation
{
private readonly IUnitOfWork _unitOfWork;

    public GetCustomerHistoryOperation(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public OperationDescriptor Descriptor => new(Name, ParametersSchema, ["internal.customer_history_loaded"], [], [], []);
    ...
    {
        var session = context.Session ?? throw new InvalidOperationException("internal.get_customer_history requires a conversation session.");
        var json = await ExecuteCoreAsync(arguments, session, cancellationToken);
        return OperationJsonResult.Parse(json, "internal.customer_history_loaded");
    }

    public string Name => "internal.get_customer_history";
    ...
    {
        OperationJsonHelper.TryGetString(arguments, "customer_phone", out var customerPhone);
        ...
        if (string.IsNullOrWhiteSpace(search))
            return OperationJsonHelper.ErrorWithNextAction("customer_required", "Customer identifier is required.", "collect_customer_identifier", recoverable: true);

        var limit = OperationJsonHelper.TryGetInt(arguments, "limit", out var rawLimit)
            ? Math.Clamp(rawLimit, 1, 20)
            : 5;

        var normalizedSearch = InternalOperationParsing.NormalizePhone(search);
    ...
    private static bool MatchesCustomer(string? name, string? email, string? phone, string search, string normalizedSearch, bool hasSearchDigits)
    {
        return (!string.IsNullOrWhiteSpace(name) && name.Contains(search, StringComparison.OrdinalIgnoreCase))
            ...
            || (!string.IsNullOrWhiteSpace(phone) && (
                ...
                || (hasSearchDigits && InternalOperationParsing.NormalizePhone(phone).Contains(normalizedSearch))));
    }
}
