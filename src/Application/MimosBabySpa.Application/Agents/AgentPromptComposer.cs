...
public sealed class AgentPromptComposer : IPromptComposer
{
    ...
    internal static string BuildEagerCaptureBlock(AgentConfig config, AgentToolContext? session)
    {
        ...
        var lines = new List<string>
        {
            ...
            "- Guarda �nicamente datos expresados o confirmados por el cliente.",
            "- Mant�n objetivos internos y marcadores de estado fuera de facts de usuario."
        };
        ...
        AgentFlowStage? currentStage)
    {
        ...
        if (session is not null && currentStage.AdvanceWhenFacts.Count > 0)
        {
            ...
            {
                lines.Add("- criterio_de_avance: la etapa se completa cuando est�n presentes estos datos del flujo.");
                lines.Add($"- datos_para_completar_etapa: {string.Join(", ", DescribeFacts(config, missingFacts))}");
                lines.Add("- acci�n: usa estos datos como pr�ximos datos �tiles solo cuando la intenci�n actual los requiera.");
            }
        }
