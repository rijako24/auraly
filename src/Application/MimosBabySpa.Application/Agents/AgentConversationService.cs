...
public sealed class AgentConversationService : IAgentConversationService
{
        ...
        CancellationToken ct)
    {
        ...
        var currentStage = _flowStageDetector.DetectCurrentStage(activeFlow, session)
            ?? activeFlow.Stages.FirstOrDefault();
        ...
        var versions = session.Facts.Keys.ToDictionary(
            key => key,
            _ => 1L,
        ...
        CancellationToken ct)
    {
        ...
        if (turn.EscalateToHuman)
        {
            return await EscalateAndPersistAsync(
                config,
                session,
                userMessage,
                turn.Response?.Guidance ?? "deterministic_stage_escalation",
                ct);
        }
        ...
        var trace = turn.Trace.Select((entry, index) => new AgentTurnTraceEntry
        {
            ...
            Iteration = index,
            StageId = stage.Id,
            ToolName = entry.OperationId,
            ToolArgumentsJson = entry.ArgumentsJson,
            ToolResultJson = entry.Outcome is null ? null : JsonSerializer.Serialize(entry.Outcome)
        }).ToList();
        return AgentTurnResult.Ok(
            rendered.Text,
            requestCompleted: turn.RequestCompleted,
            tokens: promptTokens + completionTokens,
            toolCalls: operationCount,
        ...
        CancellationToken ct)
    {
        ...
        var response = new StageResponseDefinition
        {
            ...
            Guidance = outcome.Success
                ? "Comunica únicamente el resultado vigente de la acción solicitada desde el botón."
                : outcome.Error?.Message ?? "No fue posible procesar la acción del botón."
        };
    ...
    private async Task<AgentTurnResult> EscalateAndPersistAsync(
        ...
        CancellationToken ct)
    {
        if (!_operationRegistry.TryGet("escalation.request_human", out var escalationOperation))
            throw new InvalidOperationException("Human escalation operation is not registered.");

        using var escalationArguments = JsonDocument.Parse(JsonSerializer.Serialize(new
        {
            reason,
            last_user_message = userMessage
        }));
        var escalationOutcome = await escalationOperation.ExecuteAsync(
            escalationArguments.RootElement,
            new OperationContext
            {
                AgentId = config.AgentId,
                BusinessId = config.BusinessId,
            ...
            ct);
        if (!escalationOutcome.Success)
            _logger.LogError("Conv {ConvId}: human escalation operation failed with {Code}", session.ConversationId, escalationOutcome.Code);
