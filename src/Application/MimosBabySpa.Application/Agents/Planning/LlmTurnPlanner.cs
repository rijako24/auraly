...

    {
...

        {
...

            {
                ...
                "While shoppingContext.interaction.expected_reply is resolve_pending_cart_selection, never finalize the order while that selection remains unresolved. If the customer tries to finish without choosing or cancelling it, emit the configured cart mutation signal with an empty batch so the deterministic operation can present the pending candidates again. If the customer explicitly cancels the pending mutation, emit a cancel_pending command using the pending requested_product reference and a null quantity; the deterministic operation owns cancellation semantics.",
