...
public sealed class SetFactTool : IAgentTool
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!TryReadScalarValue(valueElement, out var rawValue))
            return ToolResultHelper.Error(ToolErrorCodes.InvalidValue, "Fact value must be a scalar value.", recoverable: true);
        ...
        if (!FactKeyNormalizer.TryNormalizeKey(canonicalKey, out var key))
            return ToolResultHelper.Error(ToolErrorCodes.InvalidKey, "Fact key must be a short snake_case identifier.", recoverable: true);
        ...
        if (schemaEntry is null && ctx.Config?.FactSchema.Count > 0)
        {
            ...
            return ToolResultHelper.Error(
                ...
                $"'{key}' no es una clave reconocida del esquema de este agente. Usa exactamente una de estas claves: {validKeys}.",
                recoverable: true);
        }
        ...
        if (IsBookingServiceFact(key, schemaEntry))
        {
            return ToolResultHelper.ErrorWithLlm(
                "wrong_tool_for_service_selection",
                "Service selection must be resolved with resolve_service_selection, not set_fact.",
                new
                {
                    next_action = "resolve_service_selection",
                    tool = "resolve_service_selection",
                    arguments = new { text = string.IsNullOrWhiteSpace(ctx.LatestUserMessage) ? value : ctx.LatestUserMessage }
                },
                recoverable: true);
        }
        ...
        if (key.Equals(ConversationFactKeys.AddOns, StringComparison.OrdinalIgnoreCase))
        {
            ...
            var validation = await _addOnCatalog.ValidateAsync(ctx.BusinessId, service, value, cancellationToken);
    ...
    private static string? ValidateType(string key, string value, string type)
    {
        ...
        {
            "number" when !decimal.TryParse(
                value,
                System.Globalization.NumberStyles.Any,
                System.Globalization.CultureInfo.InvariantCulture,
                out _) => ToolResultHelper.Error(ToolErrorCodes.InvalidType, $"'{key}' must be a number, got '{value}'.", recoverable: true),

            "date" when !System.DateOnly.TryParseExact(
                value,
                "yyyy-MM-dd",
                null,
                System.Globalization.DateTimeStyles.None,
                out _) => ToolResultHelper.Error(ToolErrorCodes.InvalidType, $"'{key}' must be a date in YYYY-MM-DD format, got '{value}'.", recoverable: true),
