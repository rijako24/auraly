...
public class IntegrationAdminService : IIntegrationAdminService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var secrets = new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase)
        {
            ["privateKey"] = UseNewOrExisting(request.PrivateKey, existingSecrets, "privateKey"),
            ["publicKey"] = UseNewOrExisting(request.PublicKey, existingSecrets, "publicKey"),
            ["eventsSecret"] = UseNewOrExisting(request.EventsSecret, existingSecrets, "eventsSecret"),
            ["integritySecret"] = UseNewOrExisting(request.IntegritySecret, existingSecrets, "integritySecret")
        };

        connection.Name = "Wompi";
        connection.AccountIdentifier = secrets["publicKey"];
        connection.SettingsJson = Serialize(new
        {
    ...
    private static WompiIntegrationDto MapWompi(IntegrationConnection? connection)
    {
        ...
        return new WompiIntegrationDto(
            connection?.IsEnabled ?? false,
            GetBool(settings, "useSandbox", true),
            ...
            Has(secrets, "privateKey"),
            Has(secrets, "publicKey"),
            Has(secrets, "eventsSecret"),
            Has(secrets, "integritySecret"),
            connection?.LastError,
            connection?.LastSyncAt);
    }
