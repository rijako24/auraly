...
using static Auraly.Platform.Application.Agents.OperationEffectNames;
using Auraly.Platform.Application.Agents.Operations.Support;

using ReservationEntity = Auraly.Platform.Domain.Entities.Reservation;
namespace Auraly.Platform.Application.Agents.Operations.Reservation;

public sealed class ManageReservationOperation : IAgentOperation
{
private readonly IReservationService _reservations;
    ...
    public ManageReservationOperation(
    ...
    public OperationDescriptor Descriptor { get; } = new(
        "reservation.manage", InputSchema,
        ["reservation.managed", "missing_prerequisites", "reservation_not_found", "ambiguous_reservation", "slot_unavailable", "no_employee_available", "method_failed"],
        ["reservation.manage"], [], []);
        ...
        CancellationToken cancellationToken = default)
    {
        var session = context.Session
            ?? throw new InvalidOperationException("reservation.manage requires a conversation session.");
            var raw = await ExecuteCoreAsync(input, session, cancellationToken);
            ...
            if (!root.TryGetProperty("ok", out var ok) || ok.ValueKind != JsonValueKind.True)
            {
                ...
                var message = ReadString(error, "message") ?? "Reservation management failed.";
                var recoverable = error.ValueKind == JsonValueKind.Object
                    && error.TryGetProperty("recoverable", out var recoverableValue)
                    && recoverableValue.ValueKind == JsonValueKind.True;
                return OperationOutcome.Fail(code, message, recoverable, context: root.Clone());
            }

            var data = root.TryGetProperty("data", out var configuredData) ? configuredData.Clone() : EmptyObject();
            var effects = ReadStrings(root, "effects").Select<string, OperationEffect?>(effect => effect switch
            {
                RequestCompleted => new CompleteRequestOperationEffect(),
                EscalatedToHuman => new EscalateHumanOperationEffect(),
                _ => null
            }).Where(effect => effect is not null).Cast<OperationEffect>().ToList();
            return OperationOutcome.Ok("reservation.managed", data, effects: effects, events: ReadStrings(root, "events"));
    }
    ...
    private async Task<string> ExecuteCoreAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        OperationJsonHelper.TryGetString(arguments, "action", out var action);
        ...
        return action switch
        {
            ...
            _ => OperationJsonHelper.ErrorWithNextAction(
                "invalid_action",
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return OperationJsonHelper.ErrorWithLlm("missing_supported_reservation_change", "Automatic reservation changes require at least one configured automatic change field.", new
                {
        ...
        var request = await ReservationChangeSupport.BuildRequestAsync(
            arguments,
            ctx,
        ...
        if (!result.Success)
        {
            ...
            if (string.Equals(result.ErrorCode, "slot_unavailable", StringComparison.OrdinalIgnoreCase))
            {
                ...
                return OperationJsonHelper.ErrorWithLlm(result.ErrorCode!, result.ErrorMessage!, new
                    {
            ...
            return ReservationChangeSupport.BuildErrorResult(result);
        }
        ...
        return OperationJsonHelper.OkWithLlm(
            ReservationChangeSupport.ToPayload(result),
            ReservationChangeSupport.ToLlmPayload(result));
    }
        ...
        string? forcedReason = null)
    {
        ...
        return OperationJsonHelper.OkWithLlm(new
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (OperationJsonHelper.TryGetString(arguments, "payment_transaction_id", out var paymentIdStr)
        ...
        {
            return OperationJsonHelper.Error(
        ...
        if (string.IsNullOrWhiteSpace(dateStr) || string.IsNullOrWhiteSpace(timeStr))
            return OperationJsonHelper.MissingPrerequisites(["date", "time"]);

        if (!AgentDateRules.TryParseDate(dateStr!, out var date))
            return OperationJsonHelper.Error("invalid_date", $"'{dateStr}' is not a valid date.");
        if (!TimeOnly.TryParse(timeStr, out var time))
            return OperationJsonHelper.Error("invalid_time", $"'{timeStr}' is not a valid time.");
        ...
        if (service is null)
            return OperationJsonHelper.Error("service_not_found", "Snapshot service no longer exists.");
        ...
        if (!availability.IsAvailable)
        {
            ...
            return OperationJsonHelper.ErrorWithLlm("slot_unavailable", availability.ResponseMessage ?? "The selected time is not available.", new
                {
                    next_action = "offer_alternative_slots",
                    requested_date = date,
        ...
        {
            return OperationJsonHelper.Error("no_employee_available", ex.Message);
        }
        ...
        return OperationJsonHelper.OkWithEvents(new
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        OperationJsonHelper.TryGetString(arguments, "notes", out var notes);
    ...
    private async Task<string> ConfirmAttendanceAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (!OperationJsonHelper.TryGetBool(arguments, "customer_confirmed", out var confirmed) || !confirmed)
        {
            return OperationJsonHelper.ErrorWithLlm("confirmation_required", "Customer confirmation is required before registering attendance.", new { next_action = "collect_confirmation", confirmation_type = "attendance" }, recoverable: true);
        }
        ...
        CancellationToken cancellationToken)
    {
        if (!OperationJsonHelper.TryGetBool(arguments, "customer_confirmed", out var confirmed) || !confirmed)
        {
            return OperationJsonHelper.ErrorWithLlm("confirmation_required", "Customer confirmation is required before cancelling or suspending a reservation.", new { next_action = "collect_confirmation", confirmation_type = "cancellation" }, recoverable: true);
        }
        ...
        return success
            ? OperationJsonHelper.Ok(new { reservation_id = resolved.Reservation.ReservationId, status = "suspended" })
            : OperationJsonHelper.ErrorWithNextAction("suspend_failed", "The reservation could not be suspended.", "select_reservation", recoverable: true);
    }
    ...
    private async Task<ManageReservationResolveResult> ResolveReservationAsync(
        ...
        CancellationToken cancellationToken)
    {
        OperationJsonHelper.TryGetString(arguments, "job_id", out var jobIdStr);
        OperationJsonHelper.TryGetString(arguments, "reservation_id", out var reservationIdStr);
        ...
        return resolved.Success
            ...
            : ManageReservationResolveResult.Fail(resolved.ErrorJson ?? OperationJsonHelper.Error("reservation_not_found", "No reservation was found."));
    }

    private static string? TryParseJobIdFromPayload(AgentConversationContext ctx)
    {
    ...
    private sealed class ManageReservationResolveResult
    {
        ...
        public ReservationEntity? Reservation { get; init; }
        public ScheduledAutomationJob? SourceJob { get; init; }
        public string? ErrorJson { get; init; }

        public static ManageReservationResolveResult Ok(ReservationEntity reservation, ScheduledAutomationJob? sourceJob = null) =>
    ...
    private static bool HasString(JsonElement arguments, string property) =>
        OperationJsonHelper.TryGetString(arguments, property, out var value)
        && !string.IsNullOrWhiteSpace(value);

    private static string? Coalesce(JsonElement args, string property, string? fallback)
    {
        if (OperationJsonHelper.TryGetString(args, property, out var fromArgs))
            return fromArgs;
