...
export function TenantProvisioningWizard({ publicMode = false }: { publicMode?: boolean }) {
  ...
  const [publicCompletion, setPublicCompletion] = useState<{ tenantId: string | null } | null>(null);
  ...
  const paidProvisioningComplete = (status: { tenantId: string | null }) => {
  ...
  if (publicCompletion) return <div className="mx-auto max-w-2xl rounded-3xl border bg-card p-10 text-center shadow-xl"><span className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-emerald-100 text-emerald-700"><Check className="h-8 w-8" /></span><h1 className="mt-6 text-3xl font-semibold">Tu empresa quedó preparada</h1><p className="mx-auto mt-3 max-w-lg text-muted-foreground">Confirmamos el pago y enviamos la invitación a <strong>{form.invitationEmail}</strong>. Desde ese correo podrás crear el usuario administrador.</p><Button asChild className="mt-7"><Link href="/login">Ir a iniciar sesión</Link></Button></div>;
