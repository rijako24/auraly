...
export default function InventoryPage(){
 ...
 const [filtersExpanded,setFiltersExpanded]=useState(true); const [fromDate,setFromDate]=useState(""); const [toDate,setToDate]=useState(""); const [operationStatus,setOperationStatus]=useState("all");
 ...
 const operations=useQuery({queryKey:["inventory-operations",businessId,warehouseId,deferredSearch,operationType,fromDate,toDate,operationStatus,reasonCode,destinationWarehouseId,supplierId,purchaseEvidenceType,pages.operations,pageSize],queryFn:()=>inventoryApi.operations({warehouseId:warehouseId==="all"?undefined:warehouseId,search:deferredSearch||undefined,documentType:operationType==="all"?undefined:operationType,from:dayBoundary(fromDate),to:dayBoundary(toDate,true),status:operationType!=="all"&&operationStatus!=="all"?operationStatus:undefined,reasonCode:operationType!=="all"&&operationType!=="GoodsReceipt"&&reasonCode!=="all"?reasonCode:undefined,destinationWarehouseId:operationType==="WarehouseTransfer"&&destinationWarehouseId!=="all"?destinationWarehouseId:undefined,supplierId:operationType==="GoodsReceipt"&&supplierId!=="all"?supplierId:undefined,purchaseEvidenceType:operationType==="GoodsReceipt"&&purchaseEvidenceType!=="all"?purchaseEvidenceType:undefined,page:pages.operations,pageSize}),enabled:!!businessId,refetchInterval:query=>query.state.data?.items.some(item=>item.status==="Accepted")?2_000:false});
 ...
 const changeOperationType=(value:string)=>{setOperationType(value);setReasonCode("all");setDestinationWarehouseId("all");setSupplierId("all");setPurchaseEvidenceType("all");setPages(current=>({...current,operations:1}))};
 ...
 return <div className="space-y-5 p-6"><header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-medium text-emerald-600">Operación y trazabilidad</p><h1 className="text-3xl font-bold tracking-tight">Inventario</h1><p className="text-muted-foreground">Existencias, kárdex y operaciones procesadas por el motor ordenado de Auraly.</p></div><div className="flex flex-wrap gap-2">{permissions.has("inventory.physical-counts.manage")&&<Button variant="outline" onClick={()=>setReconcilingInventory(true)}>Conciliación de inventario</Button>}<Button onClick={()=>{setEditingPhysicalCountDraft(undefined);setCreatingOperation(true)}}><Plus className="mr-2 h-4 w-4"/>Nueva operación</Button></div></header>
 ...
 <TabsContent value="operations" className="space-y-4">
  ...
  <Card><CardContent className="pt-6"><button type="button" className="flex w-full items-center justify-between gap-3 text-left" onClick={()=>setFiltersExpanded(value=>!value)} aria-expanded={filtersExpanded}><span className="flex items-center gap-2 font-medium"><SlidersHorizontal className="h-4 w-4"/>Filtros de {operationType==="all"?"todas las operaciones":documentLabels[operationType]?.toLocaleLowerCase("es-CO")??operationType}</span>{filtersExpanded?<ChevronUp className="h-4 w-4"/>:<ChevronDown className="h-4 w-4"/>}</button>
   {filtersExpanded&&<div className="mt-4 grid gap-3 border-t pt-4 sm:grid-cols-2 lg:grid-cols-4">
    <label className="space-y-1.5 text-sm"><span className="text-muted-foreground">Desde</span><Input type="date" value={fromDate} max={toDate||undefined} onChange={event=>changeOperationFilter(setFromDate,event.target.value)}/></label>
    <label className="space-y-1.5 text-sm"><span className="text-muted-foreground">Hasta</span><Input type="date" value={toDate} min={fromDate||undefined} onChange={event=>changeOperationFilter(setToDate,event.target.value)}/></label>
    {operationType!=="all"&&<FilterSelect label="Estado" value={operationStatus} onChange={value=>changeOperationFilter(setOperationStatus,value)} options={[["all","Todos los estados"],["Accepted","En proceso"],["Processed","Procesado"],["Confirmed","Confirmado"],["Failed","Con error"]]}/>}
    {operationType!=="all"&&operationType!=="GoodsReceipt"&&<FilterSelect label="Motivo" value={reasonCode} onChange={value=>changeOperationFilter(setReasonCode,value)} options={[["all","Todos los motivos"],...(reasonOptions.data??[]).map(item=>[item.code,item.name])]}/>}
    {operationType==="WarehouseTransfer"&&<FilterSelect label="Bodega destino" value={destinationWarehouseId} onChange={value=>changeOperationFilter(setDestinationWarehouseId,value)} options={[["all","Todas las bodegas destino"],...warehouses.filter(item=>item.id!==warehouseId).map(item=>[item.id,item.name])]}/>}
    {operationType==="GoodsReceipt"&&<FilterSelect label="Proveedor" value={supplierId} onChange={value=>changeOperationFilter(setSupplierId,value)} options={[["all","Todos los proveedores"],...(goodsReceiptOptions.data?.suppliers??[]).map(item=>[item.supplierId,`${item.identification} · ${item.name}`])]}/>}
    {operationType==="GoodsReceipt"&&<FilterSelect label="Tipo de documento" value={purchaseEvidenceType} onChange={value=>changeOperationFilter(setPurchaseEvidenceType,value)} options={[["all","Todos los documentos"],...(goodsReceiptOptions.data?.purchaseEvidenceTypes??[]).map(item=>[item.code,item.label])]}/>}
   </div>}
  </CardContent></Card>
  {operationType==="StockCount"?<Tabs value={inventoryView} onValueChange={setInventoryView} className="space-y-4"><TabsList className="grid h-auto w-full grid-cols-2 rounded-2xl p-1"><TabsTrigger value="documents" className="rounded-xl py-2.5">Documentos de inventario</TabsTrigger><TabsTrigger value="drafts" className="rounded-xl py-2.5">Borradores</TabsTrigger></TabsList><TabsContent value="documents" className="space-y-4">{standardOperationHistory}</TabsContent><TabsContent value="drafts"><InventoryPhysicalCountWorkspace businessId={businessId} warehouses={warehouses} warehouseId={warehouseId==="all"?undefined:warehouseId} search={deferredSearch||undefined} permissions={permissions} onEditDraft={value=>{setEditingPhysicalCountDraft(value);setCreatingOperation(true)}}/></TabsContent></Tabs>:operationType==="ProductConversion"?<><SimpleTable headers={["Fecha","Documento","Bodega","Entrada equivalente","Salida equivalente","Merma","Tolerancia","Estado"]} rows={(operations.data?.items??[]).map(x=>[formatDateTime(x.occurredAt),x.documentNumber??"Borrador",x.warehouseName,x.conversionInputEquivalent==null?"—":fmt(x.conversionInputEquivalent),x.conversionOutputEquivalent==null?"—":fmt(x.conversionOutputEquivalent),x.conversionLossPercent==null?"—":`${fmt(x.conversionLossQuantity??0)} · ${fmt(x.conversionLossPercent)} %`,x.conversionMaximumLossPercent==null?"—":`${fmt(x.conversionMaximumLossPercent)} %`,humanLabel(x.status,statusLabels)])} loading={operations.isLoading} onRowClick={index=>void openOperationDetail(operations.data?.items[index])}/><Pager page={pages.operations} pageSize={pageSize} total={operations.data?.totalPages??0} totalItems={operations.data?.totalCount??0} setPage={page=>setPages(current=>({...current,operations:page}))} setPageSize={setPageSize}/></>:standardOperationHistory}
 </TabsContent></Tabs>
 {creatingOperation&&<Dialog open onOpenChange={v=>{if(!v){setCreatingOperation(false);setEditingPhysicalCountDraft(undefined)}}}><DialogContent className="flex max-h-[94dvh] max-w-6xl flex-col overflow-y-auto"><DialogHeader><DialogTitle>{editingPhysicalCountDraft?"Editar borrador":"Nueva operación"}</DialogTitle><DialogDescription>{editingPhysicalCountDraft?"Continúa el conteo o reconteo exactamente donde quedó guardado.":"Comienza con un conteo físico o cambia de operación sin cerrar esta ventana."}</DialogDescription></DialogHeader><InventoryOperationWorkspace businessId={businessId} warehouses={warehouses} permissions={permissions} initialKind={editingPhysicalCountDraft?"count":defaultInventoryOperationKind} physicalCountDraft={editingPhysicalCountDraft} onCancel={()=>{setCreatingOperation(false);setEditingPhysicalCountDraft(undefined)}} onCompleted={destination=>{setCreatingOperation(false);setEditingPhysicalCountDraft(undefined);setActiveTab("operations");setOperationType("StockCount");setInventoryView(destination??"documents");void Promise.all([queryClient.invalidateQueries({queryKey:["inventory-physical-counts"]}),queryClient.invalidateQueries({queryKey:["inventory-physical-count-drafts"]}),queryClient.invalidateQueries({queryKey:["inventory-operations"]})])}}/></DialogContent></Dialog>}
 <InventoryReconciliationDialog open={reconcilingInventory} businessId={businessId} warehouseId={warehouseId==="all"?undefined:warehouseId} onClose={()=>setReconcilingInventory(false)}/>
