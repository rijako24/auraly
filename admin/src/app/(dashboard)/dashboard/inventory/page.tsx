...
import { InventoryPhysicalCountWorkspace, InventoryReconciliationDialog } from "@/components/inventory/inventory-physical-count-workspace";
...
export default function InventoryPage(){
 ...
 return <div className="space-y-5 p-6"><header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-medium text-emerald-600">Operación y trazabilidad</p><h1 className="text-3xl font-bold tracking-tight">Inventario</h1><p className="text-muted-foreground">Existencias, kárdex y operaciones procesadas por el motor ordenado de Auraly.</p></div><div className="flex flex-wrap gap-2">{permissions.has("inventory.physical-counts.manage")&&<Button variant="outline" onClick={()=>setReconcilingInventory(true)}>Conciliación de inventario</Button>}<Button onClick={()=>setCreatingOperation(true)}><Plus className="mr-2 h-4 w-4"/>Nueva operación</Button></div></header>
