...
export default function InventoryPage(){
 ...
 const balances=useQuery({queryKey:["inventory-balances",businessId,warehouseId,deferredSearch,pages.balances],queryFn:()=>inventoryApi.balances({warehouseId:warehouseId==="all"?undefined:warehouseId,search:deferredSearch||undefined,page:pages.balances,pageSize:20}),enabled:!!businessId});
 const movements=useQuery({queryKey:["inventory-movements",businessId,warehouseId,deferredSearch,pages.movements],queryFn:()=>inventoryApi.movements({warehouseId:warehouseId==="all"?undefined:warehouseId,search:deferredSearch||undefined,page:pages.movements,pageSize:20}),enabled:!!businessId});
 const operations=useQuery({queryKey:["inventory-operations",businessId,warehouseId,deferredSearch,pages.operations],queryFn:()=>inventoryApi.operations({warehouseId:warehouseId==="all"?undefined:warehouseId,search:deferredSearch||undefined,page:pages.operations,pageSize:20}),enabled:!!businessId});
...
function countDifference(line:InventoryOperationDetail["lines"][number]){return (line.quantity??0)-(line.systemQuantityAtBase??0);}
...
function Pager({page,total,setPage}:{page:number;total:number;setPage:(n:number)=>void}){if(total===0)return null;return <div className="flex items-center justify-between rounded-xl border bg-card px-3 py-2"><span className="text-sm text-muted-foreground">Página {page} de {total}</span><div className="flex gap-2"><Button size="sm" variant="outline" disabled={page<=1} onClick={()=>setPage(page-1)}>Anterior</Button><Button size="sm" variant="outline" disabled={page>=total} onClick={()=>setPage(page+1)}>Siguiente</Button></div></div>}
