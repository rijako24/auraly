...
export default function InventoryPage(){
 ...
 const [operationChooser,setOperationChooser]=useState(false); const [createKind,setCreateKind]=useState<"adjustment"|"transfer"|"conversion"|"damage">(); const [physicalCreateSignal,setPhysicalCreateSignal]=useState(0);
 ...
 const operations=useQuery({queryKey:["inventory-operations",businessId,warehouseId,deferredSearch,operationType,pages.operations,pageSize],queryFn:()=>inventoryApi.operations({warehouseId:warehouseId==="all"?undefined:warehouseId,search:deferredSearch||undefined,documentType:operationType==="all"||operationType==="physical"?undefined:operationType,page:pages.operations,pageSize}),enabled:!!businessId&&operationType!=="physical",refetchInterval:query=>query.state.data?.items.some(item=>item.status==="Accepted")?2_000:false});
 ...
 const changeTab=(value:string)=>{
  ...
  else if(value==="operations"&&operationType!=="physical")void operations.refetch();
 };
 if(!businessId)return <Empty text="Selecciona una sede para consultar su inventario."/>;
 return <div className="space-y-5 p-6"><header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-medium text-emerald-600">Operación y trazabilidad</p><h1 className="text-3xl font-bold tracking-tight">Inventario</h1><p className="text-muted-foreground">Existencias, kárdex y operaciones procesadas por el motor ordenado de Auraly.</p></div><Button onClick={()=>setOperationChooser(true)}><Plus className="mr-2 h-4 w-4"/>Nueva operación</Button></header>
...
function NewOperationDialog({open,onClose,onSelect}:{open:boolean;onClose:()=>void;onSelect:(kind:"physical"|"adjustment"|"transfer"|"conversion"|"damage")=>void}){const options:Array<["physical"|"adjustment"|"transfer"|"conversion"|"damage",string,string]>=[["physical","Inventario físico","Coordina listas, preconteos y conteos antes de aplicar diferencias"],["adjustment","Ajuste","Registra una entrada o salida justificada"],["transfer","Traslado","Mueve productos entre bodegas"],["conversion","Conversión","Transforma presentaciones conservando su valor"],["damage","Avería","Retira mercancía dañada o no vendible"]];return <Dialog open={open} onOpenChange={v=>!v&&onClose()}><DialogContent><DialogHeader><DialogTitle>Nueva operación</DialogTitle><DialogDescription>Selecciona el proceso que deseas registrar.</DialogDescription></DialogHeader><div className="grid gap-3 sm:grid-cols-2">{options.map(([id,title,description])=><button key={id} className="rounded-2xl border p-4 text-left transition hover:border-emerald-400 hover:bg-emerald-50/40" onClick={()=>onSelect(id)}><strong>{title}</strong><span className="mt-1 block text-sm text-muted-foreground">{description}</span></button>)}</div></DialogContent></Dialog>}
