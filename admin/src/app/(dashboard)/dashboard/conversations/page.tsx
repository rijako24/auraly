...
export default function ConversationsPage() {
  ...
  return (
    <div className={cn("flex h-[calc(100vh-3.5rem)] w-full min-w-0 overflow-hidden rounded-lg border border-border bg-card", "max-lg:-mx-4 max-lg:h-[calc(100vh-3.5rem)] max-lg:rounded-none max-lg:border-x-0")}>
      <div className={cn("flex w-[min(100%,22rem)] sm:w-[min(100%,24rem)] lg:w-[26rem] flex-shrink-0 flex-col overflow-hidden", isMobile && "absolute inset-0 z-10", isMobile && !showList && "hidden")}>
      ...
      <div className={cn("flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-background", isMobile && "absolute inset-0 z-10", isMobile && !showChat && "hidden")}>
          ...
          <>
            <div className="flex flex-shrink-0 items-center gap-3 border-b border-border bg-muted/30 px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 flex flex-wrap items-center gap-1.5 break-all text-xs text-muted-foreground">
                  <Phone className="h-3.5 w-3.5 shrink-0" />
                  <span>{selectedConversation.userNumber}</span>
                </p>
              </div>
            </div>
