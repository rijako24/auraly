...
function ReconciliationDialog({ closure, canReconcile, onClose }: { closure: WorkSessionClosure; canReconcile: boolean; onClose: () => void }) {
  ...
  const cashGroups = cashClosureCashGroups([
    ...methodItems("Cash"),
    ...methodItems("Credit"),
  ]);
    ...
    {countable.map(item => {
      ...
      return <section key={item.paymentMethodCode} className="overflow-hidden rounded-2xl border bg-white shadow-sm">
        ...
        {cash && <div className="space-y-3 p-4">
          <p className="text-sm text-muted-foreground">Verifica individualmente entradas, salidas y ventas a cartera. Las ventas de contado, comprobantes y devoluciones no requieren confirmación.</p>
          ...
          {cashGroups.length === 0 && <p className="rounded-xl border border-dashed p-4 text-sm text-muted-foreground">No hay entradas, salidas ni ventas a cartera pendientes de verificar.</p>}
