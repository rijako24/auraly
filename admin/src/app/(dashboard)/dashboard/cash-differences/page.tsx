...
export default function CashClosuresPage() {
  ...
  return <div className="space-y-5">
    ...
    {canReadOwnSession && <ActiveWorkSessionPanel lastClosedAt={rows[0]?.closedAt ?? null} />}
    <div className="grid gap-4 sm:grid-cols-3"><Summary title="Cierres en esta página" value={closures.isError ? "—" : String(rows.length)} icon={<Scale className="h-5 w-5" />} /><Summary title="Pendientes" value={closures.isError ? "—" : String(pending)} icon={<Clock3 className="h-5 w-5 text-amber-600" />} /><Summary title="Conciliados" value={closures.isError ? "—" : String(rows.length - pending)} icon={<CheckCircle2 className="h-5 w-5 text-emerald-600" />} /></div>
...
function ReconciliationDialog({ closure, canReconcile, onClose }: { closure: WorkSessionClosure; canReconcile: boolean; onClose: () => void }) {
  ...
  const methodVerifiedAmount = (code: string) => { const items = methodItems(code); return code !== "Cash" && items.length ? items.filter(item => verificationStatus[item.verificationKey] === "Verified").reduce((sum, item) => sum + item.amount, 0) : Number(verified[code] ?? 0); };
  const methodIsConfirmed = (code: string) => { const items = methodItems(code); return code !== "Cash" && items.length ? items.every(item => verificationStatus[item.verificationKey] !== undefined) : confirmed[code]; };
  ...
  const mutation = useMutation({ mutationFn: () => workSessionDifferencesApi.reconcile(closure.workSessionClosureId, { lines: countable.map(item => ({ paymentMethodCode: item.paymentMethodCode, verifiedAmount: methodVerifiedAmount(item.paymentMethodCode), isConfirmed: methodIsConfirmed(item.paymentMethodCode), reasonCode: adjustedDifferences[item.paymentMethodCode] === 0 ? null : reasonCode })), paymentVerifications: verificationItems.filter(item => item.paymentMethodCode !== "Cash").map(item => ({ verificationKey: item.verificationKey, status: verificationStatus[item.verificationKey]! })), reclassifications, note: note.trim() || null }), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["work-session-closures"] }); onClose(); } });
  ...
  return <Dialog open onOpenChange={open => !open && onClose()}><DialogContent className="flex max-h-[94vh] max-w-4xl flex-col overflow-hidden p-0"><DialogHeader className="shrink-0 border-b bg-gradient-to-r from-slate-950 to-teal-900 p-6 text-left text-white"><DialogTitle className="text-xl text-white">{editable ? "Conciliar cierre" : "Detalle del cierre"}</DialogTitle><DialogDescription className="text-slate-200">{closure.userName} · {closure.businessName} · {new Date(closure.closedAt).toLocaleString("es-CO")}</DialogDescription></DialogHeader><div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 pb-2">
    ...
    {countable.map(item => { const items = methodItems(item.paymentMethodCode); const cash = item.paymentMethodCode === "Cash"; const movementNet = items.reduce((sum, value) => sum + value.amount, 0); const difference = differences[item.paymentMethodCode]; return <section key={item.paymentMethodCode} className="overflow-hidden rounded-2xl border bg-white shadow-sm"><div className="flex flex-col gap-3 border-b bg-slate-50 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className="rounded-xl bg-teal-100 p-2 text-teal-800"><PaymentIcon code={item.paymentMethodCode} /></div><div><strong className="text-base">{workSessionPaymentMethodName(item.paymentMethodCode)}</strong><p className="text-sm text-muted-foreground">Esperado {money.format(item.netAmount)} · reportado al cerrar {money.format(item.countedAmount ?? 0)}</p></div></div><div className={`text-left sm:text-right ${difference < 0 ? "text-red-700" : difference > 0 ? "text-emerald-700" : "text-slate-700"}`}><span className="block text-xs font-semibold uppercase tracking-wide">Resultado</span><strong>{result(difference)}</strong></div></div>
      {items.length > 0 && <div className="px-4 pt-4"><button type="button" className="flex w-full items-center justify-between gap-3 rounded-xl border bg-white p-3 text-left transition hover:bg-slate-50" onClick={() => setExpandedMethods(current => ({ ...current, [item.paymentMethodCode]: !current[item.paymentMethodCode] }))}><span><strong>{items.length} {cash ? "movimiento" : "comprobante"}{items.length === 1 ? "" : "s"}</strong><small className="mt-1 block text-muted-foreground">{cash ? `Ventas, devoluciones, entradas y salidas · neto ${money.format(movementNet)}` : `${items.filter(value => verificationStatus[value.verificationKey]).length} revisados · ${money.format(methodVerifiedAmount(item.paymentMethodCode))} confirmados`}</small></span><span className="inline-flex items-center gap-2 text-sm font-semibold text-teal-800">{expandedMethods[item.paymentMethodCode] ? "Ocultar detalle" : cash ? "Ver trazabilidad" : "Ver y conciliar detalle"}{expandedMethods[item.paymentMethodCode] ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}</span></button>{expandedMethods[item.paymentMethodCode] && <div className="mt-3"><p className="mb-3 text-sm text-muted-foreground">{cash ? "El efectivo esperado suma ventas y entradas, y resta devoluciones y salidas. Este detalle es informativo; el cuadre se hace contra el efectivo físico contado." : "Confirma cada comprobante. Si no está físicamente, márcalo como no encontrado."}</p><div className="space-y-2">{items.map(value => <VerificationRow key={value.verificationKey} item={value} status={verificationStatus[value.verificationKey]} disabled={!editable} informational={cash} onChange={status => setVerificationStatus(current => ({ ...current, [value.verificationKey]: status }))} />)}</div><div className="mt-3 flex justify-end text-sm"><span className={`rounded-xl px-3 py-2 ${cash && movementNet !== item.netAmount ? "bg-red-50 text-red-900" : "bg-teal-50 text-teal-900"}`}>{cash ? "Neto trazado" : "Total confirmado"}: <strong>{money.format(cash ? movementNet : methodVerifiedAmount(item.paymentMethodCode))}</strong>{cash && movementNet !== item.netAmount ? ` · no coincide con el esperado ${money.format(item.netAmount)}` : ""}</span></div></div>}</div>}
      {(cash || items.length === 0) && <div className="grid gap-4 p-4 sm:grid-cols-[1fr_190px_auto] sm:items-end"><p className="text-sm text-muted-foreground">{cash ? "Cuenta el efectivo físico e ingresa el total encontrado. La trazabilidad superior explica de dónde sale el esperado." : "No hay comprobantes individuales asociados; verifica el total."}</p><div className="space-y-1.5"><Label>Total verificado</Label><Input inputMode="numeric" disabled={!editable} value={formatWorkSessionCountInput(verified[item.paymentMethodCode] ?? "")} onChange={event => setVerified(current => ({ ...current, [item.paymentMethodCode]: normalizeWorkSessionCountInput(event.target.value) }))} /></div><label className="flex h-10 items-center gap-2 rounded-xl border px-3"><Checkbox disabled={!editable} checked={confirmed[item.paymentMethodCode]} onCheckedChange={value => setConfirmed(current => ({ ...current, [item.paymentMethodCode]: value === true }))} />Revisado</label></div>}
    </section>; })}
...
function VerificationRow({ item, status, disabled, informational = false, onChange }: { item: ClosurePaymentVerification; status?: VerificationStatus; disabled: boolean; informational?: boolean; onChange: (status: VerificationStatus) => void }) { const reference = item.approvalNumber || item.reference || "Sin referencia"; return <div className={`grid gap-3 rounded-xl border p-3 ${informational ? "sm:grid-cols-[1fr_auto]" : "sm:grid-cols-[1fr_auto_auto]"} sm:items-center`}><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><ReceiptText className="h-4 w-4 text-slate-500" /><strong>{verificationMovementName(item)} {item.documentNumber}</strong>{item.cardFranchiseCode && <Badge variant="secondary">{item.cardFranchiseCode}</Badge>}</div><p className="mt-1 truncate text-xs text-muted-foreground">Referencia: {reference} · {new Date(item.occurredAt).toLocaleString("es-CO")}</p></div><strong className={item.amount < 0 ? "text-red-700" : "text-slate-950"}>{money.format(item.amount)}</strong>{!informational && <div className="flex gap-2"><Button type="button" size="sm" variant={status === "Verified" ? "default" : "outline"} disabled={disabled} onClick={() => onChange("Verified")}><Check className="mr-1 h-4 w-4" />Verificado</Button><Button type="button" size="sm" variant={status === "Missing" ? "destructive" : "outline"} disabled={disabled} onClick={() => onChange("Missing")}><X className="mr-1 h-4 w-4" />No encontrado</Button></div>}</div>; }
