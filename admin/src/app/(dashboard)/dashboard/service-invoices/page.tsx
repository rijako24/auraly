  ...
  async function issue() {
    ...
    try {
      const result = await serviceInvoicesApi.issue(
        businessId, customer.customerId, customer.partySiteId,
        lines.map((line) => ({
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_390px]">
      <div className="space-y-5">
        <Card className="rounded-3xl"><CardHeader><CardTitle>1. Cliente</CardTitle></CardHeader><CardContent className="space-y-3">
          {customer ? <div className="flex items-center justify-between rounded-2xl border border-teal-200 bg-teal-50 p-4"><div><strong>{customer.displayName} · {customer.partySiteName}</strong><p className="text-sm text-muted-foreground">{customer.partySiteAddress} · {customer.identification}{customer.email ? ` · ${customer.email}` : " · Sin correo; no se programará envío"}</p></div><Button variant="ghost" onClick={() => setCustomer(undefined)}>Cambiar</Button></div> : <><div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground"/><Input className="pl-9" value={customerQuery} onChange={(event) => setCustomerQuery(event.target.value)} placeholder="Buscar cliente o sede"/></div><div className="grid max-h-52 gap-2 overflow-y-auto">{customers.map((value) => <button key={value.partySiteId} type="button" onClick={() => setCustomer(value)} className="rounded-2xl border p-3 text-left transition hover:border-teal-400 hover:bg-teal-50"><strong className="block">{value.displayName} · {value.partySiteName}</strong><small className="text-muted-foreground">{value.partySiteAddress} · {value.identification}{value.email ? ` · ${value.email}` : ""}</small></button>)}</div></>}
        </CardContent></Card>
