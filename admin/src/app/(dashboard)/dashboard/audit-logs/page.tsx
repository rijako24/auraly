...
export default function AuditLogsPage() {
  ...
  const columns: ColumnDef<AuditLog>[] = useMemo(() => [
    ...
    { accessorKey: "action", header: "AcciÃ³n", cell: ({ row }) => <span className="font-medium">{ACTION_LABELS[row.original.action] ?? row.original.action}</span> },
    { accessorKey: "entityType", header: "Entidad" },
    { accessorKey: "entityId", header: "ID Entidad", cell: ({ row }) => row.original.entityId ?? "â€”" },
    { accessorKey: "ipAddress", header: "IP", cell: ({ row }) => row.original.ipAddress ?? "â€”" },
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-semibold tracking-tight">Registro de AuditorÃ­a</h1><p className="text-muted-foreground">Historial de acciones en el sistema</p></div>
          ...
          {selectedLog && (
            <div className="mt-6 space-y-4">
              <div className="grid gap-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">AcciÃ³n</span><span>{ACTION_LABELS[selectedLog.action] ?? selectedLog.action}</span></div>
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">IP</span><span>{selectedLog.ipAddress ?? "â€”"}</span></div>
