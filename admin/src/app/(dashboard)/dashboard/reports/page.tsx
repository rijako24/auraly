...
export default function TodayReportPage(){
  const today=useQuery({queryKey:["sales-report-today"],queryFn:salesReportingApi.today,staleTime:0,refetchOnMount:"always"});
...
function useBreakdown(filter:SalesReportFilter|undefined,dimension:SalesDimension,limit:number){return useQuery({queryKey:["sales-report-today-breakdown",filter,dimension],queryFn:()=>salesReportingApi.breakdown(filter!,dimension,limit),enabled:Boolean(filter),staleTime:0,refetchOnMount:"always"})}
