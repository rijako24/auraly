...
function Header({ onAdd, disabled = false }: { onAdd: () => void; disabled?: boolean }) { return <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><h1 className="text-2xl font-semibold tracking-tight">Canales</h1><p className="text-muted-foreground">Conecta los puntos de contacto y asigna el agente que atendera cada uno.</p></div><Button onClick={onAdd} disabled={disabled}><Plus className="mr-2 h-4 w-4" />Conectar canal</Button></div>; }
