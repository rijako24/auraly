...
export default function ChannelsPage() {
  ...
  const canManage = useAuthStore((state) => state.user?.permissions.includes("agents.update") ?? false);
  ...
  return <div className="space-y-6">
    <Header onAdd={openCreate} disabled={!canManage} />
    ...
    {(channels.data ?? []).length === 0 ? <EmptyState onAdd={openCreate} disabled={!canManage} /> : <div className="grid gap-4 lg:grid-cols-2">{channels.data?.map((channel) => <ChannelCard key={channel.businessWhatsAppNumberId} channel={channel} status={statuses[channel.businessWhatsAppNumberId]} checking={validateChannel.isPending && validateChannel.variables === channel.businessWhatsAppNumberId} canManage={canManage} onCheck={() => checkConnection(channel.businessWhatsAppNumberId)} onEdit={() => openEdit(channel)} onRemove={() => remove(channel)} />)}</div>}
