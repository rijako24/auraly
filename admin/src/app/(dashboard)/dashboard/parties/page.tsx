...
import { PartyCustomerCreditRolePanel, PartyEmployeeRolePanel, PartySupplierTaxRolePanel, PartyUserRolePanel } from "@/components/parties/party-operational-role-panels";
...
import { partiesApi, type CommercialPartyRole, type PartyRole, type PartySiteInput, type PartyWorkspaceDetail, type PartyWorkspaceItem, type SellerUserAccess } from "@/services/api/parties";
...
export default function PartiesPage(){
  ...
  return <div className="space-y-6">
    ...
    {view==="list"&&<><section className="grid gap-3 rounded-2xl border bg-card p-4 xl:grid-cols-[minmax(0,1fr)_14rem_14rem]"><label className="relative"><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-muted-foreground"/><Input className="pl-9" name="party-list-filter" autoComplete="off" data-1p-ignore value={search} onChange={(e)=>{setSearch(e.target.value);setPage(1)}} placeholder="Nombre, documento, correo o teléfono"/></label><Select value={role} onValueChange={(v)=>{setRole(v);setPage(1)}}><SelectTrigger><SelectValue placeholder="Rol"/></SelectTrigger><SelectContent><SelectItem value="all">Todos los roles</SelectItem>{Object.entries(roleLabels).map(([value,label])=><SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select><Select value={status} onValueChange={(v)=>{setStatus(v);setPage(1)}}><SelectTrigger><SelectValue placeholder="Estado"/></SelectTrigger><SelectContent><SelectItem value="all">Todos los estados</SelectItem><SelectItem value="active">Activos</SelectItem><SelectItem value="inactive">Inactivos</SelectItem></SelectContent></Select></section>
...
function CreateThirdPartyDialog({open,permissions,initialParty,onClose}:{open:boolean;permissions:Set<string>;initialParty?:PartyWorkspaceDetail;onClose:()=>void}){
  ...
  const supplierTaxRules=useQuery({queryKey:["withholding-rules",businessId,"supplier-create-options"],queryFn:()=>taxationApi.listRules(false),enabled:open&&role==="Supplier"&&Boolean(businessId)});
    ...
    try{
      if(isCommercialRole(role)){
        ...
        if(role==="Customer"&&allowsCreditSales){
          ...
          await receivablesApi.updateCreditProfile(customerId,{businessId,creditLimit:null,defaultDueDays:30,isCreditEnabled:true});
        }
        if(role==="Supplier"){
          const supplierId=(accepted as {supplierId:string}).supplierId;
          const cityCode=cities.data?.find(item=>item.cityId===city)?.code??null;
          await taxationApi.saveProfile({businessId,counterpartyId:supplierId,appliesWithholding:true,responsibilities:[...supplierTaxResponsibilities],jurisdictionCode:cityCode});
        }
      }
      else{
  ...
  return <Dialog open={open} onOpenChange={(v)=>!v&&close()}><DialogContent className="max-h-[94vh] max-w-6xl overflow-hidden p-0"><div className="grid max-h-[94vh] lg:grid-cols-[250px_minmax(0,1fr)]"><aside className="hidden bg-gradient-to-b from-slate-950 to-teal-950 p-6 text-white lg:block"><p className="text-xs font-bold uppercase tracking-[.18em] text-teal-300">Terceros de Auraly</p><h2 className="mt-2 text-2xl font-semibold">Una identidad, varios roles</h2><p className="mt-3 text-sm text-slate-300">Los datos generales se comparten. Cada tipo de tercero conserva su configuración específica.</p><ol className="mt-8 space-y-4 text-sm"><li className="rounded-xl bg-white/10 p-3">1. Identidad</li><li className="rounded-xl bg-white/10 p-3">2. Ubicación</li><li className="rounded-xl bg-white/10 p-3">3. Configuración del tipo</li></ol></aside><div className="max-h-[94vh] overflow-y-auto p-6"><DialogHeader><DialogTitle>{role?`${initialParty?"Agregar":"Nuevo"} ${roleLabels[role].toLowerCase()}`:initialParty?`Agregar rol a ${initialParty.displayName}`:"Nuevo tercero"}</DialogTitle><DialogDescription>{role?"Completa los datos comunes y la configuración específica del rol.":"Selecciona un solo rol. Después podrás agregar otros roles a la misma identidad."}</DialogDescription></DialogHeader>
    {!role?<div className="mt-6 space-y-5">
      ...
      {role==="Customer"&&<div className="col-span-full space-y-4 rounded-2xl border bg-muted/15 p-4"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h3 className="font-semibold">Otras sedes del cliente</h3><p className="text-sm text-muted-foreground">Se guardan en la misma operación y estarán disponibles para ver y editar desde la ficha del tercero.</p></div><Button type="button" variant="outline" onClick={()=>setAdditionalSites(current=>[...current,newCustomerSite(current.length+2,country)])}><Plus className="mr-2 h-4 w-4"/>Agregar sede</Button></div>{fieldErrors.additionalSites&&<p className="text-sm text-destructive">{fieldErrors.additionalSites}</p>}{additionalSites.map((site,index)=><CustomerSiteCreateCard key={site.clientKey} site={site} position={index+2} onChange={next=>{setAdditionalSites(current=>current.map(item=>item.clientKey===site.clientKey?next:item));clearFieldError("additionalSites")}} onRemove={()=>setAdditionalSites(current=>current.filter(item=>item.clientKey!==site.clientKey))}/>)}</div>}
      <FormSectionTitle title={`Configuración de ${roleLabels[role].toLowerCase()}`} description={roleDescriptions[role]} />{role==="Customer"&&<><Field label="Precio asignado"><Select value={pricingMode} onValueChange={(v)=>{setPricingMode(v);setPricingId("")}}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="Default">Precio público</SelectItem><SelectItem value="Channel">Canal de precios</SelectItem></SelectContent></Select></Field>{pricingMode!=="Default"&&<><Field label="Canal" error={fieldErrors.pricingId}><Select value={pricingId} onValueChange={setPricingId} disabled={pricingOptions.isLoading}><SelectTrigger><SelectValue placeholder={pricingOptions.isLoading?"Cargando...":"Selecciona"}/></SelectTrigger><SelectContent>{pricingOptions.data?.priceChannels.map(x=><SelectItem key={x.id} value={x.id}>{x.name} ({x.code})</SelectItem>)}</SelectContent></Select></Field><Field label="Vigente desde" error={fieldErrors.pricingValidFrom}><Input type="date" value={pricingValidFrom} onChange={event=>{setPricingValidFrom(event.target.value);clearFieldError("pricingValidFrom")}}/></Field><Field label="Vigente hasta (opcional)"><Input type="date" min={pricingValidFrom} value={pricingValidUntil} onChange={event=>{setPricingValidUntil(event.target.value);clearFieldError("pricingValidUntil")}}/></Field></>}<label className="col-span-full flex items-center justify-between rounded-xl border p-4"><span><b className="block text-sm">Permite ventas a crédito</b><small className="text-muted-foreground">Habilita este cliente para facturar con saldo pendiente y crea su perfil de cartera.</small></span><Switch checked={allowsCreditSales} onCheckedChange={setAllowsCreditSales}/></label><label className="col-span-full flex items-center justify-between rounded-xl border p-4"><span><b className="block text-sm">Siempre emitir factura electrónica</b><small className="text-muted-foreground">Al seleccionar este cliente en caja, el tipo de documento queda fijado en factura electrónica.</small></span><Switch checked={requiresElectronicInvoice} onCheckedChange={setRequiresElectronicInvoice}/></label></>}
      {role==="Supplier"&&<div className="col-span-full grid gap-4 rounded-2xl border bg-muted/15 p-4 md:grid-cols-2"><div className="md:col-span-2"><b className="block text-sm">Perfil tributario del proveedor</b><small className="text-muted-foreground">Se activa automáticamente. Las responsabilidades provienen de Contabilidad y la jurisdicción corresponde a la ciudad seleccionada.</small></div><div className="space-y-2"><Label>Responsabilidades tributarias</Label><Select value={supplierResponsibilityToAdd} onValueChange={value=>{setSupplierResponsibilityToAdd("");setSupplierTaxResponsibilities(current=>new Set(current).add(value))}}><SelectTrigger><SelectValue placeholder={supplierTaxRules.isLoading?"Cargando catálogo...":"Agregar responsabilidad"}/></SelectTrigger><SelectContent>{[...new Set((supplierTaxRules.data??[]).flatMap(rule=>rule.requiredResponsibilities))].filter(code=>!supplierTaxResponsibilities.has(code)).sort().map(code=><SelectItem key={code} value={code}>{code}</SelectItem>)}</SelectContent></Select><div className="flex min-h-12 flex-wrap gap-2 rounded-xl border bg-background p-3">{[...supplierTaxResponsibilities].map(code=><Badge key={code} variant="secondary" className="gap-1">{code}<button type="button" aria-label={`Quitar ${code}`} onClick={()=>setSupplierTaxResponsibilities(current=>{const next=new Set(current);next.delete(code);return next})}><X className="h-3 w-3"/></button></Badge>)}{supplierTaxResponsibilities.size===0&&<span className="text-sm text-muted-foreground">Sin responsabilidades seleccionadas</span>}</div><p className="text-xs text-muted-foreground">Crea o modifica estas opciones en Contabilidad → Retenciones.</p></div><Field label="Ciudad o jurisdicción tributaria"><Input readOnly value={cities.data?.find(item=>item.cityId===city)?.name??"Selecciona la ciudad de la sede"}/></Field></div>}
...
function CustomerSiteCreateCard({site,position,onChange,onRemove}:{site:NewSite;position:number;onChange:(site:NewSite)=>void;onRemove:()=>void}){
  ...
  return <section className="space-y-4 rounded-2xl border bg-background p-4"><div className="flex items-center justify-between gap-3"><div><p className="font-semibold">{site.name||`Sede ${position}`}</p><p className="text-xs text-muted-foreground">{site.code} · {site.addressLine||"Dirección pendiente"}</p></div><div className="flex gap-1"><Button type="button" size="sm" variant="outline" onClick={()=>setExpanded(value=>!value)}><Pencil className="mr-1 h-4 w-4"/>{expanded?"Contraer":"Editar"}</Button><Button type="button" size="sm" variant="ghost" className="text-destructive" onClick={onRemove}><X className="mr-1 h-4 w-4"/>Quitar</Button></div></div>{expanded&&<div className="grid gap-4 border-t pt-4 md:grid-cols-2">
...
function PartyDetailDialog({target,onClose,onAddRole}:{target?:{partyId:string;edit:boolean};onClose:()=>void;onAddRole:(party:PartyWorkspaceDetail)=>void}){
  ...
  useEffect(()=>{if(!detail)return;setName(detail.displayName);setLegalName(detail.legalName??"");setFirstName(detail.firstName??"");setLastName(detail.lastName??"");setVerificationDigit(detail.verificationDigit??"");setEmail(detail.email??"");setPhone(detail.phone??"");setCustomerPriceChannelId(detail.customer?.priceChannelId??"");setCustomerRequiresInvoice(detail.customer?.requiresElectronicInvoice??false);setCustomerPriceValidFrom(detail.customer?.validFrom?.slice(0,10)??todayInput());setCustomerPriceValidUntil(detail.customer?.validUntil?.slice(0,10)??"");setSellerCode(detail.seller?.code??"");setSellerCommission(detail.seller?.defaultCommissionPercent==null?"":String(detail.seller.defaultCommissionPercent));setSellerBasis(detail.seller?.commissionBasis??"SaleAfterTax");setSellerTrigger(detail.seller?.commissionTrigger??"Sale");setCarrierCode(detail.carrier?.code??"");setCarrierMode(detail.carrier?.transportationMode??"Road");setSiteDrafts((detail.sites?.length?detail.sites:detail.primarySite?[detail.primarySite]:[]).map(partySiteDraft));if(isFinalConsumer(detail))setEditing(false)},[detail]);
  if(!target)return null;
  // clientKey is local card identity and is deliberately stripped from the API payload.
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const save=async()=>{if(!detail)return;if(detail.customer&&(!siteDrafts.length||siteDrafts.filter(site=>site.isPrimary).length!==1)){toast.error("El cliente debe conservar al menos una sede y exactamente una debe ser principal.");return}if(customerPriceChannelId&&customerPriceValidUntil&&customerPriceValidUntil<=customerPriceValidFrom){toast.error("La vigencia final del canal debe ser posterior a la inicial.");return}try{await update.mutateAsync({partyId:detail.partyId,request:{partyType:detail.partyType,displayName:name,legalName:detail.partyType==="Organization"?(legalName||name):null,firstName:detail.partyType==="NaturalPerson"?firstName:null,lastName:detail.partyType==="NaturalPerson"?lastName:null,verificationDigit:verificationDigit||null,email:email||null,phone:phone||null,rowVersion:detail.rowVersion,sites:detail.customer?siteDrafts.map(({clientKey,partySiteId,rowVersion,...site})=>({partySiteId:partySiteId??null,rowVersion:rowVersion??null,site})):undefined,customer:detail.customer?{priceChannelId:customerPriceChannelId||null,requiresElectronicInvoice:customerRequiresInvoice,validFrom:customerPriceChannelId?pricingDate(customerPriceValidFrom):null,validUntil:customerPriceChannelId?pricingDate(customerPriceValidUntil):null}:undefined,seller:detail.seller?{code:sellerCode,defaultCommissionPercent:sellerCommission===""?null:Number(sellerCommission),commissionBasis:sellerBasis,commissionTrigger:sellerTrigger}:undefined,carrier:detail.carrier?{code:carrierCode,transportationMode:carrierMode}:undefined}});await Promise.all([...roleSaveHandlers.current.values()].map(handler=>handler()));await detailQuery.refetch();setEditing(false);toast.success("Tercero, roles y sedes actualizados")}catch(error){toast.error(error instanceof Error?error.message:"No fue posible actualizar el tercero.")}};
    ...
    {detailQuery.isLoading?<div className="py-12 text-center text-muted-foreground">Cargando información...</div>:detailQuery.isError||!detail?<div className="rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-destructive">No fue posible cargar el tercero.</div>:<Tabs id="party-identity" defaultValue="general" className="scroll-mt-5 space-y-5">
      ...
      <TabsContent value="general" className="space-y-5">
        <section className="rounded-2xl border bg-muted/15 p-5">
          <h3 className="mb-4 font-semibold">Identidad compartida</h3>
      ...
      {detail.customer&&<TabsContent value="Customer" className="space-y-4">{editing?<section className="rounded-2xl border p-5"><h3 className="mb-4 text-lg font-semibold">Configuración de cliente</h3><div className="grid gap-4 md:grid-cols-2"><Field label="Precio asignado"><Select value={customerPriceChannelId||"Public"} onValueChange={value=>setCustomerPriceChannelId(value==="Public"?"":value)}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="Public">Precio público</SelectItem>{pricingOptions.data?.priceChannels.map(channel=><SelectItem key={channel.id} value={channel.id}>{channel.name} ({channel.code})</SelectItem>)}</SelectContent></Select></Field>{customerPriceChannelId&&<><Field label="Vigente desde"><Input type="date" value={customerPriceValidFrom} onChange={event=>setCustomerPriceValidFrom(event.target.value)}/></Field><Field label="Vigente hasta (opcional)"><Input type="date" min={customerPriceValidFrom} value={customerPriceValidUntil} onChange={event=>setCustomerPriceValidUntil(event.target.value)}/></Field></>}<label className="flex items-center justify-between rounded-xl border p-4 md:col-span-2"><span><b className="block text-sm">Siempre emitir factura electrónica</b><small className="text-muted-foreground">Esta preferencia se sincroniza con el punto de venta.</small></span><Switch checked={customerRequiresInvoice} onCheckedChange={setCustomerRequiresInvoice}/></label></div></section>:<RoleCard title="Cliente" rows={[["Estado",detail.customer.isActive?"Activo":"Inactivo"],["Canal de precios",pricingOptions.data?.priceChannels.find(channel=>channel.id===detail.customer?.priceChannelId)?.name??"Precio público"],["Vigencia",detail.customer.priceChannelId?`${new Date(detail.customer.validFrom).toLocaleDateString("es-CO")} – ${detail.customer.validUntil?new Date(detail.customer.validUntil).toLocaleDateString("es-CO"):"Sin vencimiento"}`:"No aplica"],["Factura electrónica",detail.customer.requiresElectronicInvoice?"Siempre":"Según la venta"]]}/>}<PartyCustomerCreditRolePanel customerId={detail.customer.customerId} editing={editing} registerSave={registerRoleSave}/></TabsContent>}
      {detail.supplier&&<TabsContent value="Supplier" className="space-y-4"><RoleCard title="Proveedor" rows={[["Estado",detail.supplier.isActive?"Activo":"Inactivo"],["Identificador",detail.supplier.supplierId]]}/><PartySupplierTaxRolePanel supplierId={detail.supplier.supplierId} editing={editing} primarySite={detail.primarySite} registerSave={registerRoleSave}/></TabsContent>}
      {detail.seller&&<TabsContent value="Seller" className="space-y-4">{editing?<section className="rounded-2xl border p-5"><h3 className="mb-4 text-lg font-semibold">Configuración de vendedor</h3><div className="grid gap-4 md:grid-cols-2"><Field label="Código"><Input value={sellerCode} onChange={event=>setSellerCode(event.target.value.toLocaleUpperCase())}/></Field><Field label="Comisión %"><Input type="number" min="0" max="100" value={sellerCommission} onChange={event=>setSellerCommission(event.target.value)}/></Field><Field label="Base de comisión"><Select value={sellerBasis} onValueChange={setSellerBasis}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="SaleBeforeTax">Venta antes de IVA</SelectItem><SelectItem value="SaleAfterTax">Venta después de IVA</SelectItem><SelectItem value="GrossMargin">Margen bruto</SelectItem></SelectContent></Select></Field><Field label="Causación"><Select value={sellerTrigger} onValueChange={setSellerTrigger}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="Sale">Al vender</SelectItem><SelectItem value="Collection">Al recaudar</SelectItem></SelectContent></Select></Field></div></section>:<RoleCard title="Vendedor" rows={[["Estado",detail.seller.isActive?"Activo":"Inactivo"],["Código",detail.seller.code],["Comisión",detail.seller.defaultCommissionPercent==null?"Sin comisión":detail.seller.defaultCommissionPercent+" %"],["Base",detail.seller.commissionBasis],["Causación",detail.seller.commissionTrigger]]}/>}<SellerAccessCard detail={detail}/></TabsContent>}
      {detail.carrier&&<TabsContent value="Carrier">{editing?<section className="rounded-2xl border p-5"><h3 className="mb-4 text-lg font-semibold">Configuración de transportador</h3><div className="grid gap-4 md:grid-cols-2"><Field label="Código"><Input value={carrierCode} onChange={event=>setCarrierCode(event.target.value.toLocaleUpperCase())}/></Field><Field label="Modalidad"><Select value={carrierMode} onValueChange={setCarrierMode}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="Road">Terrestre</SelectItem><SelectItem value="Air">Aérea</SelectItem><SelectItem value="Maritime">Marítima</SelectItem><SelectItem value="Other">Otra</SelectItem></SelectContent></Select></Field></div></section>:<RoleCard title="Transportador" rows={[["Estado",detail.carrier.isActive?"Activo":"Inactivo"],["Código",detail.carrier.code],["Modalidad",detail.carrier.transportationMode]]}/>}</TabsContent>}
      {detail.employee&&<TabsContent value="Employee"><PartyEmployeeRolePanel employeeId={detail.employee.employeeId} editing={editing} registerSave={registerRoleSave}/></TabsContent>}
      {detail.user&&<TabsContent value="User"><PartyUserRolePanel user={detail.user} editing={editing} registerSave={registerRoleSave}/></TabsContent>}
    </Tabs>}
    <DialogFooter><Button variant="outline" onClick={onClose}>Cerrar</Button>{detail&&!systemParty&&!editing&&detail.roles.length<Object.keys(roleLabels).length&&<Button variant="outline" onClick={()=>onAddRole(detail)}><Plus className="mr-2 h-4 w-4"/>Agregar rol</Button>}{detail&&!systemParty&&(!editing?<Button onClick={()=>setEditing(true)}><Pencil className="mr-2 h-4 w-4"/>Editar información</Button>:<><Button variant="ghost" onClick={()=>setEditing(false)}>Cancelar</Button><Button onClick={save} disabled={update.isPending}>{update.isPending?"Guardando...":"Guardar tercero"}</Button></>)}</DialogFooter>
...
function Field({label,children,error}:{label:string;children:React.ReactNode;error?:string}){return <div className={`space-y-2 ${error?"[&_input]:border-destructive [&_button]:border-destructive":""}`}><Label>{label}{error&&<span className="text-destructive"> *</span>}</Label>{children}<p className={`min-h-5 text-sm ${error?"text-destructive":"invisible"}`} aria-live="polite">{error??"Sin error"}</p></div>}
