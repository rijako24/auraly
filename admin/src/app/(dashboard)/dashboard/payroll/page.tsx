...
import { BarChart3, Calculator, CheckCircle2, ExternalLink, FilePlus2, FileText, Landmark, ListChecks, Loader2, Pencil, RefreshCw, Settings2, Trash2, Users, Wallet } from "lucide-react";
...
export default function PayrollPage() {
    ...
    if (!tenantId || !businessId) return <Card className="rounded-3xl"><CardContent className="p-8 text-center text-muted-foreground">Selecciona una empresa y una sede para trabajar con nómina.</CardContent></Card>;
    if (!permissions.has("payroll.read")) return <Card className="rounded-3xl"><CardContent className="p-8 text-center text-muted-foreground">No tienes permiso para consultar nómina.</CardContent></Card>;
    return <PayrollWorkspace key={`${tenantId}:${businessId}:${user?.userId}`} tenantId={tenantId} businessId={businessId} permissions={permissions}/>;
}
function PayrollWorkspace({ tenantId, businessId, permissions }: { tenantId: string; businessId: string; permissions: Set<string> }) {
    const queryClient = useQueryClient();
    const [selectedId, setSelectedId] = useState<string | null>(null), [section, setSection] = useState<Section>("runs");
    const workspace = useQuery({ queryKey: ["payroll-workspace", tenantId, businessId], queryFn: async () => {
        const [options, runs] = await Promise.all([payrollApi.options(), payrollApi.runs()]);
        return { options, runs };
    } });
    const detail = useQuery({ queryKey: ["payroll-run", tenantId, businessId, selectedId], queryFn: () => payrollApi.run(selectedId!), enabled: Boolean(selectedId) });
    const options = workspace.data?.options ?? null, runs = workspace.data?.runs ?? [], selected = detail.data ?? null, loading = workspace.isFetching;
    ...
    const subscription = useQuery({ queryKey: ["tenant-commercial", "subscription", tenantId], queryFn: tenantCommercialApi.subscription, enabled: permissions.has("payroll.fiscal") });
    const dianRemaining = subscription.data ? Math.max(0, subscription.data.dianDocumentMonthlyLimit - subscription.data.dianDocumentsUsed) : null;
    const openElectronic = () => {
        setCreatingElectronic(true);
    };
    async function load() { await Promise.all([workspace.refetch(), ...(selectedId ? [detail.refetch()] : []), queryClient.invalidateQueries({ queryKey: ["payroll-employments", tenantId, businessId] })]); }
    async function selectRun(id: string) { setSelectedId(id); }
    useEffect(() => { const query = new URLSearchParams(window.location.search), requested = query.get("section") as Section | null, partyId = query.get("partyId"); if (requested && ["runs", "employments", "novelties", "payments", "electronic", "reports", "configuration"].includes(requested))
        ...
        setCreatingEmployment(permissions.has("payroll.manage"));
    } }, [permissions]);
    return <div className="space-y-5 [&_table]:tabular-nums">
    <header className="overflow-hidden rounded-3xl bg-gradient-to-r from-slate-950 via-cyan-950 to-teal-700 p-6 text-white shadow-lg"><div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-cyan-200">Nómina Auraly</p><h1 className="mt-2 text-3xl font-black">Nómina clara y conectada</h1><p className="mt-2 max-w-2xl text-sm text-cyan-50/80">Liquida, registra los pagos y envía la nómina electrónica. Todo conectado con tu contabilidad.</p></div>
    {section === "runs" && permissions.has("payroll.calculate") && <Button className="h-12 bg-white text-slate-950 hover:bg-cyan-50" disabled={!options || workspace.isError} onClick={() => setCreatingRun(true)}><FilePlus2 className="mr-2 h-4 w-4"/>Nueva liquidación</Button>}
    {section === "employments" && permissions.has("payroll.manage") && <Button className="h-12 bg-white text-slate-950 hover:bg-cyan-50" disabled={!options || workspace.isError} onClick={() => setCreatingEmployment(true)}><Users className="mr-2 h-4 w-4"/>Nuevo contrato</Button>}</div></header>
    <Tabs value={section} onValueChange={value => { setSection(value as Section); const url = new URL(window.location.href); url.searchParams.set("section", value); url.searchParams.delete("partyId"); window.history.replaceState(null, "", url); }} className="space-y-5">
    <div className="flex min-w-0 gap-2"><div className="min-w-0 flex-1 overflow-x-auto rounded-2xl border bg-card p-1"><TabsList aria-label="Secciones de nómina" className="h-auto w-max justify-start gap-1 bg-transparent p-0">{([
      ["runs", "Liquidaciones", Calculator], ["employments", "Trabajadores", Users], ["novelties", "Novedades y descuentos", ListChecks], ["payments", "Pagos", Wallet], ["electronic", "Electrónica DIAN", FileText], ["reports", "Reportes", BarChart3], ["configuration", "Configuración", Settings2]
    ] as const).map(([value, label, Icon]) => <TabsTrigger key={value} value={value} className="shrink-0 rounded-xl px-4 py-2.5 font-semibold data-[state=active]:bg-slate-950 data-[state=active]:text-white data-[state=active]:shadow"><Icon className="mr-2 h-4 w-4"/>{label}</TabsTrigger>)}</TabsList></div><Button aria-label="Actualizar nómina" variant="outline" className="h-12 rounded-2xl" disabled={loading} onClick={() => void load()}><RefreshCw className={loading ? "h-4 w-4 animate-spin" : "h-4 w-4"}/></Button></div>
    {workspace.isPending ? <div aria-label="Cargando nómina" className="grid gap-5 lg:grid-cols-2"><Skeleton className="h-72 rounded-3xl"/><Skeleton className="h-72 rounded-3xl"/></div> : workspace.isError ? <Card className="rounded-3xl"><CardContent className="space-y-3 p-6" role="alert"><p>{message(workspace.error, "No fue posible cargar la nómina.")}</p><Button variant="outline" onClick={() => void load()}>Reintentar</Button></CardContent></Card> : <TabsContent value={section} className="mt-0">
    {detail.isError && section === "runs" && <div role="alert" className="mb-4 rounded-2xl border border-destructive/30 p-4 text-sm">{message(detail.error, "No fue posible abrir la liquidación.")} <Button variant="outline" size="sm" onClick={() => void detail.refetch()}>Reintentar detalle</Button></div>}
    {detail.isFetching && section === "runs" && <p role="status" className="mb-3 text-sm text-muted-foreground">Cargando detalle…</p>}
    {section === "runs" && (
      <RunsSection options={options} runs={runs} selected={selected} canCalculate={permissions.has("payroll.calculate") && !detail.isFetching && !detail.isError} canApprove={permissions.has("payroll.approve") && !detail.isFetching && !detail.isError} onSelect={selectRun} onChanged={load}/>
    )}
    {section === "employments" && (
    ...
    {section === "electronic" && (
      <ElectronicSection options={options} canGenerate={permissions.has("payroll.fiscal")} onGenerate={openElectronic}/>
    )}
    {section === "reports" && (
      <PayrollReports/>
    )}
    {section === "configuration" && (
      <ConfigurationSection options={options} canConfigure={permissions.has("payroll.configure")} canReadAccounting={permissions.has("accounting.read")} canConfigureAccounting={permissions.has("accounting.configure")} onConcept={() => setCreatingConcept(true)} onEditConcept={setEditingConcept} onRules={() => setCreatingRules(true)} onEditRules={setEditingRules} onChanged={load}/>
    )}
    </TabsContent>}
    </Tabs>
    <Dialog open={creatingRun} onOpenChange={setCreatingRun}><DialogContent><DialogHeader><DialogTitle>Nueva liquidación</DialogTitle><DialogDescription>Selecciona el período, la periodicidad y las reglas vigentes.</DialogDescription></DialogHeader>{options && <RunForm businessId={businessId} options={options} runs={runs} onSaved={async () => { setCreatingRun(false); await load(); }}/>}</DialogContent></Dialog>
    ...
    } }}><DialogContent className="max-h-[92dvh] max-w-3xl overflow-y-auto"><DialogHeader><DialogTitle>{editingEmployment ? "Editar relación laboral" : "Nueva relación laboral"}</DialogTitle><DialogDescription>Selecciona un empleado y completa sus datos laborales y de pago.</DialogDescription></DialogHeader>{options && <EmploymentForm businessId={businessId} options={options} initial={editingEmployment} partyId={preselectedPartyId} onSaved={async () => { setCreatingEmployment(false); setEditingEmployment(null); setPreselectedPartyId(null); setEmploymentRevision(value => value + 1); await load(); }}/>}</DialogContent></Dialog>
    <Dialog open={creatingConcept || editingConcept !== null} onOpenChange={open => { if (!open) { setCreatingConcept(false); setEditingConcept(null); } }}><DialogContent className="max-h-[92dvh] max-w-3xl overflow-y-auto"><DialogHeader><DialogTitle>{editingConcept ? "Editar concepto" : "Nuevo concepto"}</DialogTitle><DialogDescription>Define cómo se calcula, contabiliza y reporta este concepto.</DialogDescription></DialogHeader>{options && <ConceptForm options={options} initial={editingConcept} onSaved={async () => { setCreatingConcept(false); setEditingConcept(null); await load(); }}/>}</DialogContent></Dialog>
    ...
    <Dialog open={creatingElectronic} onOpenChange={setCreatingElectronic}><DialogContent><DialogHeader><DialogTitle>Consolidar nómina electrónica</DialogTitle><DialogDescription>Reserva la serie, crea el XML por trabajador y lo envía al motor fiscal para validar, firmar y transmitir a la DIAN.</DialogDescription></DialogHeader><p className="rounded-xl bg-muted p-3 text-sm">{dianRemaining === null ? "El cupo se verificará al preparar los documentos." : <>Cupo disponible: <b>{dianRemaining.toLocaleString("es-CO")} documentos DIAN</b>. Se requiere uno por trabajador.</>}{dianRemaining === 0 && " Para generar documentos nuevos necesitas ampliar el cupo; puedes recuperar un mes ya preparado."}</p><ElectronicPeriodForm businessId={businessId} onSaved={async () => { setCreatingElectronic(false); await load(); }}/></DialogContent></Dialog>
    ...
    <Dialog open={creatingNovelty} onOpenChange={setCreatingNovelty}><DialogContent className="max-h-[92dvh] max-w-3xl overflow-y-auto"><DialogHeader><DialogTitle>Nueva novedad</DialogTitle><DialogDescription>Registra el ingreso, ausencia o descuento que se incluirá al liquidar.</DialogDescription></DialogHeader>{options && <NoveltyForm options={options} onSaved={async () => { setCreatingNovelty(false); await load(); }}/>}</DialogContent></Dialog>
...
function RunsSection({ options, runs, selected, canCalculate, canApprove, onSelect, onChanged }: {
    ...
    return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(420px,1.2fr)]"><Card className="rounded-3xl"><CardHeader><CardTitle>Liquidaciones</CardTitle><CardDescription>Selecciona un período para revisar, calcular o aprobar su nómina.</CardDescription></CardHeader><CardContent className="space-y-2">{runs.map(run => <button type="button" key={run.payrollRunId} onClick={() => void onSelect(run.payrollRunId)} className={`w-full rounded-2xl border p-4 text-left transition-colors ${selected?.payrollRunId === run.payrollRunId ? "border-primary bg-primary/5" : "hover:bg-muted/40"}`}><span className="flex items-center justify-between gap-3"><b>{run.periodStart} — {run.periodEnd}</b><small>{statusLabel(options, run.status)}</small></span><span className="mt-2 grid grid-cols-3 text-sm text-muted-foreground"><span>{run.employeeCount} trabajadores</span><span className="text-right">Deducciones {money.format(run.totalDeductions)}</span><strong className="text-right text-foreground">Neto {money.format(run.netPayable)}</strong></span></button>)}{runs.length === 0 && <Empty text="Todavía no hay liquidaciones."/>}</CardContent></Card><Card className="rounded-3xl"><CardHeader><CardTitle>Detalle</CardTitle><CardDescription>{selected ? `${selected.periodStart} — ${selected.periodEnd}` : "Selecciona una liquidación"}</CardDescription></CardHeader><CardContent>{!selected ? <Empty text="El detalle mostrará trabajadores, conceptos y totales."/> : <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><Metric label="Devengado" value={selected.totalEarnings}/><Metric label="Deducciones" value={selected.totalDeductions}/><Metric label="Neto" value={selected.netPayable}/></div><div className="flex flex-wrap gap-2">{canCalculate && ["Draft", "Calculated"].includes(selected.status) && <Button variant="outline" disabled={busy !== null} onClick={() => void calculate()}><Calculator className="mr-2 h-4 w-4"/>{busy === "calculate" ? "Calculando…" : selected.status === "Draft" ? "Calcular nómina" : "Recalcular"}</Button>}{canApprove && selected.status === "Calculated" && <Button disabled={busy !== null} onClick={() => void approve()}><CheckCircle2 className="mr-2 h-4 w-4"/>{busy === "approve" ? "Aprobando…" : "Aprobar y contabilizar"}</Button>}</div>{selected.employees.map(employee => <details key={employee.payrollRunEmployeeId} className="rounded-2xl border p-4"><summary className="cursor-pointer font-semibold">{employee.employeeName}<span className="float-right">{money.format(employee.netPayable)}</span></summary><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[620px] text-sm"><thead><tr className="text-left text-muted-foreground"><th>Concepto</th><th>Naturaleza</th><th className="text-right">Base</th><th className="text-right">Valor</th></tr></thead><tbody>{employee.lines.map(line => <tr key={line.lineNumber} className="border-t"><td className="py-2">{line.conceptCode} · {line.conceptName}</td><td>{catalogLabel(options, "payroll-concept-nature", line.natureCode)}</td><td className="text-right">{line.baseAmount === null ? "—" : money.format(line.baseAmount)}</td><td className="text-right font-medium">{money.format(line.amount)}</td></tr>)}</tbody></table></div></details>)}</div>}</CardContent></Card></div>;
}
function EmploymentsSection({ revision, canManage, onEdit }: {
    ...
    const tenantId = useTenantContextStore(state => state.selectedTenantId), businessId = useBusinessContextStore(state => state.selectedBusinessId);
    const [page, setPage] = useState(1);
    const query = useQuery({ queryKey: ["payroll-employments", tenantId, businessId, page, revision], queryFn: () => payrollApi.employments({ page, pageSize: 25 }) });
    ...
    return <Card className="rounded-3xl"><CardHeader><CardTitle>Relaciones laborales</CardTitle><CardDescription>Administra los contratos y datos de pago de tus trabajadores.</CardDescription></CardHeader><CardContent className="space-y-3"><div className="overflow-x-auto rounded-xl border"><table className="w-full min-w-[820px] text-sm"><thead className="bg-muted/50"><tr><th className="p-3 text-left">Trabajador</th><th className="text-left">Contrato</th><th className="text-left">Vigencia</th><th className="text-left">Cuenta de pago</th><th className="text-right">Salario</th><th className="pr-3 text-right">Acciones</th></tr></thead><tbody>{items.map(item => <tr key={item.employmentId} className="border-t"><td className="p-3 font-medium">{item.employeeName}</td><td>{item.contractNumber}</td><td>{item.startDate} — {item.endDate ?? "vigente"}</td><td>{item.bankAccountNumber ? `•••• ${item.bankAccountNumber.slice(-4)}` : "Sin configurar"}</td><td className="text-right">{money.format(item.monthlySalary)}</td><td className="pr-3 text-right"><span className="inline-flex">{canManage && <Button size="sm" variant="ghost" onClick={() => onEdit(item)}><Pencil className="mr-2 h-4 w-4"/>Editar</Button>}{canManage && item.isActive && <Button size="sm" variant="ghost" className="text-destructive" onClick={() => void deactivate(item)}><Trash2 className="mr-2 h-4 w-4"/>Desactivar</Button>}</span></td></tr>)}</tbody></table>{query.isLoading && <p className="p-4 text-sm text-muted-foreground">Cargando trabajadores…</p>}{query.isError && <p className="p-4 text-sm text-destructive">No fue posible cargar los contratos. <Button size="sm" variant="outline" onClick={() => void query.refetch()}>Reintentar</Button></p>}{!query.isLoading && !query.isError && !items.length && <Empty text="Crea el primer contrato laboral."/>}</div><div className="flex items-center justify-between text-sm"><span className="text-muted-foreground">{query.data?.totalCount ?? 0} contratos</span><span className="flex gap-2"><Button size="sm" variant="outline" disabled={page <= 1 || query.isFetching} onClick={() => setPage(value => value - 1)}>Anterior</Button><Button size="sm" variant="outline" disabled={!query.data || page * query.data.pageSize >= query.data.totalCount || query.isFetching} onClick={() => setPage(value => value + 1)}>Siguiente</Button></span></div></CardContent></Card>;
}
function NoveltiesSection({ options, canManage, onAgreement, onEditAgreement, onChanged, onNovelty }: {
    ...
    onChanged: () => Promise<void>;
    onNovelty: () => void;
}) { async function deactivate(item: PayrollDeductionAgreement) { try { await payrollApi.saveDeductionAgreement({ ...item, isActive: false }); toast.success("Descuento desactivado."); await onChanged(); } catch (error) { toast.error(message(error, "No fue posible desactivar el descuento.")); } } function evidence(url: string) { try { const parsed = new URL(url); if (!['http:', 'https:'].includes(parsed.protocol)) throw new Error(); window.open(parsed.toString(), '_blank', 'noopener,noreferrer'); } catch { toast.error("La evidencia no tiene una URL válida."); } } return <div className="grid gap-5 xl:grid-cols-2"><Card className="rounded-3xl"><CardHeader><span className="flex items-center justify-between gap-3"><span><CardTitle>Descuentos autorizados</CardTitle><CardDescription>Acuerdos, libranzas, embargos y otros conceptos con soporte.</CardDescription></span>{canManage && <Button size="sm" onClick={onAgreement}>Agregar</Button>}</span></CardHeader><CardContent className="space-y-2">{options?.deductionAgreements.map(item => <div key={item.deductionAgreementId} className="rounded-xl border p-3 text-sm"><span className="flex justify-between gap-3"><b>{item.employeeName}</b><span>{item.isActive ? "Activo" : "Inactivo"}</span></span><p>{item.conceptName} · {item.authorityName}</p><p className="text-muted-foreground">Ref. {item.referenceNumber} · descontado {money.format(item.deductedToDate)}{item.authorizedTotal !== null ? ` de ${money.format(item.authorizedTotal)}` : ""}</p><div className="mt-2 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => evidence(item.evidenceUrl)}><ExternalLink className="mr-2 h-4 w-4"/>Ver evidencia</Button>{canManage && <Button size="sm" variant="ghost" onClick={() => onEditAgreement(item)}><Pencil className="mr-2 h-4 w-4"/>Editar</Button>}{canManage && item.isActive && <Button size="sm" variant="ghost" className="text-destructive" onClick={() => void deactivate(item)}><Trash2 className="mr-2 h-4 w-4"/>Desactivar</Button>}</div></div>)}{!options?.deductionAgreements.length && <Empty text="No hay descuentos autorizados."/>}</CardContent></Card><Card className="rounded-3xl"><CardHeader><span className="flex items-center justify-between gap-3"><span><CardTitle>Novedades</CardTitle><CardDescription>Ingresos, ausencias, deducciones y ajustes aplicables a una liquidación.</CardDescription></span>{canManage && <Button size="sm" onClick={onNovelty}>Agregar</Button>}</span></CardHeader><CardContent className="space-y-2">{options?.novelties.map(item => <div key={item.noveltyId} className="rounded-xl border p-3 text-sm"><span className="flex justify-between gap-3"><b>{item.employeeName}</b><span>{statusLabel(options, item.status)}</span></span><p>{item.noveltyTypeName} · {item.conceptName}</p><p className="text-muted-foreground">{item.startDate} — {item.endDate} · {money.format(item.totalAmount)}</p>{item.notes && <p>{item.notes}</p>}</div>)}{!options?.novelties.length && <Empty text="No hay novedades registradas."/>}</CardContent></Card></div>; }
function PaymentsSection({ options, canPay, onCreate }: {
    options: PayrollOptions | null;
    canPay: boolean;
    onCreate: () => void;
}) { return <Card className="rounded-3xl"><CardHeader><span className="flex items-center justify-between gap-3"><span><CardTitle>Pagos de nómina</CardTitle><CardDescription>Cada confirmación salda el neto por trabajador y genera su comprobante contable.</CardDescription></span>{canPay && <Button onClick={onCreate}>Confirmar pago</Button>}</span></CardHeader><CardContent><div className="overflow-x-auto rounded-xl border"><table className="w-full min-w-[680px] text-sm"><thead className="bg-muted/50"><tr><th className="p-3 text-left">Fecha</th><th className="text-left">Referencia</th><th className="text-left">Medio</th><th className="text-right">Trabajadores</th><th className="text-right">Total</th><th className="pr-3 text-right">Estado</th></tr></thead><tbody>{options?.paymentBatches.map(item => <tr className="border-t" key={item.paymentBatchId}><td className="p-3">{item.paymentDate}</td><td>{item.referenceNumber}</td><td>{item.paymentMethodName}</td><td className="text-right">{item.employeeCount}</td><td className="text-right font-medium">{money.format(item.totalAmount)}</td><td className="pr-3 text-right">{statusLabel(options, item.status)}</td></tr>)}</tbody></table>{!options?.paymentBatches.length && <Empty text="No hay pagos confirmados."/>}</div></CardContent></Card>; }
function ElectronicSection({ options, canGenerate, onGenerate }: {
    options: PayrollOptions | null;
    canGenerate: boolean;
    onGenerate: () => void;
}) { return <Card className="rounded-3xl"><CardHeader><span className="flex items-center justify-between gap-3"><span><CardTitle>Documentos electrónicos DIAN</CardTitle><CardDescription>Consolidación mensual por trabajador. La presentación se confirma cuando la DIAN acepta el documento.</CardDescription></span>{canGenerate && <Button onClick={onGenerate}>Consolidar mes</Button>}</span></CardHeader><CardContent className="space-y-3"><div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border bg-muted/20 p-4 text-sm"><p>{options?.settings?.electronicPayrollEnabled ? "Nómina electrónica habilitada." : "Activa la nómina electrónica en Configuración."} {options?.electronicConfiguration?.isActive ? `Serie ${options.electronicConfiguration.prefix} configurada.` : "La serie y las credenciales DIAN requieren configuración."}</p><Button variant="outline" asChild><Link href="/dashboard/settings/fiscal">Configuración DIAN <ExternalLink className="ml-2 h-3 w-3"/></Link></Button></div>{options?.electronicPeriods.map(period => <details key={period.electronicPeriodId} className="rounded-xl border p-4"><summary className="cursor-pointer font-semibold">{period.year}-{String(period.month).padStart(2, "0")} · {statusLabel(options, period.status)}<span className="float-right">{period.documents.length} documentos</span></summary><div className="mt-3 space-y-2">{period.documents.map(document => <div key={document.electronicPayrollDocumentId} className="flex flex-wrap justify-between gap-2 rounded-lg bg-muted/30 p-3 text-sm"><span>{document.employeeName}</span><span>{statusLabel(options, document.status)}</span></div>)}</div></details>)}{!options?.electronicPeriods.length && <Empty text="No se han consolidado períodos electrónicos."/>}</CardContent></Card>; }
...
function RunForm({ businessId, options, runs, onSaved }: {
    ...
    setOriginalId(""); }}><SelectTrigger aria-label="Tipo"><SelectValue /></SelectTrigger><SelectContent>{catalog(options, "payroll-run-kind").filter(item => item.isActive).map(item => <SelectItem key={item.optionId} value={item.code}>{item.label}</SelectItem>)}</SelectContent></Select></Field>{kind === "Adjustment" && <Field label="Liquidación original"><Select value={originalId} onValueChange={value => { setOriginalId(value); const original = runs.find(run => run.payrollRunId === value); if (original) {
    setStart(original.periodStart);
    setEnd(original.periodEnd);
    setPayment(original.paymentDate);
} }}><SelectTrigger aria-label="Liquidación original"><SelectValue placeholder="Selecciona"/></SelectTrigger><SelectContent>{runs.filter(run => run.status === "Approved" && run.runKind === "Regular").map(run => <SelectItem key={run.payrollRunId} value={run.payrollRunId}>{run.periodStart} — {run.periodEnd}</SelectItem>)}</SelectContent></Select></Field>}<CatalogField label="Periodicidad" options={catalog(options, "payroll-pay-frequency")} value={frequency} onChange={setFrequency}/><Field label="Reglas aprobadas"><Select value={ruleSetId} onValueChange={setRuleSetId}><SelectTrigger aria-label="Reglas aprobadas"><SelectValue placeholder="Selecciona"/></SelectTrigger><SelectContent>{options.ruleSets.filter(item => item.status === "Approved" && item.effectiveFrom <= start && (!item.effectiveTo || item.effectiveTo >= end)).map(item => <SelectItem key={item.ruleSetId} value={item.ruleSetId}>{item.code} · {item.name}</SelectItem>)}</SelectContent></Select></Field><Field label="Inicio"><DatePicker value={start} onChange={setStart}/></Field><Field label="Fin"><DatePicker value={end} onChange={setEnd}/></Field><Field label="Fecha de pago"><DatePicker value={payment} onChange={setPayment}/></Field>{kind === "Adjustment" && <p className="self-end rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">El ajuste liquida únicamente novedades nuevas como diferencias; no repite el salario ya aprobado.</p>}<DialogFooter className="sm:col-span-2"><Button disabled={busy || !frequency || !ruleSetId || !start || !end || !payment || end < start || payment < start || (kind === "Adjustment" && !originalId)} onClick={() => void save()}>{busy && <Loader2 className="mr-2 h-4 w-4 animate-spin"/>}Crear borrador</Button></DialogFooter></div>; }
...
function NoveltyForm({ options, onSaved }: {
    ...
    setBusy(false);
} } return <div className="grid gap-4 sm:grid-cols-2"><Field label="Trabajador"><PayrollEmploymentSelect value={form.employmentId} onChange={value=>setForm(current => ({ ...current, employmentId: value, deductionAgreementId: null }))}/></Field><Field label="Concepto"><Select value={form.conceptId} onValueChange={value => setForm(current => ({ ...current, conceptId: value, deductionAgreementId: null, unitAmount: null, totalAmount: 0 }))}><SelectTrigger aria-label="Concepto"><SelectValue placeholder="Selecciona"/></SelectTrigger><SelectContent>{options.concepts.filter(item => item.isActive).map(item => <SelectItem key={item.conceptId} value={item.conceptId}>{item.code} · {item.name}</SelectItem>)}</SelectContent></Select></Field><CatalogField label="Tipo de novedad" options={catalog(options, "payroll-novelty-type")} value={form.noveltyTypeOptionId} onChange={value => set("noveltyTypeOptionId", value)}/><Field label="Acuerdo de descuento (si aplica)"><Select value={form.deductionAgreementId ?? "_none"} onValueChange={value => set("deductionAgreementId", value === "_none" ? null : value)}><SelectTrigger aria-label="Acuerdo de descuento (si aplica)"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="_none">No aplica</SelectItem>{agreements.map(item => <SelectItem key={item.deductionAgreementId} value={item.deductionAgreementId}>{item.referenceNumber}</SelectItem>)}</SelectContent></Select></Field><Field label="Inicio"><DatePicker value={form.startDate} onChange={value => set("startDate", value)}/></Field><Field label="Fin"><DatePicker value={form.endDate} onChange={value => set("endDate", value)}/></Field><Field label="Cantidad"><Input type="number" min={0.000001} step="any" value={form.quantity} onChange={event => set("quantity", event.currentTarget.valueAsNumber || 0)}/></Field><Field label={method === "PercentageOfBase" ? "Tarifa decimal" : "Valor unitario"}><Input type="number" min={0} step="any" value={form.unitAmount ?? ""} onChange={event => set("unitAmount", event.target.value ? event.currentTarget.valueAsNumber : null)}/></Field><Field label={needsRate ? "Total calculado por el motor" : "Valor total"}><Input type="number" min={0} step="any" disabled={needsRate} value={form.totalAmount} onChange={event => set("totalAmount", event.currentTarget.valueAsNumber || 0)}/></Field><Field label="Evidencia (URL)"><Input type="url" value={form.evidenceUrl ?? ""} onChange={event => set("evidenceUrl", event.target.value || null)}/></Field><label className="space-y-2 sm:col-span-2"><span className="block text-sm font-medium">Notas</span><Input maxLength={500} value={form.notes ?? ""} onChange={event => set("notes", event.target.value || null)}/></label><DialogFooter className="sm:col-span-2"><Button disabled={busy || !form.employmentId || !form.conceptId || !form.noveltyTypeOptionId || form.quantity <= 0 || form.endDate < form.startDate || (selectedConcept?.requiresDeductionAgreement && !form.deductionAgreementId) || (needsRate && form.unitAmount === null)} onClick={() => void save()}>Registrar novedad</Button></DialogFooter></div>; }
function PaymentForm({ options, runs, onSaved }: {
    options: PayrollOptions;
    runs: PayrollRunSummary[];
    onSaved: () => Promise<void>;
}) { const today = new Date().toISOString().slice(0, 10), [runId, setRunId] = useState(""), [methodId, setMethodId] = useState(""), [date, setDate] = useState(today), [reference, setReference] = useState(""), [busy, setBusy] = useState(false), [paymentBatchId] = useState(() => crypto.randomUUID()), paidRunIds = new Set(options.paymentBatches.filter(item => item.status === "Confirmed").map(item => item.payrollRunId)); async function save() { setBusy(true); try {
    await payrollApi.createPayment({ paymentBatchId, payrollRunId: runId, paymentMethodOptionId: methodId, paymentDate: date, referenceNumber: reference });
...
function PayrollAccountingMappings({ options, canConfigure }: {
...
} }, [tenantId, businessId]); useEffect(() => { void loadAccounting(); }, [loadAccounting]); const categories = catalog(options, "payroll-accounting-category").map(option => ({ option, definition: definitions.find(item => item.category === option.code) })); async function save(category: string, accountId: string) { if (!tenantId || !businessId)
    ...
    setBusy(null);
} } return <Card className="rounded-3xl lg:col-span-2"><CardHeader><CardTitle>Integración contable de nómina</CardTitle><CardDescription>Las categorías nacen del catálogo de nómina y las cuentas del plan contable. La liquidación aprobada y el pago usan estas asignaciones.</CardDescription></CardHeader><CardContent className="space-y-3"><div className="rounded-2xl border bg-muted/30 p-4 text-sm"><Landmark className="mb-2 h-5 w-5 text-teal-700"/><b>Centros de costo</b><p className="mt-1 text-muted-foreground">La nómina y sus pagos usan la regla de todas las operaciones o el centro predeterminado de esta sede. El centro queda guardado en cada asiento y se conserva al reintentar. Consulta el gasto y sus aportes en el auxiliar contable, filtrando por centro.</p><Button asChild variant="link" className="mt-2 h-auto p-0"><Link href="/dashboard/accounting?section=centers">Ver centros y asignaciones <ExternalLink className="ml-2 h-3 w-3"/></Link></Button></div>{error && <p className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">{error} <Link className="underline" href="/dashboard/accounting">Abrir Contabilidad</Link></p>}{categories.map(({ option, definition }) => { const mapping = mappings.find(item => item.category === option.code && item.businessId === businessId) ?? mappings.find(item => item.category === option.code && item.businessId === null); return <div key={option.optionId} className="grid gap-2 rounded-xl border p-3 md:grid-cols-[minmax(220px,1fr)_minmax(260px,1.4fr)] md:items-center"><div><b className="text-sm">{option.label}</b><p className="text-xs text-muted-foreground">{option.code}</p></div><Select disabled={!canConfigure || busy !== null || Boolean(error)} value={mapping?.accountId ?? ""} onValueChange={value => void save(option.code, value)}><SelectTrigger aria-label={`Cuenta para ${option.label}`}><SelectValue placeholder="Selecciona una cuenta"/></SelectTrigger><SelectContent>{accounts.filter(account => account.isActive && account.allowsPosting && (!definition || account.accountType === definition.accountType)).map(account => <SelectItem key={account.accountId} value={account.accountId}>{account.code} · {account.name}</SelectItem>)}</SelectContent></Select></div>; })}{!categories.length && <Empty text="No hay categorías contables de nómina activas."/>}</CardContent></Card>; }
...
}) { return <Field label={label}><Select value={value} onValueChange={onChange}><SelectTrigger aria-label={label}><SelectValue placeholder="Selecciona"/></SelectTrigger><SelectContent>{options.filter(item => item.isActive).map(item => <SelectItem key={item.optionId} value={item.optionId}>{item.label}</SelectItem>)}</SelectContent></Select></Field>; }
function catalogLabel(options: PayrollOptions | null, catalogCode: string, code: string) { return options?.catalogs[catalogCode]?.find(item => item.code === code)?.label ?? code; }
function statusLabel(options: PayrollOptions | null, code: string) { return catalogLabel(options, "payroll-workflow-status", code); }
