...
function CreateDebitNote({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <Dialog open={open} onOpenChange={(value) => { if (!value) close(); }}><DialogContent className="max-h-[94dvh] max-w-4xl overflow-y-auto"><DialogHeader><DialogTitle>Nueva nota débito</DialogTitle><DialogDescription>Solo se admiten facturas electrónicas con cliente identificado. El documento aumenta la cuenta por cobrar.</DialogDescription></DialogHeader>
    {!invoice ? <div className="space-y-3"><div className="relative"><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input className="pl-9" value={invoiceSearch} onChange={(event) => setInvoiceSearch(event.target.value)} placeholder="Busca factura, CUFE, cliente o identificación" /></div><div className="max-h-80 overflow-y-auto rounded-xl border">{sales.data?.items.map((item) => <button type="button" key={item.documentId} className="flex w-full items-center justify-between gap-4 border-b p-3 text-left hover:bg-muted/60 disabled:cursor-not-allowed disabled:opacity-50" disabled={!item.customerId || !item.cufe} onClick={() => setInvoice(item)}><div><p className="font-semibold">{item.documentNumber} · {item.customerName}</p><p className="text-xs text-muted-foreground">{item.customerIdentification} · CUFE {item.cufe || "no disponible"}</p></div><span>{formatCurrency(item.totalAmount)}</span></button>)}</div></div> : <div className="space-y-5"><Card className="bg-primary/5"><CardContent className="flex items-center justify-between p-4"><div><p className="font-semibold">{invoice.documentNumber} · {invoice.customerName}</p><p className="text-xs text-muted-foreground">{invoice.customerIdentification} · {invoice.cufe}</p></div><Button variant="ghost" onClick={() => setInvoice(undefined)}>Cambiar factura</Button></CardContent></Card><div className="grid gap-4 md:grid-cols-2"><div className="space-y-2"><Label>Concepto DIAN</Label><Select value={concept} onValueChange={(value) => setConcept(value as keyof typeof concepts)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.entries(concepts).map(([code, label]) => <SelectItem key={code} value={code}>{code}. {label}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>Vencimiento de cartera</Label><Input type="date" value={dueAt} onChange={(event) => setDueAt(event.target.value)} /></div><div className="space-y-2 md:col-span-2"><Label>Motivo</Label><Input maxLength={300} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Explica por qué aumenta el valor de la factura" /></div><div className="space-y-2 md:col-span-2"><Label>Descripción de la línea</Label><Input maxLength={300} value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Intereses, gasto o ajuste facturado" /></div><div className="space-y-2"><Label>Valor antes de impuesto</Label><Input type="number" min="0.01" step="0.01" value={base} onChange={(event) => setBase(event.target.value)} /></div><div className="space-y-2"><Label>IVA</Label><Select value={taxRate} onValueChange={setTaxRate}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="0">0%</SelectItem><SelectItem value="5">5%</SelectItem><SelectItem value="19">19%</SelectItem></SelectContent></Select></div><div className="space-y-2 md:col-span-2"><Label>Observaciones</Label><Textarea maxLength={1000} value={notes} onChange={(event) => setNotes(event.target.value)} /></div></div><div className="rounded-xl border bg-muted/40 p-4 text-right"><p className="text-sm text-muted-foreground">Total de la nota</p><p className="text-2xl font-semibold">{formatCurrency((Number(base) || 0) * (1 + (Number(taxRate) || 0) / 100))}</p></div></div>}
    <DialogFooter><Button variant="outline" onClick={close}>Cancelar</Button><Button disabled={!invoice || confirm.isPending} onClick={submit}>{confirm.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Crear nota débito</Button></DialogFooter></DialogContent></Dialog>;
}
