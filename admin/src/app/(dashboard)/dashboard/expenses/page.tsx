...
function ExpenseForm({businessId,options,onSaved}:{businessId:string;options:ExpenseOptions;onSaved:()=>Promise<void>}){
  ...
  return <form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}>
    ...
    <div className="sm:col-span-2 rounded-xl border bg-muted/30 p-3 text-sm"><b>Cuenta contable:</b> {concept?`${concept.expenseAccountCode} · ${concept.expenseAccountName}`:"se define con el concepto"}<p className="mt-1 text-muted-foreground"><b>Centro de costo:</b> {concept?.defaultCostCenterName??"Sin centro predeterminado"}. La retención se calcula con el perfil tributario del proveedor.</p></div>
...
function ConceptForm({businessId,options,onSaved}:{businessId:string;options:ExpenseOptions;onSaved:()=>Promise<void>}){
  const [name,setName]=useState(""),[accountId,setAccountId]=useState(""),[costCenterId,setCostCenterId]=useState("none"),[busy,setBusy]=useState(false);
  async function save(){setBusy(true);try{await expensesApi.saveConcept({conceptId:crypto.randomUUID(),businessId,name,expenseAccountId:accountId,defaultCostCenterId:costCenterId==="none"?null:costCenterId,withholdingConceptCode:null,isActive:true});toast.success("Concepto creado.");setName("");await onSaved()}catch(error){toast.error(error instanceof Error?error.message:"No fue posible crear el concepto.")}finally{setBusy(false)}}
  return <div className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><Field label="Nombre del concepto"><Input value={name} onChange={event=>setName(event.target.value)}/></Field><Field label="Cuenta de gasto"><Select value={accountId} onValueChange={setAccountId}><SelectTrigger><SelectValue placeholder="Selecciona"/></SelectTrigger><SelectContent>{options.expenseAccounts.map(item=><SelectItem key={item.accountId} value={item.accountId}>{item.code} · {item.name}</SelectItem>)}</SelectContent></Select></Field><Field label="Centro de costo predeterminado"><Select value={costCenterId} onValueChange={setCostCenterId}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="none">Sin predeterminado</SelectItem>{options.costCenters.map(item=><SelectItem key={item.costCenterId} value={item.costCenterId}>{item.code} · {item.name}</SelectItem>)}</SelectContent></Select></Field><div className="flex items-end"><Button className="w-full" disabled={busy||!name.trim()||!accountId} onClick={()=>void save()}>{busy&&<Loader2 className="mr-2 h-4 w-4 animate-spin"/>}Crear concepto</Button></div></div><div className="space-y-2 border-t pt-4">{options.concepts.map(item=><div key={item.conceptId} className="rounded-xl border p-3 text-sm"><b>{item.name}</b><span className="block text-muted-foreground">{item.expenseAccountCode} · {item.expenseAccountName}{item.defaultCostCenterName?` · ${item.defaultCostCenterName}`:""}</span></div>)}</div></div>;
}
