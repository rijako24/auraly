...
export default function ExpensesPage() {
  ...
  async function cancelExpense(){if(!detail||!selectedCancellationReason)return;setCancelBusy(true);try{
    ...
    setDetail({...detail,status:"CancellationPending",cancellationId,cancellationReason:selectedCancellationReason.label});
  ...
  return <div className="space-y-6">
    ...
    <Dialog open={cancelling} onOpenChange={setCancelling}><DialogContent><DialogHeader><DialogTitle>Anular gasto</DialogTitle><DialogDescription>Se reversarán el gasto, IVA, retenciones y saldo por pagar. Si ya se pagó, quedará un saldo a favor frente al proveedor. El documento soporte aceptado generará una nota de ajuste ante la DIAN; si la factura la emitió el proveedor, solicita la nota crédito correspondiente.</DialogDescription></DialogHeader><Field label="Motivo de anulación"><Select value={cancelReasonOptionId} onValueChange={setCancelReasonOptionId} disabled={cancellationReasons.isPending||cancellationReasons.isError||!cancellationReasons.data?.length}><SelectTrigger><SelectValue placeholder={cancellationReasons.isPending?"Cargando motivos...":"Selecciona un motivo"}/></SelectTrigger><SelectContent>{cancellationReasons.data?.map(option=><SelectItem key={option.id} value={option.id}>{option.label}</SelectItem>)}</SelectContent></Select>{cancellationReasons.isError&&<div className="mt-2 flex items-center gap-2 text-sm text-destructive">No fue posible cargar los motivos.<Button size="sm" variant="outline" onClick={()=>void cancellationReasons.refetch()}>Reintentar</Button></div>}{!cancellationReasons.isPending&&!cancellationReasons.isError&&!cancellationReasons.data?.length&&<p className="mt-2 text-sm text-destructive">No hay motivos activos para anular gastos.</p>}</Field><DialogFooter><Button variant="outline" onClick={()=>setCancelling(false)}>Volver</Button><Button variant="destructive" disabled={cancelBusy||!selectedCancellationReason} onClick={()=>void cancelExpense()}>{cancelBusy&&<Loader2 className="mr-2 h-4 w-4 animate-spin"/>}Confirmar anulación</Button></DialogFooter></DialogContent></Dialog>
