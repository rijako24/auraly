...
export default function ExpensesPage() {
  ...
  return <div className="space-y-6">
    ...
    <Dialog open={creating} onOpenChange={setCreating}><DialogContent className="max-h-[92dvh] max-w-3xl overflow-y-auto"><DialogHeader><DialogTitle>Registrar gasto</DialogTitle><DialogDescription>El proveedor es opcional para gastos ocasionales; el concepto define la cuenta contable.</DialogDescription></DialogHeader>{options&&<ExpenseForm businessId={businessId} options={options} onSaved={async()=>{setCreating(false);await load()}}/>}</DialogContent></Dialog>
...
function ExpenseForm({businessId,options,onSaved}:{businessId:string;options:ExpenseOptions;onSaved:()=>Promise<void>}){
  ...
  const occasional=options.suppliers.find(item=>item.identification==="OCASIONAL");
  const [form,setForm]=useState<ConfirmExpense>({expenseId:crypto.randomUUID(),businessId,supplierId:occasional?.supplierId??"",conceptId:"",costCenterId:null,supplierDocumentNumber:"",issuedAt:localNoon(today),dueDate:localNoon(today),currencyCode:"COP",description:"",taxExclusiveAmount:0,vatAmount:0,withholdingJurisdictionCode:"CO",evidenceUrl:null});
  ...
  return <form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}>
    <Field label="Proveedor o beneficiario"><Select value={form.supplierId} onValueChange={supplierId=>setForm({...form,supplierId})}><SelectTrigger><SelectValue placeholder="Gasto ocasional / sin proveedor"/></SelectTrigger><SelectContent>{options.suppliers.map(item=><SelectItem key={item.supplierId} value={item.supplierId}>{item.name}{item.identification!=="OCASIONAL"?` · ${item.identification}`:""}</SelectItem>)}</SelectContent></Select></Field>
