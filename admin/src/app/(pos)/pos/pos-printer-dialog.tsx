  ...
  return <div className="space-y-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
    ...
    {value.enabled && <>
      <Field label="Puerto de la balanza">
        <Select value={value.portName||undefined} onValueChange={portName=>onChange({...value,portName})}><SelectTrigger><SelectValue placeholder="Selecciona un puerto"/></SelectTrigger><SelectContent className="bg-white text-slate-950">{serialPorts.map(port=><SelectItem key={port} value={port}>{port}</SelectItem>)}</SelectContent></Select>
      </Field>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Bits de datos"><Select value={String(value.dataBits)} onValueChange={dataBits=>onChange({...value,dataBits:Number(dataBits)})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent className="bg-white text-slate-950">{[5,6,7,8].map(bits=><SelectItem key={bits} value={String(bits)}>{bits}</SelectItem>)}</SelectContent></Select></Field>
        <Field label="Paridad"><Select value={value.parity} onValueChange={parity=>onChange({...value,parity:parity as typeof value.parity})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent className="bg-white text-slate-950"><SelectItem value="None">Ninguna</SelectItem><SelectItem value="Odd">Impar</SelectItem><SelectItem value="Even">Par</SelectItem><SelectItem value="Mark">Marca</SelectItem><SelectItem value="Space">Espacio</SelectItem></SelectContent></Select></Field>
        <Field label="Bits de parada"><Select value={value.stopBits} onValueChange={stopBits=>onChange({...value,stopBits:stopBits as typeof value.stopBits})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent className="bg-white text-slate-950"><SelectItem value="One">1</SelectItem><SelectItem value="OnePointFive">1,5</SelectItem><SelectItem value="Two">2</SelectItem></SelectContent></Select></Field>
