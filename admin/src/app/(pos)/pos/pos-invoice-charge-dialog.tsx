...
export function PosInvoiceChargeDialog({ client, draft, onUpdated, onClose }: {
  ...
  const [showCatalog, setShowCatalog] = useState((draft.charges?.length ?? 0) === 0);
  ...
  function selectCharge(value: InvoiceCharge) {
    ...
    setSupplierId(active.length === 1 ? active[0].supplierId : "");
    setManualAmount(String(value.value ?? ""));
  }
  async function save(event: React.FormEvent) {
    event.preventDefault();
    if (!selected || busy) return;
    const input: AddInvoiceCharge = {
      appliedChargeId: crypto.randomUUID(),
    ...
    try {
      onUpdated(await client.saveCharge(draft.draftId.value, input));
      setSelected(null);
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4">
    <div ref={modal} role="dialog" aria-modal="true" aria-labelledby="invoice-charge-title" tabIndex={-1}
      ...
      {(draft.charges?.length ?? 0) > 0 && <section aria-label="Cargos agregados" className="mb-5 space-y-2"><div className="flex items-center justify-between gap-3"><h3 className="font-semibold">Cargos agregados</h3><Button type="button" size="sm" disabled={busy || (draft.charges?.length ?? 0) >= 10} onClick={() => { setSelected(null); setShowCatalog(true); }}><Plus className="mr-2 h-4 w-4"/>Agregar cargo</Button></div>{draft.charges?.map(charge => <div key={charge.appliedChargeId} className="flex items-center gap-3 rounded-xl border border-teal-200 bg-teal-50 p-3"><div className="min-w-0 flex-1"><b>{charge.name}</b><p className="text-xs text-slate-600">{charge.supplier.name}</p><p className="mt-1 text-xs font-medium text-teal-800">{charge.invoicedAmount > 0 ? "Incluido en factura" : "Gasto · no incrementa el cobro"}</p></div><strong className="whitespace-nowrap">{money.format(charge.amount)}</strong><Button variant="ghost" size="icon" disabled={busy} onClick={() => void remove(charge.appliedChargeId)} aria-label={`Quitar ${charge.name}`}><Trash2 className="h-4 w-4"/></Button></div>)}</section>}
      ...
      </form> : showCatalog && (query.isPending ? <p role="status" className="py-8 text-center">Cargando cargos…</p> : query.isError ? <div role="alert"><p>No fue posible consultar los cargos.</p><Button variant="outline" onClick={() => void query.refetch()}>Reintentar</Button></div> : <section className="space-y-2" aria-label="Cargos disponibles">
        {query.data.items.filter(charge => charge.isActive).map(charge => <button type="button" key={charge.chargeId} disabled={busy || (draft.charges?.length ?? 0) >= 10} onClick={() => selectCharge(charge)} className="flex w-full items-center justify-between gap-3 rounded-xl border border-slate-200 px-4 py-3 text-left transition hover:border-teal-500 hover:bg-teal-50 disabled:opacity-40"><span><b>{charge.name}</b><span className="block text-xs text-slate-500">{charge.code}</span></span><span className="text-sm font-semibold text-teal-700">Agregar</span></button>)}
