...
export function useDeletePurchaseOrderDraft(){const client=useQueryClient();const businessId=useBusinessContextStore(s=>s.selectedBusinessId);return useMutation({mutationFn:({id,token}:{id:string;token:string})=>purchaseOrdersApi.deleteDraft(id,token),onSuccess:()=>client.invalidateQueries({queryKey:["purchase-orders",businessId]})});}
