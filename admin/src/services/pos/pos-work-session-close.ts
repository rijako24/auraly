...
export function workSessionClosureHtml(value: ClosureForPrint): string {
  ...
  const paymentRows = value.paymentTotals.map(item => {
    ...
    row("Entradas / salidas", item.otherAmount ?? 0) +
