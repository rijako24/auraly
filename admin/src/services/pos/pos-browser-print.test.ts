...
test("browser printing reports a rejected print call", async () => {
  const originalDocument = globalThis.document;
  const originalWindow = globalThis.window;
  const timers: Array<() => void> = [];
  const printWindow = {
  ...
  Object.defineProperty(globalThis, "window", {
    configurable: true,
    value: { setTimeout: (callback: () => void) => {
      timers.push(callback);
      return timers.length;
    } },
  });
  ...
  try {
    const dispatched = printPosHtmlDocument("<p>Recibo</p>", "No se pudo imprimir");
    timers.shift()?.();
    timers.shift()?.();
    await assert.rejects(dispatched, /No se pudo imprimir/);
  } finally {
    Object.defineProperty(globalThis, "document", { configurable: true, value: originalDocument });
    Object.defineProperty(globalThis, "window", { configurable: true, value: originalWindow });
  }
