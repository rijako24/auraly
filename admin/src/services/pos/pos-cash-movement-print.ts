...
@page{size:80mm auto;margin:4mm}*{box-sizing:border-box}body{width:72mm;margin:0 auto;color:#111;font:12px/1.4 ui-monospace,Consolas,monospace}header{text-align:center;border-bottom:1px dashed #555;padding-bottom:8px}h1{margin:0;font:800 19px/1.2 Arial,sans-serif}h2{margin:5px 0 0;font-size:13px}.details{font-size:13px;padding-top:8px}.row{display:flex;justify-content:space-between;gap:10px;margin:6px 0}.row span:last-child{text-align:right;overflow-wrap:anywhere}.amount{margin:12px 0 0;border:2px solid #111;padding:9px;text-align:center;font-size:18px;font-weight:800}.signature{margin-top:34px;border-top:1px solid #111;padding-top:4px;text-align:center}.id{margin-top:10px;text-align:center;font-size:9px;color:#555}</style></head><body><header><h1>${escapeHtml(companyName || "Empresa")}</h1><h2>${title}</h2></header><section class="details">${row("Motivo", ticket.reasonName)}${optional}${row("Responsable", ticket.responsibleName)}${row("Fecha y hora", new Date(ticket.occurredAt).toLocaleString("es-CO", { dateStyle: "short", timeStyle: "short" }))}</section><div class="amount">${money(ticket.amount)}</div><div class="signature">Firma</div><div class="id">${escapeHtml(ticket.documentId)}</div><script>addEventListener('load',()=>setTimeout(()=>window.print(),150));</script></body></html>`;
}
