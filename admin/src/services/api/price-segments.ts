...
export type PriceChannelStrategy = "PercentageOverBasePrice"|"PercentageOverAverageCost"|"FixedMarginOverAverageCost"|"SellAtAverageCost"|"FixedSpecialPrice";
export interface PriceSegmentSummary { id:string; kind:PriceSegmentKind; code:string; name:string; isActive:boolean; createdAt:string; productCount:number; customerCount:number; strategy:PriceChannelStrategy|null; value:number|null; }
...
export const priceSegmentsApi = {
  ...
  create: (data:{kind:PriceSegmentKind;name:string;channelStrategy?:PriceChannelStrategy;channelValue?:number|null;items?:Array<{productId:string;amount:number;minimumQuantity:number;validFrom:string|null;validUntil:string|null}>}) => apiClient.post<PriceSegmentSummary>("/commerce/v1/pricing/segments",data),
  ...
  saveChannelSettings: (id:string,channelStrategy:PriceChannelStrategy,channelValue:number|null) => apiClient.put<void>(`/commerce/v1/pricing/segments/PriceChannel/${id}/settings`,{channelStrategy,channelValue}),
};
