...
export interface PriceSegmentItem { productId:string; productCode:string; productName:string; amount:number; currencyCode:string; minimumQuantity:number; excluded:boolean; }
...
export const priceSegmentsApi = {
  ...
  saveItem: (id:string,productId:string,data:{amount:number;minimumQuantity:number;excluded:boolean}) => apiClient.put<void>(`/commerce/v1/pricing/segments/${id}/items/${productId}`,data),
