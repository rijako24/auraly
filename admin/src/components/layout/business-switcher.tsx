...
export function BusinessSwitcher() {
  ...
  return <>
    <Sheet open={mobileOpen} onOpenChange={setMobileOpen}><SheetTrigger asChild><button type="button" className="flex min-w-0 flex-1 items-center gap-2 rounded-2xl px-1 py-1 text-left sm:hidden"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-slate-950 to-teal-700 text-white shadow-sm"><Building2 className="h-4 w-4"/></span><span className="min-w-0 flex-1"><small className="block text-[10px] font-bold uppercase tracking-wider text-teal-700">Negocio</small><strong className="block truncate text-sm">{selected?.name??"Seleccionar"}</strong></span><ChevronDown className="h-4 w-4 shrink-0 text-muted-foreground"/></button></SheetTrigger><SheetContent side="bottom" onOpenAutoFocus={(event)=>event.preventDefault()} className="flex max-h-[calc(100dvh-env(safe-area-inset-top)-0.75rem)] flex-col overflow-hidden rounded-t-[2rem] px-4 pb-[max(1rem,env(safe-area-inset-bottom))]"><SheetHeader className="shrink-0 text-left"><SheetTitle>Cambiar negocio</SheetTitle><SheetDescription>Selecciona el espacio en el que vas a trabajar.</SheetDescription></SheetHeader><label className="relative my-4 block shrink-0"><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-muted-foreground"/><Input value={search} onChange={(event)=>setSearch(event.target.value)} className="h-11 rounded-xl pl-9" inputMode="search" placeholder="Toca para buscar un negocio"/></label><div className="min-h-0 flex-1 overflow-y-auto overscroll-contain pb-2">{list}</div></SheetContent></Sheet>
