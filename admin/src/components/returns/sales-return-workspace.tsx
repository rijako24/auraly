...
function SalesReturnEditor({ sale, open, canConfirm, onCashRefundConfirmed, onClose }: { sale?: ReturnableSale; open: boolean; canConfirm: boolean; onCashRefundConfirmed?: () => void | Promise<void>; onClose: () => void }) {
  ...
  return <Dialog open={open} onOpenChange={(value) => { if (!value) onClose(); }}><DialogContent className="max-h-[94dvh] max-w-6xl overflow-y-auto"><DialogHeader><DialogTitle className="flex items-center gap-2"><RotateCcw className="h-5 w-5 text-primary" /> Nueva devolución</DialogTitle><DialogDescription>{sale.documentNumber} · DIAN {sale.fiscalNumber} · {sale.customerName} · {sale.warehouseName}</DialogDescription></DialogHeader>
    ...
    <div className="grid gap-4 lg:grid-cols-3"><div className="space-y-2"><Label>Tipo de devolución</Label><Select value={resolutionMethod} onValueChange={(value) => { setResolutionMethod(value); setPaymentNumber(""); }}><SelectTrigger><SelectValue placeholder="Selecciona" /></SelectTrigger><SelectContent>{availableMethods.map(method => <SelectItem key={method.id} value={method.code}>{method.label}</SelectItem>)}</SelectContent></Select>{resolutionMethod === "CustomerCredit" && <p className="text-xs text-muted-foreground">Saldo de cartera disponible: {formatCurrency(sale.receivableOutstanding)}</p>}</div>{economicResolution === "Refund" && <div className="space-y-2"><Label>Pago original</Label><Select value={paymentNumber} onValueChange={setPaymentNumber}><SelectTrigger><SelectValue placeholder="Selecciona el pago del mismo medio" /></SelectTrigger><SelectContent>{paymentsForMethod.map((payment) => <SelectItem key={payment.paymentNumber} value={String(payment.paymentNumber)}>Pago {payment.paymentNumber} · disponible {formatCurrency(payment.availableAmount)}</SelectItem>)}</SelectContent></Select></div>}<div className="space-y-2"><Label>Notas internas</Label><Textarea maxLength={1000} value={notes} onChange={(event) => setNotes(event.target.value)} /></div></div>
