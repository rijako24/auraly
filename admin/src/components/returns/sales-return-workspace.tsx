      ...
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-5">
        ...
        <div className="overflow-auto rounded-2xl border" style={{ maxHeight: `${PRODUCT_GRID_HEADER_HEIGHT_REM + DEFAULT_VISIBLE_PRODUCT_LINES * PRODUCT_GRID_LINE_HEIGHT_REM}rem` }}>
          <div className="min-w-[980px]">
            <div className="sticky top-0 z-10 grid grid-cols-[minmax(18rem,1fr)_6rem_7rem_9rem_8rem_10rem] gap-3 bg-muted px-4 py-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground"><span>Producto</span><span>Vendido</span><span>Disponible</span><span className="text-right">Precio comprado</span><span>Devolver</span><span className="text-right">Total</span></div>
            {visibleLines.map((line) => <div key={line.originalLineNumber} className="grid min-h-[4.25rem] grid-cols-[minmax(18rem,1fr)_6rem_7rem_9rem_8rem_10rem] items-center gap-3 border-t px-4 py-3"><div><p className="font-medium">{line.description}</p><p className="text-xs text-muted-foreground">{line.productCode}{line.reference ? ` · ${line.reference}` : ""} · IVA {line.taxRate}%</p></div><span>{line.soldQuantity}</span><span className="font-semibold text-primary">{line.availableQuantity}</span><span className="text-right font-medium tabular-nums">{formatCurrency(salesReturnPurchasedUnitPrice(line))}</span><Input aria-label={`Cantidad a devolver de ${line.description}`} type="number" min={0} max={line.availableQuantity} step="0.000001" disabled={returnScopeCode === "FullCancellation" || line.availableQuantity <= 0} value={quantities[line.originalLineNumber] ?? ""} onChange={(event) => setQuantities((current) => ({ ...current, [line.originalLineNumber]: Math.max(0, Number(event.target.value)) }))} /><span className="text-right font-semibold tabular-nums">{formatCurrency(selection.lineTotals[line.originalLineNumber] ?? 0)}</span></div>)}
