...
const reasons = [
  ["CustomerChangedMind", "El cliente cambiÃ³ de opiniÃ³n"],
  ["WrongProduct", "Producto equivocado"],
  ["QualityIssue", "Problema de calidad"],
  ["BillingCorrection", "CorrecciÃ³n de la venta"],
  ["Other", "Otro motivo"],
] as const;
...
export function SalesReturnWorkspace({ embedded = false }: { embedded?: boolean }) {
  ...
  return <div className={embedded ? "space-y-4" : "space-y-6"}>
    ...
    {!embedded && <section className="grid gap-3 md:grid-cols-3"><Summary icon={ReceiptText} label="Facturas encontradas" value={String(list.data?.totalCount ?? 0)} /><Summary icon={PackageCheck} label="Inventario" value="Reingreso controlado" /><Summary icon={HandCoins} label="ResoluciÃ³n" value="Efectivo o saldo cliente" /></section>}
    <section className="flex flex-col gap-3 rounded-2xl border bg-card p-4 md:flex-row">
      <div className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input className="pl-9" value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="NÃºmero Auraly, nÃºmero DIAN, CUFE, cliente o producto" /></div>
...
function SalesReturnEditor({ sale, open, canConfirm, onClose }: { sale?: ReturnableSale; open: boolean; canConfirm: boolean; onClose: () => void }) {
  ...
  const submit = async () => {
    if (!reasonDescription.trim()) { toast.error("Describe brevemente el motivo de la devoluciÃ³n."); return; }
    ...
    if (resolution === "Refund") {
      ...
      catch { toast.error("No fue posible abrir la sesiÃ³n del usuario en la bodega de la venta."); return; }
    }
    try {
      ...
      toast.success(`DevoluciÃ³n ${result.documentNumber} aceptada por el motor documental.`);
      onClose();
    } catch { toast.error("No fue posible confirmar. La disponibilidad o el pago original pudieron cambiar."); }
  };

  return <Dialog open={open} onOpenChange={(value) => { if (!value) onClose(); }}><DialogContent className="max-h-[94dvh] max-w-6xl overflow-y-auto"><DialogHeader><DialogTitle className="flex items-center gap-2"><RotateCcw className="h-5 w-5 text-primary" /> Nueva devoluciÃ³n</DialogTitle><DialogDescription>{sale.documentNumber} Â· DIAN {sale.fiscalNumber} Â· {sale.customerName} Â· {sale.warehouseName}</DialogDescription></DialogHeader>
    <div className="overflow-hidden rounded-2xl border"><div className="grid grid-cols-[minmax(0,1fr)_7rem_7rem_8rem_12rem] gap-3 bg-muted/60 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground"><span>Producto</span><span>Vendido</span><span>Disponible</span><span>Devolver</span><span>Destino</span></div>{sale.lines.map((line) => <div key={line.originalLineNumber} className="grid grid-cols-[minmax(0,1fr)_7rem_7rem_8rem_12rem] items-center gap-3 border-t px-4 py-3"><div><p className="font-medium">{line.description}</p><p className="text-xs text-muted-foreground">{line.productCode}{line.reference ? ` Â· ${line.reference}` : ""} Â· {formatCurrency(line.unitPrice)} Â· IVA {line.taxRate}%</p></div><span>{line.soldQuantity}</span><span className="font-semibold text-primary">{line.availableQuantity}</span><Input type="number" min={0} max={line.availableQuantity} step="0.000001" disabled={line.availableQuantity <= 0} value={quantities[line.originalLineNumber] ?? ""} onChange={(event) => setQuantities((current) => ({ ...current, [line.originalLineNumber]: Math.max(0, Number(event.target.value)) }))} /><Select value={dispositions[line.originalLineNumber] ?? "Sellable"} onValueChange={(value) => setDispositions((current) => ({ ...current, [line.originalLineNumber]: value as SalesReturnDisposition }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Sellable">Vuelve a inventario</SelectItem><SelectItem value="NotReturned">Sin retorno fÃ­sico</SelectItem></SelectContent></Select></div>)}</div>
    <div className="grid gap-4 lg:grid-cols-[16rem_minmax(0,1fr)_18rem]"><div className="space-y-2"><Label>Motivo</Label><Select value={reasonCode} onValueChange={setReasonCode}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{reasons.map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>DescripciÃ³n del motivo</Label><Input maxLength={300} value={reasonDescription} onChange={(event) => setReasonDescription(event.target.value)} placeholder="QuÃ© ocurriÃ³ y por quÃ© se devuelve" /></div><Card className="border-primary/20 bg-primary/5"><CardContent className="p-4"><p className="text-xs uppercase tracking-wide text-muted-foreground">Valor estimado</p><p className="text-2xl font-semibold">{formatCurrency(estimated)}</p><p className="text-xs text-muted-foreground">El servidor conserva el redondeo original.</p></CardContent></Card></div>
    <div className="grid gap-4 lg:grid-cols-3"><div className="space-y-2"><Label>ResoluciÃ³n econÃ³mica</Label><Select value={resolution} onValueChange={(value) => setResolution(value as SalesReturnResolution)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{sale.customerId && <SelectItem value="CustomerCredit">Aplicar a cartera / saldo a favor</SelectItem>}<SelectItem value="Refund">Devolver efectivo</SelectItem></SelectContent></Select></div>{resolution === "Refund" && <div className="space-y-2"><Label>Pago original en efectivo</Label><Select value={paymentNumber} onValueChange={setPaymentNumber}><SelectTrigger><SelectValue placeholder="Selecciona el pago" /></SelectTrigger><SelectContent>{cashPayments.map((payment) => <SelectItem key={payment.paymentNumber} value={String(payment.paymentNumber)}>Pago {payment.paymentNumber} Â· disponible {formatCurrency(payment.availableAmount)}</SelectItem>)}</SelectContent></Select></div>}<div className="space-y-2"><Label>Notas internas</Label><Textarea maxLength={1000} value={notes} onChange={(event) => setNotes(event.target.value)} /></div></div>
    <DialogFooter><Button variant="outline" onClick={onClose}>Cancelar</Button><Button disabled={!canConfirm || chosen.length === 0 || confirm.isPending} onClick={submit}><RotateCcw className="mr-2 h-4 w-4" /> {confirm.isPending ? "Confirmando..." : "Confirmar devoluciÃ³n"}</Button></DialogFooter>
