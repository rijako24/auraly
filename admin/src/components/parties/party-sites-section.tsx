...
export function PartySitesSection({ detail, editing = false, drafts, onChange }: { detail: PartyWorkspaceDetail; editing?: boolean; drafts?: PartySiteDraft[]; onChange?: (sites: PartySiteDraft[]) => void }) {
  ...
  return <section className="rounded-2xl border p-5">
    ...
    {!sites.length ? <p className="mt-4 rounded-xl border border-dashed p-5 text-sm text-muted-foreground">No tiene sedes. Agrega al menos una antes de guardar.</p> : <div className="mt-4 grid items-start gap-4 md:grid-cols-2">{sites.map(site => <SiteCard key={site.clientKey} site={site} canEdit={canManage && !target} onEdit={() => setTarget({ ...site })} onRemove={() => remove(site)}/>)}</div>}
    {target && <InlineSiteEditor key={target.clientKey} value={target} onApply={apply} onClose={() => setTarget(null)}/>}
  </section>;
}
...
function InlineSiteEditor({ value, onApply, onClose }: { value: PartySiteDraft; onApply: (site: PartySiteDraft) => void; onClose: () => void }) {
  ...
  </div><div className="mt-5 flex justify-end gap-2"><Button type="button" variant="outline" onClick={onClose}>Cancelar</Button><Button type="button" onClick={apply}>Aplicar sede</Button></div></div>;
}
