...
export function PartySitesSection({ detail, editing = false, drafts, onChange }: { detail: PartyWorkspaceDetail; editing?: boolean; drafts?: PartySiteDraft[]; onChange?: (sites: PartySiteDraft[]) => void }) {
  ...
  return <section className="rounded-2xl border p-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold">Sedes y puntos de entrega</h3><p className="text-sm text-muted-foreground">Agrega, edita o quita tarjetas. Los cambios se aplican únicamente al pulsar Guardar tercero.</p></div>{canManage && !target && <Button type="button" variant="outline" onClick={add}><Plus className="mr-2 h-4 w-4"/>Agregar sede</Button>}</div>
    {!sites.length && !target ? <p className="mt-4 rounded-xl border border-dashed p-5 text-sm text-muted-foreground">No tiene sedes. Agrega al menos una antes de guardar.</p> : <div className="mt-4 grid items-start gap-4 md:grid-cols-2">{sites.map(site => target?.clientKey===site.clientKey?<InlineSiteEditor key={site.clientKey} value={target} onApply={apply} onClose={()=>setTarget(null)}/>:<SiteCard key={site.clientKey} site={site} canEdit={canManage && !target} onEdit={() => setTarget({ ...site })} onRemove={() => remove(site)}/>)}{target&&!sites.some(site=>site.clientKey===target.clientKey)&&<InlineSiteEditor key={target.clientKey} value={target} onApply={apply} onClose={()=>setTarget(null)}/>}</div>}
  </section>;
}
...
function InlineSiteEditor({ value, onApply, onClose }: { value: PartySiteDraft; onApply: (site: PartySiteDraft) => void; onClose: () => void }) {
  ...
  return <article className="rounded-2xl border-2 border-teal-200 bg-teal-50/30 p-5 shadow-sm md:col-span-2"><div className="mb-4 flex items-start justify-between gap-3"><div><h4 className="font-semibold">{value.partySiteId ? `Editar ${value.name}` : "Agregar sede"}</h4><p className="text-sm text-muted-foreground">Este cambio quedará pendiente hasta guardar el tercero.</p></div><Button type="button" size="icon" variant="ghost" onClick={onClose} aria-label="Cerrar editor de sede"><X className="h-4 w-4"/></Button></div><div className="grid gap-4 md:grid-cols-2">
  ...
  </div><div className="mt-5 flex justify-end gap-2"><Button type="button" variant="outline" onClick={onClose}>Cancelar</Button><Button type="button" onClick={apply}>{value.partySiteId?"Actualizar sede":"Agregar sede"}</Button></div></article>;
}
