...
export function CustomerMapPanel({ onOpenCustomer }: { onOpenCustomer: (partyId: string) => void }) {
  ...
  const sellers = useMemo(() => unique(map.data?.flatMap((site) => site.assignments.map((value) => ({ id: value.sellerId, name: value.sellerName }))) ?? []), [map.data]);
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-3 rounded-2xl border bg-card p-4 md:grid-cols-3">
      <Select value={sellerId} onValueChange={(value) => { setSellerId(value); setRouteId("all"); }}><SelectTrigger><SelectValue placeholder="Vendedor"/></SelectTrigger><SelectContent><SelectItem value="all">Todos los vendedores</SelectItem>{sellers.map((item) => <SelectItem key={item.id} value={item.id}>{item.name}</SelectItem>)}</SelectContent></Select>
