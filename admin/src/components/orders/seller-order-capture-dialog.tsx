...
export function SellerOrderCaptureDialog({ businessId, warehouseId, route, stop, editing, localFirst = false, allowsNegativeStockSales = false, onClose, onCreated }: { businessId: string; warehouseId: string; route: SalesRouteDetail | null; stop: SalesRouteStop; editing?: CommerceOrderDetail|null; localFirst?: boolean; allowsNegativeStockSales?: boolean; onClose: () => void; onCreated: (orderId: string) => Promise<void> }) {
  ...
  useEffect(() => {
    if (editing) return;
    let active = true;
    void Promise.all([loadSellerCatalog(businessId, warehouseId, stop.customerId), loadSellerDraft(key)]).then(([catalog, draft]) => {
      if (!active) return;
      const known=Object.fromEntries(catalog.map((item) => [item.productId, item]));
  ...
  const search = async (reset = true) => {
    ...
    if (!reset && (!hasMoreResults || loading || loadingMoreRef.current)) return;
    ...
    try {
      if (localFirst) {
        ...
        const filtered = cached.filter((item) => `${item.productCode} ${item.reference ?? ""} ${item.name}`.toLocaleLowerCase("es").includes(normalized));
        const page = filtered.slice(offset, offset + 10);
        if (searchRef.current?.value.trim() !== term) return;
        setResults((current) => reset ? page : [...current, ...page]);
        setNextOffset(offset + page.length);
        setHasMoreResults(offset + page.length < filtered.length);
        setKnownItems((current) => ({ ...current, ...Object.fromEntries(cached.map((item) => [item.productId, item])) }));
        if (reset && !page.length) toast.info("No encontramos coincidencias en el catálogo guardado.");
        return;
      }
      const page = await sellerOrdersApi.catalog({ businessId, warehouseId, customerId: stop.customerId, search: term, skip: offset, take: 10 });
      if (searchRef.current?.value.trim() !== term) return;
      ...
      setResults((current) => reset ? visibleItems : [...current, ...visibleItems]);
      setNextOffset(page.nextOffset ?? offset + visibleItems.length);
  ...
  return <><Dialog open onOpenChange={(open) => !open && onClose()}><DialogContent className="flex h-[100dvh] max-h-[100dvh] w-screen max-w-none flex-col gap-0 overflow-hidden rounded-none p-0 sm:h-[min(86dvh,720px)] sm:w-[calc(100vw-3rem)] sm:max-w-3xl sm:rounded-[2rem] sm:border-slate-200">
    <DialogHeader className="shrink-0 border-b bg-background px-4 pb-4 pt-5 text-left sm:px-6"><div className="flex items-start gap-3"><Button type="button" size="icon" variant="ghost" className="-ml-2 shrink-0" onClick={onClose}><ArrowLeft className="h-5 w-5"/></Button><div className="min-w-0 flex-1"><DialogTitle className="truncate">{editing?`Editar ${editing.orderNumber}`:stop.customerName}</DialogTitle><DialogDescription className="line-clamp-1">{stop.siteName} · {stop.addressLine}</DialogDescription></div><Badge className="rounded-full bg-teal-600">{selected.length}</Badge></div><div className="relative mt-4"><Search className="pointer-events-none absolute left-3 top-3.5 h-5 w-5 text-teal-700"/><Input ref={searchRef} enterKeyHint="search" autoComplete="off" className="h-12 rounded-2xl border-teal-300 bg-white pl-10 pr-12 text-base" value={query} onChange={(event)=>{setQuery(event.target.value);setResults([]);setSearched(false);setHasMoreResults(false);setNextOffset(0)}} onKeyDown={(event)=>{if(event.key==="Enter"){event.preventDefault();void search(true)}}} placeholder="Buscar por nombre, código o referencia"/>{query&&<Button type="button" size="icon" variant="ghost" className="absolute right-1.5 top-1.5 h-9 w-9 rounded-xl" onClick={()=>{setQuery("");setResults([]);setSearched(false);setHasMoreResults(false);setNextOffset(0);searchRef.current?.focus()}}><X className="h-4 w-4"/></Button>}</div></DialogHeader>
    <main className="min-h-0 flex-1 overflow-y-auto bg-slate-50/70 px-3 py-4 sm:px-6" onScroll={(event)=>{const target=event.currentTarget;if(target.scrollHeight-target.scrollTop-target.clientHeight<120&&hasMoreResults&&!loadingMoreRef.current)void search(false)}}>
      ...
      {query.trim().length>=2||searched||loading?<SearchResults searching={loading} loadingMore={loadingMore} searched={searched} results={results} quantities={quantities} online={online} allowsNegativeStockSales={allowsNegativeStockSales} originalLinesByProduct={originalLinesByProduct} onChange={addFromSearch} onEdit={setQuantityItem}/>:<Cart items={selected} online={online} allowsNegativeStockSales={allowsNegativeStockSales} originalLinesByProduct={originalLinesByProduct} onChange={change} onEdit={setQuantityItem} onSearch={()=>searchRef.current?.focus()}/>}
    </main>
    <footer className="shrink-0 border-t bg-background/95 px-3 pb-[max(.75rem,env(safe-area-inset-bottom))] pt-3 shadow-[0_-12px_35px_-24px_rgba(15,118,110,.7)] backdrop-blur sm:px-6"><div className="mb-3 flex items-center justify-between gap-3"><button type="button" onClick={()=>{setQuery("");setResults([]);setSearched(false);setHasMoreResults(false);setNextOffset(0)}} className="text-left"><small className="block text-muted-foreground">{selected.length} productos · {units.toLocaleString("es-CO", { maximumFractionDigits: 3 })} unidades</small><strong className="text-xl">{money.format(total)}</strong></button>{(query||searched)&&<Button type="button" variant="outline" onClick={()=>{setQuery("");setResults([]);setSearched(false);setHasMoreResults(false);setNextOffset(0)}}><ShoppingBag className="mr-2 h-4 w-4"/>Ver pedido</Button>}</div><Input value={notes} onChange={(event) => setNotes(event.target.value)} className="mb-3" placeholder="Nota para este pedido (opcional)"/><Button className="h-12 w-full bg-teal-600 text-base font-bold hover:bg-teal-700" disabled={sellerOrderSubmitDisabled(saving, selected.length, Boolean(editing), online, invalidQuantity)} onClick={submit}><PackagePlus className="mr-2 h-5 w-5"/>{saving ? "Guardando…" : editing?`Actualizar pedido · ${money.format(total)}`:online ? `Crear pedido · ${money.format(total)}` : "Guardar para sincronizar"}</Button></footer>
...
function SearchResults({ searching, loadingMore, searched, results, quantities, online, allowsNegativeStockSales, originalLinesByProduct, onChange, onEdit }: { searching: boolean; loadingMore: boolean; searched: boolean; results: SellerCatalogItem[]; quantities: Record<string, number>; online: boolean; allowsNegativeStockSales: boolean; originalLinesByProduct: ReadonlyMap<string, CommerceOrderDetail["lines"][number]>; onChange: (item: SellerCatalogItem, delta: number) => void; onEdit: (item: SellerCatalogItem) => void }) {
  return <div className="space-y-3">{searching&&<p className="rounded-3xl border bg-white p-8 text-center text-sm text-muted-foreground">Buscando productos…</p>}{searched&&!searching&&!results.length&&<p className="rounded-3xl border border-dashed bg-white p-10 text-center text-sm text-muted-foreground">No encontramos productos con esa búsqueda.</p>}{results.map((item)=><ProductRow key={item.productId} item={item} quantity={quantities[item.productId]??0} online={online} maximumQuantity={sellerOrderMaximumQuantity(item.manageStock, item.quantityOnHand, originalLinesByProduct.get(item.productId)?.quantity ?? null, allowsNegativeStockSales)} selected={(quantities[item.productId]??0)>0} onChange={onChange} onEdit={onEdit}/>)}{loadingMore&&<p className="py-3 text-center text-sm text-muted-foreground">Cargando 10 más…</p>}</div>;
}
