...
function StatusBadge({ status }: { status: string }) { const label = status === "Available" ? "Listo para facturar" : status === "Invoiced" ? "Facturado" : status === "ProcessingEmission" ? "Procesando emisión" : status === "EmissionFailed" ? "Error de emisión" : status === "InReview" || status === "Pending" ? "Pendiente de inventario" : status; const attention=status === "InReview"||status === "Pending"||status === "ProcessingEmission"||status === "EmissionFailed"; return <Badge variant="outline" className={attention ? "border-amber-200 bg-amber-50 text-amber-800" : "border-emerald-200 bg-emerald-50 text-emerald-800"}>{label}</Badge>; }
