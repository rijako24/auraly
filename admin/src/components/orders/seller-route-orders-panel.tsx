...
import { ClipboardList, CloudOff, Loader2, PackageCheck, Pencil, RefreshCw } from "lucide-react";
...
export function SellerRouteOrdersPanel({ route, warehouseId, operationalDate, revision, localFirst = false, allowsNegativeStockSales = false }: { route: SalesRouteDetail; warehouseId: string; operationalDate: string; revision: number; localFirst?: boolean; allowsNegativeStockSales?: boolean }) {
...
function snapshotItem(value:Awaited<ReturnType<typeof loadSellerOrderSnapshots>>[number]):CommerceOrderListItem{return{orderId:value.orderId,orderNumber:value.orderNumber,status:value.synchronized?value.status:"LocalPending",source:1,customerId:value.customerId,partySiteId:value.partySiteId,partySiteName:value.partySiteName,customerName:value.customerName,customerIdentification:null,customerPhone:null,currency:"COP",total:value.total,lineCount:value.lineCount,createdAt:value.createdAt,canInvoice:false,invoiceDocumentId:null,claim:null}}
