...
export function OutsideRouteCustomerDialog({ routeId, onClose, onSelect }: { routeId: string; onClose: () => void; onSelect: (stop: SalesRouteStop) => void }) {
  ...
  const search = async () => {
    ...
    try {
      const page = await routesApi.candidates(routeId, { page: 1, pageSize: 50, search: term });
