...
export function PortfolioPaymentWizard({direction,open,onOpenChange,initialParty,initialInvoice,workSessionId,onCompleted,edgeClient,businessId:businessIdOverride,printOnPos=false,businessName:posBusinessName}:{direction:Direction;open:boolean;onOpenChange:(open:boolean)=>void;initialParty?:PartyRoleSelection|null;initialInvoice?:Invoice|null;workSessionId?:string|null;onCompleted?:()=>void;edgeClient?:PosEdgeClient|null;businessId?:string|null;printOnPos?:boolean;businessName?:string}){
  ...
  const invoiceKey=open&&partyId?JSON.stringify([edgeClient?"edge":printOnPos?"web-pos":"web",direction,businessId,partyId,invoicePage]):null;
  ...
  useEffect(()=>{
    ...
    const load=async():Promise<InvoicePage>=>{
      ...
      if(direction==="receivable"){const value=edgeClient?await edgeClient.portfolioReceivables(partyId,invoicePage,20):printOnPos?await receivablesApi.posList({page:invoicePage,pageSize:20,customerId:partyId,outstandingOnly:true}):await receivablesApi.list({page:invoicePage,pageSize:20,customerId:partyId,outstandingOnly:true});page={totalCount:value.totalCount,totalPages:value.totalPages,items:value.items.map<Invoice>(x=>({id:x.receivableId,number:x.documentNumber,dueDate:x.dueDate,outstanding:x.outstandingAmount,currency:x.currencyCode,overdue:x.isOverdue}))};}
      else{const value=edgeClient?await edgeClient.portfolioPayables(partyId,invoicePage,20):printOnPos?await payablesApi.posList({page:invoicePage,pageSize:20,supplierId:partyId,outstandingOnly:true}):await payablesApi.list({page:invoicePage,pageSize:20,supplierId:partyId,outstandingOnly:true});page={totalCount:value.totalCount,totalPages:value.totalPages,items:value.items.map<Invoice>(x=>({id:x.payableId,number:x.documentNumber,dueDate:x.dueDate,outstanding:x.outstandingAmount,currency:x.currencyCode,overdue:x.isOverdue}))};}
      if(!edgeClient&&!printOnPos&&invoicePage===1&&requested&&initialPartyId===partyId&&!page.items.some(item=>item.id===requested.id)){
        if(direction==="receivable"){
    ...
    return()=>{active=false};
  },[invoiceKey,partyId,invoicePage,direction,edgeClient,printOnPos]);
  ...
  const mutation=useMutation({mutationFn:async(tenders:PosPaymentInput[])=>{
    ...
    const accepted=await(edgeClient?edgeClient.confirmPortfolioPayable(request,`payable-payment-${paymentId}`):printOnPos?payablesApi.confirmPosPayment(request,`payable-payment-${paymentId}`):payablesApi.confirmPayment(request,`payable-payment-${paymentId}`));
    return {accepted,printContext};
  ...
  return <Dialog open={open} onOpenChange={next=>{if(!next&&mutation.isPending)return;changeOpen(next)}}><DialogContent className="flex max-h-[92dvh] w-[96vw] max-w-4xl flex-col overflow-hidden p-0">
    ...
    <div className="min-h-0 overflow-y-auto p-6"><div className="space-y-5"><div className="space-y-2"><Label>{role==="Customer"?"Cliente":"Proveedor"}</Label><PartyRoleSelect role={role} value={partyId??""} sourceKey={edgeClient?"edge-portfolio":printOnPos?"web-pos-portfolio":"web-portfolio"} preload={open} loadPage={(search,page,pageSize)=>edgeClient?edgeClient.portfolioParties(role,search,page,pageSize):printOnPos?partiesApi.posPortfolioRoleOptions({role,search,page,pageSize}):partiesApi.portfolioRoleOptions({role,search,page,pageSize})} selectedOption={party?{value:partyId!,label:party.displayName,description:party.identification}:null} placeholder={`Buscar por nombre o identificación`} onChange={(_,value)=>{setParty(value??null);setSelected({});setSelectedLimits({});setSelectedNumbers({});setInvoicePage(1);}}/></div>
