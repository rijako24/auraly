...
export function GeographyMaster({ canManage }: { canManage: boolean }) {
  ...
  return <>
    <MasterHierarchyExplorer title="Ubicación geográfica" description="Países, departamentos y ciudades en un solo árbol. Busca en toda la estructura, expande lo necesario y administra cada registro en contexto." levels={levels} nodes={nodes} loading={query.isLoading} error={query.isError ? (query.error instanceof Error ? query.error.message : "No fue posible cargar la ubicación geográfica.") : null} onRetry={() => void query.refetch()} canManage={canManage} onCreate={openCreate} onEdit={openEdit} />
