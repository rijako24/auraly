...
const typeLabels:Record<string,string>={SalesInvoice:"Factura de venta",ServiceInvoice:"Factura de servicios",SalesReceipt:"Documento POS",SalesReturn:"Nota crédito de venta",SalesDebitNote:"Nota débito de venta",GoodsReceipt:"Recepción de compra",GoodsReceiptCostDocument:"Costo adicional de compra",Expense:"Gasto",PurchaseReturn:"Devolución de compra",PayablePayment:"Pago a proveedor",ReceivablePayment:"Recaudo de cliente",CashReceipt:"Recibo de caja",CashDisbursement:"Comprobante de egreso",AccountingAccountAdjustment:"Nota contable",AccountingManualVoucher:"Comprobante manual",AccountingOpeningBalance:"Saldo inicial",StockCount:"Conteo de inventario",InventoryAdjustment:"Ajuste de inventario",Damage:"Baja de inventario",ProductConversion:"Conversión de producto",WarehouseTransferReceipt:"Recepción de traslado"};
...
export function AccountingDocumentDialog({documentId,sourceLabel,onClose}:{documentId?:string;sourceLabel?:string;onClose:()=>void}){
  ...
  if(reportOpen&&value)return <Dialog open onOpenChange={open=>!open&&setReportOpen(false)}><DialogContent showClose={false} className="h-[96dvh] max-h-[96dvh] w-[98vw] max-w-[1500px] overflow-hidden p-2 sm:p-4"><ReportViewer
    ...
    description={`${typeLabels[value.sourceDocumentType]??value.sourceDocumentType} · ${value.description} · ${new Date(value.occurredAt).toLocaleDateString("es-CO")}`}
  ...
  return <Dialog open onOpenChange={open=>!open&&onClose()}><DialogContent className="accounting-print-root flex max-h-[94dvh] max-w-6xl flex-col overflow-hidden p-0">
    <DialogHeader className="border-b px-6 py-5"><DialogTitle className="flex items-center gap-2"><BookOpenCheck className="h-5 w-5 text-primary"/>Comprobante contable {value?.entryNumber??""}</DialogTitle><DialogDescription>{sourceLabel??(posting.data?typeLabels[posting.data.sourceDocumentType]??posting.data.sourceDocumentType:"Documento origen")} · trazabilidad del asiento persistido</DialogDescription></DialogHeader>
    <div className="overflow-y-auto p-6">
      ...
      {value&&<div className="space-y-5">
        {fiscal.data&&<div className="grid gap-3 rounded-2xl border border-cyan-200 bg-cyan-50 p-4 sm:grid-cols-3"><Datum label="Documento fiscal" value={fiscal.data.fiscalDocumentType}/><Datum label="Número DIAN" value={fiscal.data.dianNumber}/><Datum label="Estado DIAN" value={fiscal.data.status}/>{fiscal.data.uniqueCode&&<div className="sm:col-span-3 break-all font-mono text-[10px]"><b>{fiscal.data.uniqueCodeType}</b><br/>{fiscal.data.uniqueCode}</div>}</div>}
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Datum label="Comprobante" value={value.entryNumber}/><Datum label="Documento origen" value={typeLabels[value.sourceDocumentType]??value.sourceDocumentType}/><Datum label="Fecha del documento" value={new Date(value.occurredAt).toLocaleString("es-CO")}/><Datum label="Contabilizado" value={new Date(value.postedAt).toLocaleString("es-CO")}/></div>
...
function postingStatus(status:string){return status==="CommercialEffectsApplied"?"Efecto comercial sin asiento":status==="AccountingPendingConfiguration"?"Requiere configuración contable":status==="Pending"?"Pendiente de contabilización":status;}
