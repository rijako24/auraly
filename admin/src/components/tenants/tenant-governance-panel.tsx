...
export function TenantGovernancePanel({ tenant }: { tenant: Tenant }) {
  ...
  const users = useUsers({ tenantId: tenant.tenantId, page: 1, pageSize: 100 });
  const devices = useQuery({ queryKey: ["tenants", tenant.tenantId, "devices"], queryFn: () => tenantsApi.listDevices(tenant.tenantId) });
  ...
  const updateCapacity = useMutation({
    mutationFn: () => tenantsApi.update(tenant.tenantId, {
      maximumUsers,
      maximumEnrolledDevices: maximumDevices,
    }),
  ...
  const changeTenantState = useMutation({
    ...
    onSuccess: async () => {
      setConfirmTenantState(false);
      toast.success(tenant.isActive ? "Organización inactivada" : "Organización activada");
      await refresh();
    },
  ...
  const changeUserState = useMutation({
    ...
    onSuccess: async (_, user) => {
      ...
      toast.success(user.isActive ? "Usuario inactivado" : "Usuario activado");
      await refresh();
    },
  ...
  const deactivateDevice = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast.success("Caja desenrolada", { description: "El cupo quedó disponible para enrolar otra caja." });
      await refresh();
    },
  ...
  return <div className="space-y-5">
    <section className="rounded-2xl border bg-card p-6 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">La empresa puede consultar estos límites; solo un administrador global de Auraly puede modificarlos.</p>
        </div>
        <Button variant={tenant.isActive ? "destructive" : "default"} onClick={() => setConfirmTenantState(true)}>
      ...
      {(overUsers || overDevices) && <div className="mt-5 flex gap-3 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950">
        ...
        <p>La organización está en sobrecupo. Los usuarios y cajas actuales permanecen, pero no se permiten nuevas altas, reactivaciones ni enrolamientos hasta volver al límite.</p>
      </div>}
      ...
      <div className="mt-5 grid gap-4 rounded-xl border bg-muted/20 p-4 sm:grid-cols-[1fr_1fr_auto] sm:items-end">
    ...
    <section className="rounded-2xl border bg-card p-6 shadow-sm">
      <div><p className="text-xs font-semibold uppercase tracking-wide text-primary">Acceso</p><h2 className="mt-1 text-xl font-semibold">Usuarios del tenant</h2><p className="mt-1 text-sm text-muted-foreground">Inactiva cuentas concretas para liberar cupo. No se elimina su historial.</p></div>
      <div className="mt-5 overflow-hidden rounded-xl border">
        ...
        {userItems.map((user) => <div key={user.userId} className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-3 border-t px-4 py-3">
          ...
          <Button size="sm" variant="outline" onClick={() => setSelectedUser(user)}>{user.isActive ? "Inactivar" : "Activar"}</Button>
        </div>)}
    ...
    <section className="rounded-2xl border bg-card p-6 shadow-sm">
      <div><p className="text-xs font-semibold uppercase tracking-wide text-primary">Cajas</p><h2 className="mt-1 text-xl font-semibold">Cajas enroladas</h2><p className="mt-1 text-sm text-muted-foreground">Consulta la sede asociada y desenrola una caja para reutilizar el cupo en otra sede.</p></div>
      <div className="mt-5 overflow-hidden rounded-xl border">
        ...
        {(devices.data ?? []).map((device) => <div key={device.deviceId} className="grid grid-cols-[minmax(0,1fr)_minmax(9rem,.6fr)_auto_auto] items-center gap-3 border-t px-4 py-3">
          ...
          {device.isActive ? <Button size="sm" variant="outline" onClick={() => setSelectedDevice(device)}>Desenrolar</Button> : <Badge variant="secondary">Desenrolada</Badge>}
        </div>)}
        {!devices.isLoading && (devices.data ?? []).length === 0 && <p className="border-t p-8 text-center text-sm text-muted-foreground">Sin datos</p>}
      </div>
    </section>
    <Dialog open={confirmTenantState} onOpenChange={setConfirmTenantState}>
      <DialogContent><DialogHeader><DialogTitle>{tenant.isActive ? "¿Inactivar esta organización?" : "¿Activar esta organización?"}</DialogTitle><DialogDescription>{tenant.isActive ? "Se bloquearán nuevos inicios de sesión y toda sincronización de cajas. Las sesiones actuales se revocarán y los datos permanecerán para auditoría." : "Los usuarios activos podrán volver a iniciar sesión. Las sesiones anteriores permanecerán revocadas."}</DialogDescription></DialogHeader><DialogFooter><Button variant="outline" onClick={() => setConfirmTenantState(false)}>Cancelar</Button><Button variant={tenant.isActive ? "destructive" : "default"} disabled={changeTenantState.isPending} onClick={() => changeTenantState.mutate()}>{tenant.isActive ? "Sí, inactivar" : "Sí, activar"}</Button></DialogFooter></DialogContent>
    </Dialog>

    <Dialog open={!!selectedDevice} onOpenChange={(open) => !open && setSelectedDevice(null)}>
      <DialogContent><DialogHeader><DialogTitle>¿Desenrolar esta caja?</DialogTitle><DialogDescription>La caja dejará de sincronizar, se cerrarán sus sesiones abiertas y el cupo quedará disponible. Los documentos existentes permanecen para auditoría.</DialogDescription></DialogHeader><DialogFooter><Button variant="outline" onClick={() => setSelectedDevice(null)}>Cancelar</Button><Button variant="destructive" disabled={!selectedDevice || deactivateDevice.isPending} onClick={() => selectedDevice && deactivateDevice.mutate(selectedDevice)}>Sí, desenrolar</Button></DialogFooter></DialogContent>
    </Dialog>
