...
export function PlatformTenantSubscriptions() {
  ...
  return <div className="mx-auto max-w-[1800px] space-y-6">
    <div><p className="mb-1 text-sm font-medium text-primary">PLATAFORMA AURALY</p>
      <h1 className="text-3xl font-semibold tracking-tight">Suscripciones de tenants</h1>
      <p className="mt-1 text-muted-foreground">Planes, cupos, vigencias y renovaciones de todos los tenants.</p>
    </div>
    <Card>
      <CardHeader><CardTitle>Suscripciones</CardTitle><CardDescription>{result ? `${result.totalCount.toLocaleString("es-CO")} tenants con suscripción` : "Consultando suscripciones…"}</CardDescription></CardHeader>
      <CardContent className="space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row">
          <div className="relative flex-1"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground"/><Input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="Buscar tenant, identificador o correo" className="pl-9"/></div>
          <Select value={status} onValueChange={(value) => { setStatus(value); setPage(1); }}><SelectTrigger className="sm:w-52"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="all">Todos los estados</SelectItem><SelectItem value="Active">Activas</SelectItem><SelectItem value="PastDue">En gracia</SelectItem><SelectItem value="Suspended">Suspendidas</SelectItem><SelectItem value="Cancelled">Canceladas</SelectItem></SelectContent></Select>
        </div>
        <div className="overflow-x-auto rounded-xl border"><Table><TableHeader><TableRow><TableHead>Tenant</TableHead><TableHead>Plan</TableHead><TableHead>Estado</TableHead><TableHead>Vigencia</TableHead><TableHead>Cupos contratados</TableHead><TableHead>Documentos usados</TableHead><TableHead>Renovación</TableHead><TableHead/></TableRow></TableHeader>
          ...
          {result?.items.map((item) => <TableRow key={item.subscriptionId}>
            ...
            <TableCell><p className="font-medium">{item.planName}</p><p className="text-xs text-muted-foreground">{item.billingPeriod === "Annual" ? "Anual" : "Mensual"}</p></TableCell>
            <TableCell><Status value={item.status}/></TableCell>
            <TableCell className="whitespace-nowrap">Hasta {date.format(new Date(item.currentPeriodEnd))}</TableCell>
            <TableCell className="min-w-64 text-xs"><span className="font-medium">{item.fullUserLimit}</span> usuarios · <span className="font-medium">{item.sellerUserLimit}</span> vendedores<br/><span className="font-medium">{item.posDeviceLimit}</span> cajas · <span className="font-medium">{item.payrollEmployeeLimit}</span> empleados · <span className="font-medium">{item.dianDocumentMonthlyLimit.toLocaleString("es-CO")}</span> DIAN</TableCell>
            <TableCell>{item.dianDocumentsUsed.toLocaleString("es-CO")} / {item.dianDocumentMonthlyLimit.toLocaleString("es-CO")}</TableCell>
...
function Status({ value }: { value: string }) {
  ...
  const labels: Record<string, string> = { Active: "Activa", PastDue: "En gracia", Suspended: "Suspendida", Cancelled: "Cancelada", Draft: "Borrador", PendingPayment: "Pendiente", PaymentConfirmed: "Pagada", Invoicing: "Facturando", Activated: "Activada", Expired: "Vencida", PaymentFailed: "Pago fallido" };
