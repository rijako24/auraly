...
export function ProductPicker({
  ...
  return <div ref={pickerRef} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setOpen(false); }} className="relative [&_strong]:font-normal">
    ...
    {open && <div id={`${inputId}-results`} ref={listRef} role="listbox" onScroll={scroll} className={`${resultsMode === "popover" ? "absolute z-30 shadow-xl" : "relative min-h-40"} mt-1 max-h-72 w-full overflow-auto rounded-xl border bg-popover p-1`}>
      {query.isLoading ? <p className="flex items-center gap-2 p-4 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Buscando productos…</p> : query.isError ? <div className="p-4 text-sm text-red-700"><p>No fue posible cargar los productos.</p><Button className="mt-3" size="sm" variant="outline" onClick={() => void query.refetch()}>Reintentar</Button></div> : products.length === 0 ? <p className="p-4 text-sm text-muted-foreground">No hay productos activos que coincidan con la búsqueda.</p> : <><div className="px-3 py-2 text-xs text-muted-foreground">{products.length.toLocaleString("es-CO")} de {totalCount.toLocaleString("es-CO")} productos</div>{products.map((product, index) => <button key={product.productId} type="button" role="option" aria-selected={activeIndex === index} onMouseDown={(event) => event.preventDefault()} onMouseEnter={() => setActiveIndex(index)} onClick={() => choose(product)} className={`flex w-full items-center justify-between gap-4 rounded-lg px-3 py-2.5 text-left text-sm ${activeIndex === index ? "bg-emerald-50 text-emerald-950" : "hover:bg-muted"}`}><span className="min-w-0"><strong className="block truncate">{product.productName}</strong><small className="block truncate text-muted-foreground">{product.productCode}{product.reference ? ` · ${product.reference}` : ""}{conversionOnly && product.conversionFactor ? ` · factor ${product.conversionFactor}` : ""}</small></span><span className="flex shrink-0 items-center gap-3 text-xs text-muted-foreground">Saldo {product.quantityOnHand}{selectedProductIds.has(product.productId) && <Check className="h-4 w-4 text-emerald-700" aria-label="Agregado" />}</span></button>)}{query.hasNextPage && <Button type="button" variant="ghost" className="mt-1 w-full" disabled={query.isFetchingNextPage} onMouseDown={(event) => event.preventDefault()} onClick={() => void query.fetchNextPage()}>{query.isFetchingNextPage && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Cargar 50 más</Button>}</>}
    </div>}
  </div>;
}
