...
import { Barcode, Boxes, Check, CircleDollarSign, PackagePlus, ReceiptText, Scale, Tags, Truck } from "lucide-react";
...
export function ProductCreateWorkspace({ open, onOpenChange, onCreated }: Props) {
  ...
  const create = useMutation({
    ...
    onSuccess: async (product) => {
      ...
      setForm(initialState); setBarcode(""); setValidationError(undefined);
          ...
          <div className="min-h-0 flex-1 overflow-y-auto scroll-smooth p-4 sm:p-6">
            <div className="mx-auto max-w-5xl space-y-5">
              ...
              <Section id="new-identity" icon={PackagePlus} title="Identidad" description="Lo que el equipo verá al buscar y vender.">
                <div className="grid gap-4 md:grid-cols-2"><Field label="Referencia"><Input value={form.reference} onChange={(e) => setForm({ ...form, reference: e.target.value })} placeholder="Referencia del fabricante" /></Field><Field label="Nombre *" className="md:col-span-2"><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Nombre claro para venta y búsqueda" /></Field><Field label="Descripción" className="md:col-span-2"><Textarea className="min-h-24" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Presentación, uso y detalles relevantes" /></Field></div>
              </Section>
              <Section id="new-classification" icon={Tags} title="Clasificación, marca y unidad" description="Auraly conserva la ruta completa y la unidad real en la que se vende.">
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2"><Field label="Marca"><Select value={form.productBrandId ?? none} onValueChange={(value) => setForm({ ...form, productBrandId: value === none ? null : value })}><SelectTrigger><SelectValue placeholder="Sin marca" /></SelectTrigger><SelectContent><SelectItem value={none}>Sin marca</SelectItem>{(brands.data ?? []).map((item) => <SelectItem key={item.productBrandId} value={item.productBrandId}>{item.name}</SelectItem>)}</SelectContent></Select></Field><Field label="Unidad en la que se vende"><Select value={form.baseUnitCode} onValueChange={(value) => setForm({ ...form, baseUnitCode: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{(units.data ?? []).map((item) => <SelectItem key={item.productUnitId} value={item.code}>{item.name} · {item.symbol}</SelectItem>)}</SelectContent></Select><p className="text-xs text-muted-foreground">Describe qué cantidad se vende; la regla de fracciones pertenece al producto.</p></Field></div>
              </Section>
              ...
              <Section id="new-supplier" icon={Truck} title="Proveedor principal y empaque habitual" description="Opcional. Permite recibir por caja, bulto o paquete y convertir a la unidad del producto.">
                <div className="grid items-start gap-4 lg:grid-cols-3"><Field label="Proveedor principal"><Select value={form.supplierId ?? none} onValueChange={(value) => setForm({ ...form, supplierId: value === none ? null : value })}><SelectTrigger><SelectValue placeholder="Sin proveedor" /></SelectTrigger><SelectContent><SelectItem value={none}>Asignar después</SelectItem>{(options.data?.suppliers ?? []).map((supplier) => <SelectItem key={supplier.supplierId} value={supplier.supplierId}>{supplier.name} · {supplier.identification}</SelectItem>)}</SelectContent></Select></Field><Field label="Código del proveedor"><Input value={form.supplierProductCode} onChange={(e) => setForm({ ...form, supplierProductCode: e.target.value })} /></Field><Field label="Empaque en que lo entrega"><Select value={form.packageName} onValueChange={(value) => setForm({ ...form, packageName: value })}><SelectTrigger><SelectValue placeholder="Selecciona el empaque" /></SelectTrigger><SelectContent>{purchasePresentationOptions.map((option) => <SelectItem key={option} value={option}>{option}</SelectItem>)}</SelectContent></Select></Field><Field label="Contenido por empaque"><Input type="number" min={0.000001} step="0.001" value={form.unitsPerPackage} onChange={(e) => setForm({ ...form, unitsPerPackage: Number(e.target.value) })} /></Field></div>
              </Section>
              <Section id="new-taxes" icon={CircleDollarSign} title="IVA, costo y precio preparado" description="El IVA se incluye en el precio de venta; publicar sigue siendo una decisión explícita.">
                <div className="grid gap-4 md:grid-cols-3"><Field label="IVA de venta *"><Select value={form.salesTaxProfileId} onValueChange={(value) => { const rate = taxes.data?.find((item) => item.taxProfileId === value)?.rate ?? 0; const next = recalculateProductPricing("cost", form.cost, { cost: form.cost, margin: form.margin, salePrice: form.salePrice, salesTaxRate: rate }); setForm({ ...form, salesTaxProfileId: value, salePrice: next.salePrice }); }}><SelectTrigger><SelectValue placeholder="Selecciona" /></SelectTrigger><SelectContent>{(taxes.data ?? []).map((tax) => <SelectItem key={tax.taxProfileId} value={tax.taxProfileId}>{tax.name} · {tax.rate}%</SelectItem>)}</SelectContent></Select></Field><Field label="IVA de compra *"><Select value={form.purchaseTaxProfileId} onValueChange={(value) => { const rate = taxes.data?.find((item) => item.taxProfileId === value)?.rate ?? 0; setForm({ ...form, purchaseTaxProfileId: value, purchaseTaxTreatment: rate === 0 ? "NotApplicable" : form.purchaseTaxTreatment === "NotApplicable" ? "DeductibleInputVat" : form.purchaseTaxTreatment }); }}><SelectTrigger><SelectValue placeholder="Selecciona" /></SelectTrigger><SelectContent>{(taxes.data ?? []).map((tax) => <SelectItem key={tax.taxProfileId} value={tax.taxProfileId}>{tax.name} · {tax.rate}%</SelectItem>)}</SelectContent></Select></Field><Field label="Tratamiento del IVA de compra"><Select value={form.purchaseTaxTreatment} disabled={(purchaseTax?.rate ?? 0) === 0} onValueChange={(value) => setForm({ ...form, purchaseTaxTreatment: value as CreateState["purchaseTaxTreatment"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="DeductibleInputVat">IVA descontable</SelectItem><SelectItem value="CapitalizedCost">Mayor valor del costo</SelectItem><SelectItem value="NotApplicable">No aplica</SelectItem></SelectContent></Select></Field></div>
                <div className="mt-5 space-y-5">
                  ...
                  <div className="grid gap-4 lg:grid-cols-2"><MoneyField label="Costo base" kind="currency" value={form.cost} onChange={(value) => changePricing("cost", value)} /><MoneyField label="Margen sobre el precio antes de IVA" kind="percent" value={form.margin} onChange={(value) => changePricing("margin", value)} /></div>
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-4 text-emerald-950">
                    ...
                    <div className="grid items-stretch gap-2 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr]"><CreateFormulaValue label="Costo base" value={form.cost} /><CreateFormulaSign value={`÷ (1 − ${form.margin.toLocaleString("es-CO")}%)`} /><CreateFormulaValue label="Precio antes de IVA" value={netSalePrice} detail={`Margen: ${formatCurrency(marginAmount)}`} /><CreateFormulaSign value="+" /><CreateFormulaValue label={`IVA de venta (${salesTaxRate}%)`} value={Math.max(0, form.salePrice - netSalePrice)} /><CreateFormulaSign value="=" /><div className="rounded-xl bg-emerald-600 p-3 text-white"><Label className="text-xs text-white">Precio de venta preparado · IVA incluido</Label><FormattedNumberInput className="mt-2 h-11 border-white/30 bg-white text-lg font-bold text-emerald-950" kind="currency" value={form.salePrice} onValueChange={(next) => changePricing("salePrice", next ?? 0)} /><p className="mt-2 text-xs text-white/75">Al cambiarlo se conserva el costo y se recalcula el margen.</p></div></div>
                  ...
                  <p className="text-xs text-muted-foreground">El precio queda preparado. Solo se publica desde Rentabilidad y precios.</p>
                </div>
              </Section>
            </div>
          </div>
...
function MoneyField({ label, kind, value, onChange }: { label: string; kind: "currency" | "percent"; value: number; onChange: (value: number) => void }) { return <div className="rounded-xl border bg-background p-4"><Label>{label}</Label><FormattedNumberInput className="mt-3 h-12 text-lg font-semibold" kind={kind} value={value} onValueChange={(next) => onChange(next ?? 0)} /></div>; }
