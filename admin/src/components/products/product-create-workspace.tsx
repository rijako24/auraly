...
export function ProductCreateWorkspace({ open, onOpenChange, onCreated }: Props) {
  ...
  const units = useQuery({ queryKey: ["product-units"], queryFn: productMerchandisingApi.units, enabled: open, staleTime: 5 * 60 * 1000 });
  const taxes = useQuery({ queryKey: ["tax-profiles", businessId], queryFn: () => taxProfilesApi.list(false), enabled: open && !!businessId, staleTime: 5 * 60 * 1000 });
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const supplier = form.supplierId && supplierDetails ? { supplierId: form.supplierId, ...supplierDetails } : null;
            ...
            <div className="mx-auto max-w-5xl space-y-5">
              ...
              <Section id="new-family" icon={Link2} title="Familia de productos" description="Relaciona presentaciones, colores o tallas. Cada producto conserva su inventario y precio, salvo que elijas compartirlos.">
                ...
                <div className="mt-3 space-y-3">
                  ...
                  {linkedProducts.map((item, index) => <article key={item.childProductId} className="rounded-xl border p-4">
                    <div className="mb-3 flex items-start justify-between gap-3"><div><p className="font-semibold">{item.childProductName}</p><p className="text-xs text-muted-foreground">{item.childProductCode || "Sin código"}</p></div><Button type="button" size="icon" variant="ghost" aria-label={`Quitar ${item.childProductName}`} onClick={() => setLinkedProducts((current) => current.filter((_, currentIndex) => currentIndex !== index))}><Trash2 className="h-4 w-4" /></Button></div>
                    <div className="grid items-stretch gap-3 md:grid-cols-3">
                      <FamilyOption label="Compartir inventario" detail="Cada venta descontará del producto principal." checked={item.sharesInventory} onToggle={(checked) => updateLinked(index, { sharesInventory: checked, inventoryFactor: checked ? item.inventoryFactor ?? 1 : null, allowsConversion: checked ? false : item.allowsConversion, conversionFactor: checked ? null : item.conversionFactor })}>
                        <FamilyNumberField label="Unidades del principal por cada unidad vendida" disabled={!item.sharesInventory} value={item.inventoryFactor} onValue={(value) => updateLinked(index, { inventoryFactor: value })} />
                      </FamilyOption>
                      <FamilyOption label="Vincular costo" detail="Deriva el costo del principal y prepara el precio conservando el margen propio." checked={item.sharesPrice} onToggle={(checked) => updateLinked(index, { sharesPrice: checked, priceFactor: checked ? item.priceFactor ?? 1 : null })}>
                        <FamilyNumberField label="Multiplicador del costo principal" disabled={!item.sharesPrice} value={item.priceFactor} onValue={(value) => updateLinked(index, { priceFactor: value })} />
                      </FamilyOption>
                      <FamilyOption label="Permitir conversión" detail="Habilita conversiones con los integrantes permitidos." checked={item.allowsConversion} onToggle={(checked) => { updateLinked(index, { allowsConversion: checked, conversionFactor: checked ? item.conversionFactor ?? 1 : null, sharesInventory: checked ? false : item.sharesInventory, inventoryFactor: checked ? null : item.inventoryFactor }); if (checked) { setForm((current) => ({ ...current, manageInventory: true })); setConversionMaximumLossPercent((current) => current ?? 0); } }}>
                        <div className="grid grid-cols-2 gap-3">
                          <FamilyNumberField label="Unidades equivalentes al producto principal" disabled={!item.allowsConversion} value={item.conversionFactor} onValue={(value) => updateLinked(index, { conversionFactor: value })} />
                          <FamilyNumberField label="Merma máxima permitida (%)" disabled={!item.allowsConversion} value={conversionMaximumLossPercent} kind="percent" onValue={setConversionMaximumLossPercent} />
                        </div>
                      </FamilyOption>
                    </div>
                  </article>)}
...
function FamilyOption({ label, detail, checked, onToggle, children }: { label: string; detail: string; checked: boolean; onToggle: (checked: boolean) => void; children: React.ReactNode }) { return <div className="flex h-full min-w-0 flex-col rounded-xl border bg-background p-3"><div className="flex min-h-16 items-start justify-between gap-3"><div><p className="text-sm font-medium">{label}</p><p className="text-xs text-muted-foreground">{detail}</p></div><Switch checked={checked} onCheckedChange={onToggle} /></div><div className="mt-auto border-t pt-3">{children}</div></div>; }
function FamilyNumberField({ label, disabled, value, kind, onValue }: { label: string; disabled: boolean; value: number | null; kind?: "percent"; onValue: (value: number | null) => void }) { return <div className="min-w-0 space-y-1.5"><Label className="flex min-h-10 items-end text-xs leading-tight">{label}</Label><FormattedNumberInput disabled={disabled} kind={kind} value={value ?? ""} onValueChange={onValue} /></div>; }
