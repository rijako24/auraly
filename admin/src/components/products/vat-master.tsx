...
export function VatMaster({ canManage }: { canManage: boolean }) {
  ...
  return <>
    ...
    <Dialog open={editing !== undefined} onOpenChange={(value) => !value && setEditing(undefined)}><DialogContent><DialogHeader><DialogTitle>{editing ? "Editar" : "Nuevo"} IVA</DialogTitle></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="vat-code">Código interno</Label><Input id="vat-code" value={code} onChange={(event) => setCode(event.target.value)} placeholder="IVA-19" /></div><div className="space-y-2"><Label htmlFor="vat-rate">Tarifa %</Label><Input id="vat-rate" inputMode="decimal" value={rate} onChange={(event) => setRate(event.target.value)} placeholder="19" /></div><div className="space-y-2 sm:col-span-2"><Label htmlFor="vat-name">Nombre</Label><Input id="vat-name" value={name} onChange={(event) => setName(event.target.value)} placeholder="IVA 19%" /></div></div><label className="flex items-center justify-between rounded-xl border p-3"><span><strong className="block text-sm">IVA activo</strong><small className="text-muted-foreground">Los productos anteriores conservan el valor usado.</small></span><Switch checked={active} onCheckedChange={setActive} /></label><DialogFooter><Button variant="outline" onClick={() => setEditing(undefined)}>Cancelar</Button><Button onClick={() => save.mutate()} disabled={save.isPending || !code.trim() || !name.trim() || !Number.isFinite(Number(rate.replace(",", ".")))}>Guardar</Button></DialogFooter></DialogContent></Dialog>
  </>;
}
