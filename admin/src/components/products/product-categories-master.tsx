...
export function ProductCategoriesMaster({ canManage }: { canManage: boolean }) {
  ...
  return <div className="space-y-5">
    ...
    <Dialog open={!!editor} onOpenChange={(open) => !open && setEditor(null)}><DialogContent><DialogHeader><DialogTitle>{editor?.category ? "Editar" : "Crear"} {editor ? levels[editor.level].toLocaleLowerCase("es") : "clasificación"}</DialogTitle></DialogHeader>
      ...
      <div className="grid gap-4 sm:grid-cols-[1fr_140px]"><div className="space-y-2"><Label>Nombre</Label><Input autoFocus value={editor?.name ?? ""} onChange={(event) => editor && setEditor({ ...editor, name: event.target.value })} /></div><div className="space-y-2"><Label>Orden visual</Label><Input type="number" value={editor?.order ?? "0"} onChange={(event) => editor && setEditor({ ...editor, order: event.target.value })} /></div></div>
