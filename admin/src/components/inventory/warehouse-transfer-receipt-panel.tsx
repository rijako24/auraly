...
export function WarehouseTransferReceiptPanel({ businessId, warehouses }:{ businessId:string;warehouses:Warehouse[] }) {
  ...
  const valid=Boolean(detail.data)&&detail.data!.lines.every(line=>{const value=Number(quantities[line.lineNumber]);return Number.isFinite(value)&&value>=0&&value<=line.pendingQuantity;})&&detail.data!.lines.some(line=>Number(quantities[line.lineNumber])>0)&&(!changed||Boolean(differenceReason));
  ...
  return <Card>
    ...
    <CardContent className="space-y-4">
      ...
      {detail.data&&<>
        ...
        {changed&&<div className="space-y-2"><Label>Motivo de la diferencia</Label><Select value={differenceReason} onValueChange={setDifferenceReason}><SelectTrigger><SelectValue placeholder="Selecciona el motivo" /></SelectTrigger><SelectContent>{(reasons.data??[]).map(item=><SelectItem key={item.inventoryReasonId} value={item.code}>{item.name}</SelectItem>)}</SelectContent></Select></div>}
        ...
        <div className="flex gap-2"><Button disabled={!valid||receive.isPending} onClick={()=>receive.mutate()}>{receive.isPending?"Procesando…":"Confirmar entrada"}</Button><Button variant="outline" onClick={()=>setSelectedId(null)} disabled={receive.isPending}>Cancelar</Button></div>
      </>}
