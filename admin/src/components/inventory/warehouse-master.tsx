...
export function WarehouseMaster({ canManage }: { canManage: boolean }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
    ...
    <Card><CardHeader><CardTitle>{selected ? "Editar bodega" : "Nueva bodega"}</CardTitle><CardDescription>El código interno lo genera Auraly.</CardDescription></CardHeader><CardContent className="space-y-4">
      ...
      <label className="flex items-center justify-between rounded-xl border p-3"><span><b className="block text-sm">Bodega de venta</b><small className="text-muted-foreground">Al activarla estará disponible en ventas, inventario y recepción. Las bodegas internas nunca aparecen en esos procesos.</small></span><Switch checked={form.useForSales} onCheckedChange={(value) => setForm({ ...form, useForSales: value })} disabled={!canManage || selected?.isSystem} /></label>
