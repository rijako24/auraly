...
const emptyForm = { name: "", allowNegativeStockSales: false, priceFormationCostBasis: "LatestReceiptCost", useForSales: true, useForGoodsReceipts: true, isInventoryVisible: true, isActive: true };
...
export function WarehouseMaster({ canManage }: { canManage: boolean }) {
  ...
  const edit = (item: WarehouseMasterItem) => { setSelected(item); setForm({ name: item.name, allowNegativeStockSales: item.allowNegativeStockSales, priceFormationCostBasis: item.priceFormationCostBasis, useForSales: item.useForSales, useForGoodsReceipts: item.useForGoodsReceipts, isInventoryVisible: item.isInventoryVisible, isActive: item.isActive }); };
  return <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><Boxes className="h-5 w-5 text-primary" />Bodegas</CardTitle><CardDescription>El inventario, la política de negativos y el costo se administran por bodega.</CardDescription></CardHeader><CardContent>
      ...
      <div className="space-y-2">{rows.map((item) => <button key={item.warehouseId} type="button" onClick={() => edit(item)} className="flex w-full items-center gap-3 rounded-xl border p-4 text-left hover:bg-muted/40"><span className="flex-1"><b>{item.name}</b><small className="mt-1 block text-muted-foreground">{item.code} · {item.isSystem ? "Bodega del sistema" : [item.useForSales && "Venta", item.useForGoodsReceipts && "Recepción", item.isInventoryVisible && "Inventario"].filter(Boolean).join(" · ")}</small></span><Badge variant={item.isActive ? "secondary" : "outline"}>{item.isActive ? "Activa" : "Inactiva"}</Badge><Pencil className="h-4 w-4 text-muted-foreground" /></button>)}{!rows.length && <p className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">No hay bodegas que coincidan.</p>}</div>
    </CardContent></Card>
    <Card><CardHeader><CardTitle>{selected ? "Editar bodega" : "Nueva bodega"}</CardTitle><CardDescription>El código interno lo genera Auraly.</CardDescription></CardHeader><CardContent className="space-y-4">
      ...
      <label className="flex items-center justify-between rounded-xl border p-3"><span><b className="block text-sm">Disponible para ventas</b><small className="text-muted-foreground">Aparece en facturación y en el cambio de ubicación del POS.</small></span><Switch checked={form.useForSales} onCheckedChange={(value) => setForm({ ...form, useForSales: value })} disabled={!canManage || selected?.isSystem} /></label>
      <label className="flex items-center justify-between rounded-xl border p-3"><span><b className="block text-sm">Disponible para recepción</b><small className="text-muted-foreground">Aparece al registrar entradas de mercancía.</small></span><Switch checked={form.useForGoodsReceipts} onCheckedChange={(value) => setForm({ ...form, useForGoodsReceipts: value })} disabled={!canManage || selected?.isSystem} /></label>
      <label className="flex items-center justify-between rounded-xl border p-3"><span><b className="block text-sm">Visible en inventario</b><small className="text-muted-foreground">Permite consultas y movimientos manuales desde Inventario.</small></span><Switch checked={form.isInventoryVisible} onCheckedChange={(value) => setForm({ ...form, isInventoryVisible: value })} disabled={!canManage || selected?.isSystem} /></label>
