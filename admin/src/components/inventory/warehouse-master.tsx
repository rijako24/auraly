...
const emptyForm = { name: "", allowNegativeStockSales: false, priceFormationCostBasis: "LatestReceiptCost", isActive: true };
...
export function WarehouseMaster({ canManage }: { canManage: boolean }) {
  ...
  const edit = (item: WarehouseMasterItem) => { setSelected(item); setForm({ name: item.name, allowNegativeStockSales: item.allowNegativeStockSales, priceFormationCostBasis: item.priceFormationCostBasis, isActive: item.isActive }); };
  return <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><Boxes className="h-5 w-5 text-primary" />Bodegas</CardTitle><CardDescription>El inventario, la política de negativos y el costo se administran por bodega.</CardDescription></CardHeader><CardContent>
      ...
      <div className="space-y-2">{rows.map((item) => <button key={item.warehouseId} type="button" onClick={() => edit(item)} className="flex w-full items-center gap-3 rounded-xl border p-4 text-left hover:bg-muted/40"><span className="flex-1"><b>{item.name}</b><small className="mt-1 block text-muted-foreground">{item.code} · {item.priceFormationCostBasis === "WeightedAverageCost" ? "Costo promedio" : "Último costo"} · {item.allowNegativeStockSales ? "Permite negativos" : "Bloquea negativos"}</small></span><Badge variant={item.isActive ? "secondary" : "outline"}>{item.isActive ? "Activa" : "Inactiva"}</Badge><Pencil className="h-4 w-4 text-muted-foreground" /></button>)}{!rows.length && <p className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">No hay bodegas que coincidan.</p>}</div>
    </CardContent></Card>
    <Card><CardHeader><CardTitle>{selected ? "Editar bodega" : "Nueva bodega"}</CardTitle><CardDescription>El código interno lo genera Auraly.</CardDescription></CardHeader><CardContent className="space-y-4">
      <div className="space-y-2"><Label>Nombre</Label><Input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} disabled={!canManage} /></div>
      ...
      <label className="flex items-center justify-between rounded-xl border p-3"><span><b className="block text-sm">Bodega activa</b><small className="text-muted-foreground">Conserva el historial cuando se desactiva.</small></span><Switch checked={form.isActive} onCheckedChange={(value) => setForm({ ...form, isActive: value })} disabled={!canManage} /></label>
      <div className="flex gap-2"><Button className="flex-1" onClick={() => save.mutate()} disabled={!canManage || !form.name.trim() || save.isPending}>{selected ? "Guardar cambios" : "Crear bodega"}</Button>{selected && <Button variant="outline" onClick={() => { setSelected(null); setForm(emptyForm); }}>Cancelar</Button>}</div>
    </CardContent></Card>
  </div>;
}
