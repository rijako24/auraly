...
test("facturación electrónica abre sin numeración por equipo y orienta si falta el perfil tributario", async ({ page }) => {
  ...
  await page.route("**/api/commerce/v1/fiscal/configuration/devices**", async (route) => { requestedDeviceNumbering = true; await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ businessId, availableConsecutives: 0, devices: [] }) }); });
  ...
  expect(requestedDeviceNumbering).toBe(false);
});
